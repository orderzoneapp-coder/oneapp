...
- 왼쪽 고정 패널은 안정 거래처 키 + 주문번호(없으면 그룹) + 창고를 한 주문서 행으로 묶고 `주문번호·창고·담당자·지역·거래처·대표 품목 외 N건·단위별 수량·공급가액·적요(직원)`을 표시한다. 담당자·지역 조회 필터는 전체 업무량을 바꾸지 않으며, 필터 중 재배정해 행이 빠져도 필터와 다음 행 접근을 유지한다. 기본 작업 화면에서는 중앙을 가장 넓게 유지하면서 좌측 폭을 380px 기준으로 확보하고, 좁은 좌측에서는 필터 초기화 버튼을 다음 행 전체 폭으로 배치한다. 주문번호와 담당자 열은 가로 이동 중에도 고정하고 표 최소폭을 992px로 제한하며 글자 크기를 줄여 맞추지 않는다.
- 왼쪽과 중앙의 담당자 입력은 같은 고객 단위 명령을 사용한다. 거래처 ID/코드가 있으면 현재 작업의 같은 거래처 전체에, 없으면 같은 주문·창고 배송 단위에만 적용한다. 혼합·일부 미지정은 단일 담당자로 숨기지 않고 같은 값의 재입력은 이력을 추가하지 않는다.
- 오른쪽 보조 섹션은 위치·닫기·다시 열기·독립 너비·열림 상태 보존까지만 구현한다. 처리결과·이력·후속 작업 내용은 별도 승인 전 권고안이다. 닫기·다시 열기는 사용자가 지정한 `hidden`/`aria-hidden` 상태를 기준으로 처리하고, 사용자가 다시 연 뒤 표 갱신만으로 닫지 않는다.
