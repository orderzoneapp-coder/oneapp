...
- 오른쪽 재고 패널은 명시적으로 선택한 상품만 표시한다. 초기 무선택과 선택 상품의 `재고자료 없음`을 구분하고, 자료가 없는 경우 상품 식별과 주문수량은 유지하되 재고·예상잔량을 0으로 만들거나 다른 상품으로 전환하지 않는다. 닫기·다시 열기는 사용자가 지정한 `hidden`/`aria-hidden` 상태를 기준으로 처리하고, 닫힘 CSS의 계산 결과를 열림 판정에 재사용하지 않는다. 다시 열 때 선택 상품·재고 내용·저장된 폭·중앙 입력을 유지한다.
