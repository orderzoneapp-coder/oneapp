...
- 왼쪽 고정 패널은 안정 거래처 키 + 주문번호(없으면 그룹) + 창고를 한 주문서 행으로 묶고 `주문번호·창고·담당자·지역·거래처·대표 품목 외 N건·단위별 수량·공급가액·적요(직원)`을 표시한다. 담당자·지역 조회 필터는 전체 업무량을 바꾸지 않으며, 필터 중 재배정해 행이 빠져도 필터와 다음 행 접근을 유지한다.
...
- 오른쪽 재고 패널은 명시적으로 선택한 상품만 표시한다. 초기 무선택과 선택 상품의 `재고자료 없음`을 구분하고, 자료가 없는 경우 상품 식별과 주문수량은 유지하되 재고·예상잔량을 0으로 만들거나 다른 상품으로 전환하지 않는다.
