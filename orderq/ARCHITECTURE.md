...
Version: 0.8.9
Reviewed: 2026-09-03
Official voucher boundary review: `NEXUS-SI-V2-06C` / 2026-09-03
...
vNext 0.8.9 keeps the 0.7.1 `input → document history → operations` work surfaces and the separate official-voucher background synchronization boundary. It retains the ORDER Q-owned official command Adapter and Gateway used by SmartInput purchase/sale finalize, the default-off V2 stocktake checkpoint decision boundary, and the read-only unresolved-product review model plus pure rematch-impact preview. Phase 6C adds a default-off owner command for explicit inventory rematching without adding a product UI, Store, migration, Cloud activation, correction, or cancellation path. The document list and existing optimistic-revision editing behavior remain unchanged.
...
- The V2 owner façade is the ORDER Q `OfficialCommandGateway`, backed by the ORDER Q `OfficialVoucherRepository`. This is an app-owner boundary, not the NEXUS common Gateway. SmartInput purchase/sale finalize uses it through the ORDER Q command Adapter, and Phase 6C exposes rematch build/commit only through the same Adapter/Gateway. Legacy remote product-resolution replay is routed through the Gateway; the former raw local rematch writer fails closed. Correction and cancellation remain follow-up work. The Repository continues to recheck company, command idempotency, expected Revision, identity and transaction atomicity.
