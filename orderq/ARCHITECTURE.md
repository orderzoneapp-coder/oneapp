...
Version: 0.8.5
Reviewed: 2026-09-02
Official voucher boundary review: `NEXUS-SI-V2-05` / 2026-09-02
...
- A matched effect dated before the latest confirmed stocktake, or on the same date when order cannot be proven, requires an explicit user decision before official commit. Selection is per latest confirmed `companyId + productCode + warehouseId` checkpoint, with the existing hidden `productId` accepted only as a checkpoint compatibility key. Command `occurredAt` and checkpoint `confirmedAt` are audit timestamps, not business-order evidence. `included` preserves the voucher and checkpoint, records `ABSORBED_BY_CHECKPOINT`, and does not change current stock; `not included` preserves both and creates exactly one deterministic linked `APPLIED_AS_LATE_ADJUSTMENT` movement; `cancel` writes no official document, line, Revision, inventory, AR/AP, command or queue record. The command payload and Revision preserve the decision, exact target/effect/checkpoint, `businessDate`, actor and judgment time. The current rematch batch's automatic `RESOLVED_WITHOUT_MOVEMENT_AFTER_STOCKTAKE` behavior is not extended in this phase; later rematch work must consume the shared pure checkpoint classifier before it can meet V2 completion.
