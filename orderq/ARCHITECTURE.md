...
`/orderq/dispatch.html` is the M3 local dispatch workbench. `saveDispatchDraft()` persists editable decisions, lines, and warehouse allocations behind a revision boundary. `releaseDispatch()` creates only local work-list reservations; it cannot write sales documents, inventory movements, or fulfillment records. Reservations reduce available inventory without changing on-hand inventory and are released by recall or expiry. Worker-reported quantities and exceptions remain provisional work facts. Confirmed dispatch side effects belong exclusively to M4.
