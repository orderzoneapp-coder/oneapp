...
Version: 0.8.2
Reviewed: 2026-09-02
Official voucher boundary review: `NEXUS-SI-V2-02` / 2026-09-02
...
vNext 0.8.2 keeps the 0.7.1 `input → document history → operations` work surfaces and the separate official-voucher background synchronization boundary. It adds the ORDER Q-owned official command Adapter and Gateway used by SmartInput purchase/sale finalize without changing official transaction behavior. The document list derives representative product and total quantity without storing duplicate summary fields. Expanded documents edit customer, warehouse, assignee, delivery date, workflow states, memo, products, quantities, and prices with the existing optimistic-revision boundary. Item additions, removals, and field changes are appended to the existing `ORDER_EVENT` detail payload. A fully cancelled document remains immutable except for assignee and administrator state, including `CHECKED → UNCHECKED`.
...
`NEXUS-SI-V2-02` changes only the local call boundary for SmartInput purchase/sale finalize. SmartInput UI calls separate purchase/sale Finalize Services; the existing builders call `official-command-adapter.js`, then `official-command-gateway.js`, then `official-voucher-repository.js`. The Gateway transparently delegates to the Repository, so the current preflight, IDs, conversion, AR/AP creation, local queue and one-transaction rollback remain the compatibility baseline. Cloud replay still calls the Repository directly; rematch, correction and cancellation owner routing remain explicit follow-up work. This phase changes no HTML, CSS, IndexedDB schema/data, feature gate, Cloud deployment or production activation, and does not implement optional/unmatched-customer or stocktake decisions.
