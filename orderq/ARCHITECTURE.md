...
- Inspection and commit both require owner-resolved `customerId`, `warehouseId`, and every item's `productId`, stored code and name. One unresolved item quarantines only that whole candidate as `REVIEW_REQUIRED` with zero writes; another normal candidate still commits. A potentially matching legacy actual-ledger bundle with incomplete owner identity is `EXISTING_LEDGER_BUNDLE_INVALID`, never evidence for an exact `NEW` decision.
