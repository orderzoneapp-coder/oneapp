...
M4 adds `confirmDispatch()` for NORMAL exact and partial dispatch. One IndexedDB readwrite transaction changes `RELEASED` to `CONFIRMED` and writes the operational sales fact with ERP status `READY`, negative `SALE_ISSUE` movements, normal order-fulfillment events, consumed reservations, decision history, reconciliation warnings, and local Outbox rows. A failure at any checkpoint aborts every side effect. The command is idempotent per dispatch revision, preserves raw negative inventory, and leaves partial remainder available for a later draft. Multi-warehouse partial actuals must be explicit. Substitute products, conversion, measurement, and over-dispatch approval remain outside M4, while multi-device confirmation remains disabled until M9.
