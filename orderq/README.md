...
단가 셀의 ▲▼ 버튼이나 키보드 위·아래 화살표는 1원 증감 대신 공통 마스터의 `출고가 → 도매A → 도매B → 상장가 → 시중가 → 행사가` 중 값이 있는 항목을 순환한다. 항목명과 금액이 함께 바뀌며 직접 금액을 수정하면 `직접입력`으로 표시한다. 선택한 단가 종류는 주문 품목의 `priceType`으로 보존한다.
