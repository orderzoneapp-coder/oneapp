...
import {
  commitPreparedImport, getCollectorSnapshot, rebuildFulfillmentEvidence, rollbackImportBatch,
  saveCollectorSettings, confirmParserEvidence, cancelParserEvidenceConfirmation,
...
async function renderSnapshot(){
  ...
  const ledgerCount=(s.ledgerLines||[]).length; $('#chip-ledger').textContent=ledgerCount?number(ledgerCount):'없음';
  const ready=matchingReady(s); const reviewCount=s.links.filter(r=>r.requiresReview && r.historicalOrderLineId && r.salesLineId && Number(r.allocatedQuantity||0)>=0).length;
  ...
  if(!ready){
    ...
    const targetOrders=s.orderLines.length; const matchedOrderIds=new Set(validLinks.filter(r=>!r.requiresReview).map(r=>r.historicalOrderLineId)); const autoRate=targetOrders?Math.round(matchedOrderIds.size/targetOrders*100):0;
    $('#matchingSummary').innerHTML=`<div><span>주문행</span><strong>${number(targetOrders)}</strong></div><div><span>판매행</span><strong>${number(s.salesLines.length)}</strong></div><div><span>자동매칭률</span><strong>${autoRate}%</strong></div><div><span>확인필요</span><strong>${number(reviewCount)}</strong></div>`;
  ...
  $('#historyRows').innerHTML=s.batches.length?s.batches.map(r=>`<tr><td>${esc(formatTime(r.importedAt))}</td><td>${esc(SOURCE_LABEL[r.sourceType]||r.sourceType)}</td><td>${esc(r.fileName)} / ${esc(r.sheetName)}</td><td class="num">${number(r.insertedRowCount??r.rowCount)}</td><td>${stateBadge(r.status)}</td><td>${r.status==='COMMITTED'?`<button class="btn danger" data-rollback="${esc(r.importBatchId)}">롤백</button>`:''}</td></tr>`).join(''):'<tr><td colspan="6" class="empty">수집 이력이 없습니다.</td></tr>';
  return s;
}
...
function applyWorkTab(work){
  ...
  if(isCollection){ const c=WORK[work]; $('#workTitle').textContent=c.label; $('#workDescription').textContent=c.description; $('#dropTitle').textContent=`${c.label} 파일을 여기에 놓으세요`; $('#commitBtn').textContent=c.action; $('#orderMethodTabs').classList.toggle('hidden',work!=='order'); if(work!=='order'){activeOrderMethod='excel'; showOrderMethod();} renderPrepared(); }
}
...
$('#mismatchBox').addEventListener('click',e=>{const detail=e.target.closest('[data-mismatch-detail]');const move=e.target.closest('[data-mismatch-move]');const exclude=e.target.closest('[data-mismatch-exclude]');if(detail&&lastMismatch)show(`${lastMismatch.result.fileName}: ${SOURCE_LABEL[lastMismatch.result.sourceType]||lastMismatch.result.sourceType} · ${number(lastMismatch.result.rows.length)}행`,'info');if(move&&lastMismatch){const result=lastMismatch.result;prepared.push(result);lastMismatch=null;renderMismatch();applyWorkTab(move.dataset.mismatchMove);}if(exclude){lastMismatch=null;renderMismatch();}});
...
$('#photoCandidates').addEventListener('click',async e=>{const b=e.target.closest('[data-photo-analyze]');if(!b)return;const d=photoDrafts[Number(b.dataset.photoAnalyze)];if(!d?.text.trim())return show('사진 주문내용을 먼저 확인해 입력해 주세요.','warn');try{const result=await analyzeHistoricalText({rawText:d.text,sourceId:`PHOTO:${d.name}`,fileName:`${d.name}.txt`,defaultDate:$('#textDate').value});prepared.push(result);renderPrepared();show(`사진 주문 후보 ${number(result.rows.length)}행을 만들었습니다. 수집 확정 전에는 매칭하지 않습니다.`);}catch(err){show(err.message||String(err),'error');}});
$('#commitBtn').addEventListener('click',async()=>{const items=currentPrepared();if(!items.length)return;const button=$('#commitBtn');button.disabled=true;try{let inserted=0,skipped=0;const types=[];for(const item of items){const r=await commitPreparedImport(item);inserted+=r.inserted;skipped+=r.skipped;types.push(item.sourceType);prepared.splice(prepared.indexOf(item),1);}renderPrepared();await maybeRebuildAfterSourceChange(types);await renderSnapshot();await bestEffortSync();const label=WORK[activeWork].label;const suffix=activeWork==='sales'?(await getCollectorSnapshot()).orderLines.length?' · 주문자료와 자동매칭했습니다.':' · 주문자료를 수집하면 판매결과와 자동매칭합니다.':'';show(`${label} 수집 완료: ${number(inserted)}행 · 중복제외 ${number(skipped)}행${suffix}`);}catch(e){show(`수집 실패: ${e.message||e}`,'error');}finally{button.disabled=!currentPrepared().length;}});
...
$('#historyRows').addEventListener('click',async e=>{const b=e.target.closest('[data-rollback]');if(!b||!confirm('이 수집 배치를 롤백할까요? 원본 감사기록은 유지됩니다.'))return;try{const before=await getCollectorSnapshot();const batch=before.batches.find(r=>r.importBatchId===b.dataset.rollback);await rollbackImportBatch(b.dataset.rollback);if([COLLECTOR_SOURCE.ORDER,COLLECTOR_SOURCE.KAKAO,COLLECTOR_SOURCE.SALES].includes(batch?.sourceType)){const after=await getCollectorSnapshot();if(matchingReady(after))await rebuildFulfillmentEvidence();}await renderSnapshot();await bestEffortSync();show('수집 배치를 롤백했습니다.');}catch(err){show(err.message||String(err),'error');}});
