...
export async function closeSeriesIdentity(companyId,closeBusinessDate,currency='KRW',companyWideScopeIdentity=''){
  if(!text(companyId)||text(currency)!=='KRW')throw new Error('CLOSE_SCOPE_INVALID');
  return `CLOSE-${await stableCloseId('CLOSE_SERIES',text(companyId),date(closeBusinessDate),'KRW')}`;
}
...
export async function planCloseRevision({series=null,actionType,expectedSeriesHeadRevision=0,expectedEffectiveRevision=0,targetRevision=null,sourceSealReceipt,sourceADigest,resultBDigest,issueDecisionDigest:decisionDigest,actorId,commandId,idempotencyKey,freshVerification={},closeCloudDeployment={},closeAlgorithmVersion=CLOSE_ALGORITHM_VERSION}){
  ...
  const closeRevisionId=`CR-${await stableCloseId(closeSeriesId,revision,actionType,sourceADigest,resultBDigest,closeAlgorithmVersion)}`;
...
export function deriveFinancialCloseRows({entries=[],voucherEvents=[],customers=[]},kind,closeRevisionId=''){
  ...
  return ordered.map(entry=>{const event=exactEventForEntry(entry,voucherEvents,entityType),snapshot=event.afterSnapshot||event.beforeSnapshot||event.after||event.before||{},businessDate=text(snapshot.businessDate||snapshot.purchaseDate||snapshot.saleDate);if(!businessDate)throw new Error('FINANCIAL_ENTRY_BUSINESS_DATE_MISSING');const original=finite(entry.totalAmount),net=original+visit(entry.entryId);if((original>0&&net<0)||(original<0&&net>0))throw new Error('FINANCIAL_ENTRY_OVER_REVERSAL');const customer=customersById.get(text(entry.partnerId))||{};return {closeFinancialRowId:`CFR-${closeRevisionId}-${entry.entryId}`,closeRevisionId,entryId:entry.entryId,entryType:entry.entryType,partnerId:entry.partnerId,partnerCode:customer.customerCode||'',partnerName:customer.customerName||'',displayPartnerSnapshotSource:'CLOSE_SNAPSHOT_A',currency:entry.currency||'KRW',ledgerSequence:entry.ledgerSequence,supplyAmount:finite(entry.supplyAmount),totalAmount:original,vatAmount:finite(entry.vatAmount??0),taxType:entry.taxType,sourceDocumentRevision:entry.sourceDocumentRevision,effectOrdinal:entry.effectOrdinal,effectKey:entry.effectKey,commandId:entry.commandId,idempotencyKey:entry.idempotencyKey,occurredAt:entry.occurredAt,createdAt:entry.createdAt,netAmount:net,derivedStatus:net===0?'FULLY_REVERSED':net!==original?'PARTIALLY_ADJUSTED':'EFFECTIVE',documentType:payable?'PURCHASE':'SALE',documentId:entry[idField],voucherEventId:event.eventId,documentBusinessDate:businessDate,reversalOf:entry.reversalOf||''};});
}
...
export async function buildCloseResultSnapshot({closeRevisionId,closeSeriesId,inventoryRows=[],orderRows=[],purchaseRows=[],salesRows=[],receivableRows=[],payableRows=[],issues=[],auditEvents=[]}){
  ...
  const logical=JSON.parse(JSON.stringify(result));delete logical.closeRevisionId;for(const rows of Object.values(logical).filter(Array.isArray))for(const row of rows)for(const key of Object.keys(row))if(key==='closeRevisionId'||/^close.*(?:RowId|RevisionId)$/i.test(key)||key==='auditEventId')delete row[key];
  result.resultBDigest=await canonicalSha256(logical);result.resultSnapshotId=`CRS-${result.resultBDigest.slice(0,40)}`;return result;
}
