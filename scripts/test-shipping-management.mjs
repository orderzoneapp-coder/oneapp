...
assert.match(orderOpsHtml, /brand-badge">v1\.51</, "ORDER Q visible version must be v1.51");
...
assert.match(orderOpsHtml, /ORDER Q v1\.51 · 출고관리/,
...
assert.ok(orderOpsHtml.includes('class="execution-panel"'),
  "the public v1.51 execution controls must be separate from the upload strip");
...
assert.match(orderOpsHtml, /ORDER Q v1\.51: align System\.IO directly under the global header[\s\S]*?\.page-shell\s*\{\s*padding-top:\s*0;/,
...
for (const requiredInteractionContract of [
  ...
  assert.ok(orderOpsHtml.includes(requiredInteractionContract),
    `public ORDER Q v1.51 interaction contract is missing: ${requiredInteractionContract}`);
}
...
assert.match(html, /brand-badge">v1\.51</, "canonical ORDER Q visible version must be v1.51");
...
for (const requiredInteractionContract of [
  ...
  assert.ok(html.includes(requiredInteractionContract),
    `canonical ORDER Q v1.51 interaction contract is missing: ${requiredInteractionContract}`);
}
...
for (const contract of [
  ...
  assert.ok(html.includes(contract), `ORDER Q v1.51 contract is missing: ${contract}`);
}
