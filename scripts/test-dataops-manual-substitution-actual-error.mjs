...
for (const field of ["기초", "입고", "출고", "대체입고", "대체출고", "실사"]) {
  assert.equal(sourceAfter[field], actualSource[field], `manual substitution must preserve source ${field}`);
  assert.equal(targetAfter[field], salesTarget[field], `manual substitution must preserve target ${field}`);
}

const duplicateRuntime = createRuntime();
const duplicateFirst = duplicateRuntime.run(
  [actualSource, salesTarget],
  actualSource.batchKey,
  salesTarget.batchKey,
);
const duplicateSecond = duplicateRuntime.run(
  duplicateFirst.rows,
  actualSource.batchKey,
  salesTarget.batchKey,
);
assert.equal(duplicateSecond.history.length, 1, "the same resolved pair must not create duplicate history");
assert.deepEqual(duplicateSecond.alerts, []);
