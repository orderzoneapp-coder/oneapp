...
assert.match(auth, /const COMPANY_FOOTER_VERSION = '1\.1\.0'/);
  ...
  if (url.pathname === '/__company_footer_test__') {
    ...
    response.end(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><script>
      ...
      window.fetch = (url, options = {}) => {
        if (!String(url).includes('script.google.com/macros/s/')) return nativeFetch(url, options);
        window.__gatewayCalls += 1;
        const request = JSON.parse(options.body || '{}');
        window.__knownRevisions.push(request.knownRevision);
        if (request.action !== 'nexus_public_company_snapshot') return Promise.reject(new Error('ACTION_DENIED'));
        ${scenario === 'error'
          ? "return Promise.reject(new Error('OFFLINE'));"
          : `return new Promise(resolve => setTimeout(() => resolve({ ok: true, json: () => Promise.resolve({ status: 'success', action: 'nexus_public_company_snapshot', data: ${JSON.stringify(result)} }) }), 25));`}
      };
      window.ONEAPP_AUTH = {
