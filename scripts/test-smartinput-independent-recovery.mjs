...
for (const marker of ['parser-card', 'photoResizer', 'workbench', 'workspace--single', 'tableScroll', 'estimateLibraryView', 'catalogPickerList', 'linkedEstimateList']) {
  assert.match(html, new RegExp(marker), `${marker} must remain in the single-workspace SmartInput shell`);
}
assert.match(html, /class="header-customer-group"[\s\S]*id="customerInput"/, 'customer entry must live in the app header');
assert.match(html, /id="sourcePanelToggleButton"/, 'source intake must be collapsible without losing the work table');
assert.doesNotMatch(html, /class="related-panel"/, 'the obsolete fixed right-side panel must stay removed');
