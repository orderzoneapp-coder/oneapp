...
assert.match(html, /const RECOVERY_INITIAL_VISIBLE = 5/);
assert.match(html, /id="recoveryMoreButton"[^>]*>더보기<\/button>/);
assert.match(html, /state\.recoveryRecords\.slice\(0, RECOVERY_INITIAL_VISIBLE\)/);
...
assert.match(html, /\.workspace-storage\s*\{[^}]*width:\s*min\(1180px, 100%\);[^}]*overflow-x:\s*hidden;/);
...
assert.match(html, /id="downloadButton"[^>]*disabled[^>]*>[\s\S]*?엑셀출력 F10/);
