...
try {
  ...
  const linkedSourceId = await evaluate(client, `(() => {const choices=[...document.querySelectorAll('.linked-source-edit-dialog [data-source-choice]')];const choice=choices[1];choice.click();return choice.value.split(':')[0];})()`);
  ...
  assert.equal(await evaluate(client, `new Promise((resolve,reject)=>{const request=indexedDB.open('oneapp-smartinput',5);request.onerror=()=>reject(request.error);request.onsuccess=()=>{const db=request.result;const tx=db.transaction('estimates','readonly');const get=tx.objectStore('estimates').getAll();get.onerror=()=>reject(get.error);get.onsuccess=()=>{resolve(get.result.filter(record=>record.estimateKind!=='LINKED_GROUP').every(record=>!record.draft?.rows?.some(row=>Number(row.quantity)===9)));db.close();};};})`), true, 'one failed write must roll back the entire IndexedDB bundle');
  assert.equal(await evaluate(client, `document.querySelector('#inputRows [data-field="quantity"]').value`), '9', 'failed atomic save must preserve the working input');
  ...
  await click(client, '.linked-source-edit-dialog [data-confirm-source]');
  await expr(client, `new Promise((resolve,reject)=>{const request=indexedDB.open('oneapp-smartinput',5);request.onerror=()=>reject(request.error);request.onsuccess=()=>{const db=request.result;const tx=db.transaction('estimates','readonly');const get=tx.objectStore('estimates').getAll();get.onerror=()=>reject(get.error);get.onsuccess=()=>{resolve(get.result.find(record=>record.estimateId===${JSON.stringify(linkedSourceId)})?.draft?.rows?.some(row=>Number(row.quantity)===9));db.close();};};})`, 'selected linked source write-through record');
  assert.equal(await evaluate(client, `new Promise((resolve,reject)=>{const request=indexedDB.open('oneapp-smartinput',5);request.onerror=()=>reject(request.error);request.onsuccess=()=>{const db=request.result;const tx=db.transaction('estimates','readonly');const get=tx.objectStore('estimates').getAll();get.onerror=()=>reject(get.error);get.onsuccess=()=>{resolve(get.result.filter(record=>record.estimateKind!=='LINKED_GROUP'&&record.estimateId!==${JSON.stringify(linkedSourceId)}).every(record=>!record.draft?.rows?.some(row=>Number(row.quantity)===9)));db.close();};};})`), true, 'unselected original estimates must remain unchanged');
