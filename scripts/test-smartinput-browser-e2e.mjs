...
try {
  ...
  assert.ok(metrics.resizer.width > 0, 'desktop must preserve the parser resizer');
  assert.equal(await evaluate(client, `!document.querySelector('#estimateLibraryView').classList.contains('is-open')&&getComputedStyle(document.querySelector('#estimateLibraryView')).pointerEvents==='none'`), true,
    'the saved-estimate and related-voucher dialog must be closed by default');
  assert.equal(metrics.columns.trim().split(/\s+/).length, 3, 'the closed dialog must not reserve a workspace column');
  ...
  assert.deepEqual({ open: visualZones.dialogOpen, related: visualZones.relatedButtonText }, { open: false, related: '관련 전표 불러오기' },
    'the default order workspace must expose only an explicit related-voucher action');
  assert.deepEqual({ estimateHeadingHidden: visualZones.estimateHeadingHidden, estimateListHidden: visualZones.estimateListHidden }, { estimateHeadingHidden: true, estimateListHidden: true });
  ...
  assert.equal(await evaluate(client, `document.querySelector('.related-panel').classList.contains('is-open')&&document.querySelector('#relatedPanelToggle').getAttribute('aria-expanded')==='true'&&document.querySelector('#relatedDialogTitle').textContent.trim()==='관련 전표 불러오기'&&!document.querySelector('#voucherContextView').hidden&&document.querySelector('#catalogPickerList').hidden`), true,
    'the explicit related-voucher action must open the shared dialog without estimate management');
  assert.equal(await evaluate(client, `document.activeElement===document.querySelector('#relatedPanelCloseButton')`), true, 'opening the dialog must move focus to its close control');
  await click(client, '#relatedPanelCloseButton');
  await expr(client, `!document.querySelector('.related-panel').classList.contains('is-open')&&document.querySelector('#relatedPanelToggle').getAttribute('aria-expanded')==='false'`, 'related-voucher dialog close');
  ...
  await input(client, '#inputRows tr[data-default-row="true"] [data-product-search]', '마스터');
  await evaluate(client, `(() => {const rows=[...document.querySelectorAll('#inputRows tr[data-row-id]:not([data-default-row])')];rows.at(-1).querySelector('[data-product-search]').dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));return true;})()`);
  ...
  assert.equal(officialSyncResult.acked >= 3, true);
  ...
  await click(client, '#relatedPanelToggle');
  await evaluate(client, `(() => {const select=document.querySelector('#voucherActivitySourceMode');select.value='order';select.dispatchEvent(new Event('change',{bubbles:true}));return true;})()`);
  await expr(client, `document.querySelector('#voucherContextDelivery').textContent.includes('조회 완료')&&document.querySelector('#voucherContextList').textContent.includes('격리 검증 거래처')`, 'date-scoped order activity adapter');
  ...
  await click(client, '#estimateListButton');
  await expr(client, `document.querySelector('#estimateLibraryView').classList.contains('is-open')&&document.querySelector('#relatedDialogTitle').textContent.trim()==='견적서 목록'&&document.querySelector('#voucherContextView').hidden&&!document.querySelector('#catalogPickerList').hidden`, 'on-demand estimate list dialog');
  ...
  assert.equal(estimateRailFooter.height <= 48, true, 'estimate dialog footer must remain compact');
  assert.deepEqual(estimateRailFooter.buttons.map(button => button.id), ['selectedEstimateDeleteButton', 'estimateRenameButton'], 'estimate dialog footer must contain only deletion and rename');
  assert.equal(new Set(estimateRailFooter.buttons.map(button => button.y)).size, 1, 'estimate list actions must remain horizontal');
  assert.deepEqual(await evaluate(client, `[...document.querySelectorAll('#estimateOutputActions .button')].map(button=>button.id)`), ['saveEstimateAsButton', 'estimateNoticeButton', 'estimateExcelButton'], 'estimate table footer must keep only Save As, Kakao, and Excel outputs');
  await click(client, '#relatedPanelCloseButton');
  await click(client, '#addRowButton');
  ...
  await wait(350);
  await click(client, '#estimateListButton');
  await expr(client, `document.querySelector('#estimateLibraryView').classList.contains('is-open')&&!document.querySelector('#catalogPickerList').hidden`, 'saved estimate list opened on request');
  assert.equal(await evaluate(client, `document.querySelectorAll('#estimateLibraryLinkedButton,#estimateMultiSelectButton,#estimateCreateButton,#linkedEstimateList').length`), 0,
    'complex linked-estimate management must not be exposed');
  const focusedEstimateListShot = await capture(client, 'smartinput-estimate-list-dialog.png');
  await click(client, '#catalogPickerList .estimate-card:not(.is-selected) [data-select-estimate-card]');
  await expr(client, `document.querySelectorAll('#catalogPickerList .is-selected').length===1&&!document.querySelector('#estimateLibraryView').classList.contains('is-open')`, 'selecting one estimate must return to the work table');
  await click(client, '#estimateListButton');
  ...
  await click(client, '#relatedPanelCloseButton');
  await expr(client, `!document.querySelector('#estimateExcelButton').disabled`, 'estimate rename completion');
  ...
  await expr(client, `document.querySelectorAll('#catalogPickerList [data-estimate-id]').length===1`, 'single selected estimate deletion');
  ...
  if (await evaluate(client, `document.querySelector('#estimateLibraryView').classList.contains('is-open')`)) await click(client, '#relatedPanelCloseButton');
  ...
  await wait(300);
  const intermediateClosed = await evaluate(client, `(() => {const workspace=document.querySelector('#smartInputWorkspace');const panel=document.querySelector('#estimateLibraryView');return {open:panel.classList.contains('is-open'),columns:getComputedStyle(workspace).gridTemplateColumns,resizer:Boolean(document.querySelector('#relatedPanelResizer')),backdropHidden:document.querySelector('#relatedDialogBackdrop').hidden};})()`);
  assert.deepEqual({ open: intermediateClosed.open, resizer: intermediateClosed.resizer, backdropHidden: intermediateClosed.backdropHidden }, { open: false, resizer: false, backdropHidden: true }, 'intermediate workspace must start without a permanent side panel');
  assert.equal(intermediateClosed.columns.trim().split(/\s+/).length, 3, 'intermediate workspace must retain parser, resizer, and work table only');
  await click(client, '#relatedPanelToggle');
  const intermediatePanel = await evaluate(client, `(() => {const panel=document.querySelector('#estimateLibraryView').getBoundingClientRect();const close=document.querySelector('#relatedPanelCloseButton').getBoundingClientRect();return {left:panel.left,top:panel.top,right:panel.right,bottom:panel.bottom,width:panel.width,height:panel.height,closeText:document.querySelector('#relatedPanelCloseButton').textContent.trim(),backdropVisible:!document.querySelector('#relatedDialogBackdrop').hidden};})()`);
  console.log('SmartInput intermediate dialog metrics', intermediatePanel);
  assert.ok(intermediatePanel.left >= 0 && intermediatePanel.top >= 0 && intermediatePanel.right <= 1280 && intermediatePanel.bottom <= 900, 'intermediate modal must stay inside the viewport');
  assert.deepEqual({ closeText: intermediatePanel.closeText, backdropVisible: intermediatePanel.backdropVisible }, { closeText: '×', backdropVisible: true });
  await click(client, '#relatedPanelCloseButton');

  await client.send('Emulation.setDeviceMetricsOverride', { width: 390, height: 844, deviceScaleFactor: 1, mobile: true });
  await wait(200);
  assert.equal(await evaluate(client, `document.querySelector('#relatedPanelResizer')===null`), true, 'mobile modal must not expose width resizing');
  await click(client, '#relatedPanelToggle');
  await expr(client, `document.querySelector('#estimateLibraryView').classList.contains('is-open')`, 'mobile related-voucher dialog open');
  assert.equal(await evaluate(client, `(() => {const panel=document.querySelector('#estimateLibraryView').getBoundingClientRect();return panel.left>=0&&panel.top>=0&&panel.right<=innerWidth&&panel.bottom<=innerHeight;})()`), true, 'mobile dialog must fit the viewport');
  await click(client, '#relatedPanelCloseButton');
