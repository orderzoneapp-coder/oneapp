...
try {
  ...
  assert.deepEqual(estimateRailFooter.buttons.map(button => button.id), ['selectedEstimateDeleteButton', 'estimateCreateButton'], 'right rail footer must contain only deletion and linked-estimate creation');
  ...
  await expr(client, `document.querySelector('#estimateCreateButton').textContent.includes('연동견적서 저장')&&document.querySelectorAll('#catalogPickerList .is-selected').length===0`, 'linked creation must start directly with an empty explicit selection');
  await click(client, '#catalogPickerList [data-select-estimate-card]');
  assert.equal(await evaluate(client, `document.querySelector('#estimateCreateButton').disabled`), true, 'one source cannot finalize a linked estimate');
  await click(client, '#catalogPickerList .estimate-card:not(.is-selected) [data-select-estimate-card]');
