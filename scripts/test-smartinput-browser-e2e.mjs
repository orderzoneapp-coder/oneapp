...
try {
  ...
  assert.match(await evaluate(client, `document.querySelector('#voucherContextList .voucher-activity-item')?.getAttribute('href')||''`), new RegExp(`voucher-query\\.html\\?mode=order&date=2026-08-29&focus=${orderResult.orderId}`), 'activity card must route to read-only voucher detail without replacing the worktable');
  await wait(350);
