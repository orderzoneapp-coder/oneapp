...
try {
  ...
  await click(client, '#estimateEditorButton');
  assert.equal(await evaluate(client, `!document.querySelector('.workspace').classList.contains('is-estimate-library-open')&&document.querySelector('#sourceInputPanel').offsetWidth>0&&document.querySelector('#tableScroll').offsetWidth>0`), true, 'returning from the library must restore the independent parser and table');
  await click(client, '#newEstimateButton');
  assert.equal(await evaluate(client, `!document.querySelector('.workspace').classList.contains('is-estimate-library-open')&&!document.querySelector('#estimateEditorView').hidden`), true, 'new estimate must retain the protected editor path');
  await click(client, '#addRowButton');
  ...
  await wait(500);
  await click(client, '#estimateLibraryButton');
  await expr(client, `document.querySelector('.workspace').classList.contains('is-estimate-library-open')`, 'linked editor returns to the explicit full-width library');
