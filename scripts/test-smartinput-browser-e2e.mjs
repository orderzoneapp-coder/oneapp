...
try {
  ...
  assert.equal(await evaluate(client, `document.querySelectorAll('#catalogPickerList .is-selected').length`), 0, 'linked creation must start with an empty explicit selection');
  await click(client, '#catalogPickerList [data-select-estimate-card]');
  assert.equal(await evaluate(client, `document.querySelector('#estimateCreationSaveButton').disabled`), true, 'one source cannot finalize a linked estimate');
  await click(client, '#catalogPickerList .estimate-card:not(.is-selected) [data-select-estimate-card]');
  ...
  await click(client, '#estimateCreationSaveButton');
