...
assert.doesNotMatch(html, /font-?awesome|cdnjs\.cloudflare\.com\/ajax\/libs\/font-awesome/i);
...
assert.match(source, /MASTER_ICON_PATHS/);
assert.match(source, /React\.createElement\(icon\.tag/);
assert.doesNotMatch(source, /\bfas\b|fa-[a-z]|window\.lucide|dangerouslySetInnerHTML/,
  'Master icons must not depend on an unresolved external icon runtime');
const definedIcons = new Set([...source.matchAll(/^\s+'([^']+)': \[/gm)].map(match => match[1]));
const usedIcons = [...source.matchAll(/<SafeIcon name="([^"]+)"/g)].map(match => match[1]);
assert.deepEqual(usedIcons.filter(name => !definedIcons.has(name)), [],
  'every rendered Master icon must have a repository-native SVG definition');
...
assert.doesNotMatch(compiled, /window\.lucide|dangerouslySetInnerHTML|\bfas\b|fa-[a-z]/);
