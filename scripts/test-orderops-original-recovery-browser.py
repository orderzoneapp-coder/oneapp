...
def filter_menu(page, label):
    trigger = page.locator('#previewTable .column-sort-trigger[aria-label=' + json.dumps(label + ' 정렬 및 조건 필터', ensure_ascii=False) + ']')
    trigger.locator('xpath=ancestor::th').hover()
    trigger.click()
...
try:
    with sync_playwright() as pw:
        ...
        for shortcut in ['F5', 'F6', 'F7']:
            ...
            for label in labels:
                trigger = p.locator('#previewTable .column-sort-trigger[aria-label=' + json.dumps(label, ensure_ascii=False) + ']')
                trigger.locator('xpath=ancestor::th').hover()
                trigger.click()
