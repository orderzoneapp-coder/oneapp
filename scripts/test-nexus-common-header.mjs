...
assert.match(componentSource, /const VERSION = '1\.10\.0'/);
...
assert.match(cssSource, /@media \(max-width: 680px\)[\s\S]*\.nav \{ width: 100%; max-width: 100%; min-width: 0;/,
  'mobile header navigation must clip its horizontal track inside the padded host width');
...
assert.match(nexusHome, /nexus-auth\.js\?v=2\.1\.2/);
...
assert.equal(manifestContract.resources.componentVersion, '1.10.0');
...
for (const file of manifestContract.consumers) {
  ...
  assert.match(source, /nexus-top\.js\?v=1\.10\.0/, `${file} must load the current NEXUS component`);
  assert.match(source, /nexus-auth\.js\?v=2\.1\.2/, `${file} must enforce the NEXUS V2 login session`);
}
...
for (const [file, appId] of entries) {
  ...
  assert.match(source, /nexus-top\.js\?v=1\.10\.0/);
  assert.match(source, /nexus-auth\.js\?v=2\.1\.2/);
