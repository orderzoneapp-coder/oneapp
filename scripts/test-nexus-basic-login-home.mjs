...
assert.match(html, /nexus-ui-theme-init\.js\?v=1\.3\.0" data-nexus-app-id="nexus-home"/, 'NEXUS home must initialize the shared theme before paint');
...
assert.match(html, /id="rememberLogin"[^>]+type="checkbox"/, 'login must expose an explicit persistent-login choice');
assert.match(html, /이 컴퓨터에서 로그인 유지/, 'persistent-login wording must identify this computer');
...
assert.match(css, /\.nexus-remember-login\s*\{[^}]*min-height:\s*44px/s, 'remember-login control must retain a touch-sized interaction');
