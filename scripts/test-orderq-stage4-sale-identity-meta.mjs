...
const source={customers,products:[{productId:'P1',productCode:'A',revision:3,status:'ACTIVE'}],warehouses:[{warehouseId:'W1',revision:4,status:'ACTIVE'}],
 orders:[{orderId:'O1',orderNo:'N1',customerId:'D1',revision:5,status:'OPEN'}],orderItems:[{orderItemId:'OI1',orderId:'O1',productId:'P1',sourceRowNumber:2,revision:6,status:'OPEN',baseUnit:'EA',actualToBaseFactor:1,actualToRecognizedFactor:1,conversionSource:'ORDER_Q',conversionRuleVersion:'1'}],dispatches:[],dispatchLines:[]};
const allocation={sourceRowNumber:2,sourceOccurrence:1,sourceRowKey:'ORDER_ROW:2',orderNumber:'N1',productId:'P1',warehouseId:'W1',salesCustomerId:'C1',deliveryCustomerId:'D1',billingCustomerId:'B1',productCode:'A',quantity:0,unit:'EA',unitPrice:10};
...
assert.throws(()=>buildSaleStage4Sidecar({allocations:[{...allocation,sourceOccurrence:''}]},source),/SOURCE_OCCURRENCE_REQUIRED/);
assert.throws(()=>buildSaleStage4Sidecar({allocations:[allocation,{...allocation,sourceRowKey:'ORDER_ROW:2:B'}]},source),/SOURCE_OCCURRENCE_DUPLICATE/);
...
assert.throws(()=>readSalesMeta([Object.keys(meta),Object.keys(meta).map(key=>meta[key]),Object.keys(meta).map(key=>key==='sourceRowKey'?'R2':meta[key])]),/META_(MUTATED|IDENTITY_DUPLICATE)/);
