...
assert.match(itemManager, /\/nexus\/common\/nexus-top\.js\?v=1\.8\.1/);
...
for (const [name, source] of [
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.8\.1/, `${name} must use the shared NEXUS TOP component`);
...
for (const [name, appId] of nexusApps) {
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.8\.1/, `${name} must load NEXUS TOP`);
