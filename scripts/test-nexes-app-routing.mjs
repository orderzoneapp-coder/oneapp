...
const navigationFiles = ["dashboard.html"];
...
assert.match(itemManager, /\/nexus\/common\/nexus-top\.js\?v=1\.10\.0/);
assert.match(itemManager, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.2/);
...
for (const [name, source] of [
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.10\.0/, `${name} must use the shared NEXUS TOP component`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.2/, `${name} must enforce the NEXUS V2 login session`);
...
for (const [name, appId] of nexusApps) {
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.10\.0/, `${name} must load NEXUS TOP`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.2/, `${name} must enforce NEXUS V2 authentication`);
