...
assert.match(itemManager, /\/nexus\/common\/nexus-top\.js\?v=1\.6\.0/);
assert.match(itemManager, /\/nexus\/common\/nexus-auth\.js\?v=1\.0\.0/);
...
for (const [name, source] of [
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.6\.0/, `${name} must use the shared NEXUS TOP component`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=1\.0\.0/, `${name} must enforce the NEXUS login session`);
...
for (const [name, appId] of nexusApps) {
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.6\.0/, `${name} must load NEXUS TOP`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=1\.0\.0/, `${name} must enforce NEXUS authentication`);
