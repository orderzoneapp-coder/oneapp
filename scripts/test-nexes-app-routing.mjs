...
assert.match(itemManager, /\/nexus\/common\/nexus-top\.js\?v=1\.9\.2/);
assert.match(itemManager, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.1/);
...
for (const [name, source] of [
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.9\.2/, `${name} must use the shared NEXUS TOP component`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.1/, `${name} must enforce the NEXUS V2 login session`);
...
for (const [name, appId] of nexusApps) {
  ...
  assert.match(source, /\/nexus\/common\/nexus-top\.js\?v=1\.9\.2/, `${name} must load NEXUS TOP`);
  assert.match(source, /\/nexus\/common\/nexus-auth\.js\?v=2\.1\.1/, `${name} must enforce NEXUS V2 authentication`);
