...
try {
  ...
  assert.equal(await ev('document.querySelectorAll("#prepareMatchSummary [data-match-state]").length'),3,'loaded Excel exposes compact mapping status');
  assert.equal(await ev('document.querySelectorAll(".orderops-raw-preview table .nexus-table-column-tool").length'),0,'raw preview does not inherit central table search tools');
