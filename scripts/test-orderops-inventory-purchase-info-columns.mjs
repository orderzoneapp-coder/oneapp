...
const html = source.replace('initializeLocalRecovery().then(loadOrderQSourceFromRoute)', 'globalThis.__inventoryColumnTest={state,renderResults,getPreviewDefinitions,renderTableMarkup}; initializeLocalRecovery().then(loadOrderQSourceFromRoute)')
...
try {
  ...
  const inventory=[['사용','품목코드','단위','품목명','규격','수량','1창고','2전송','3서울','4전송','7진영','기본','전송','창고'],['Yes','INV-001','EA','부족 상품','BOX',4,4,'','','','','','',''],['Yes','INV-002','EA','충분 상품','EA',20,20,'','','','','','',''],['Yes','INV-003','EA','재고만 상품','EA',8,8,'','','','','','','']];
  ...
  check('information red follows unit EA despite specification BOX',await ev(`(()=>{const model=__inventoryColumnTest.getPreviewDefinitions(__inventoryColumnTest.state.workspace).inventory.sourceRows.find(row=>row.productCode==='INV-001');const row=document.querySelector('.preview-inventory tr[data-product-code="INV-001"]');const cell=row.querySelector('td.information-value');return model.unit==='EA'&&model.specification==='BOX'&&row.classList.contains('box-unit-row')&&cell.classList.contains('information-non-box-unit')&&getComputedStyle(cell.querySelector('.order-information-cell')).color==='rgb(185, 28, 28)';})()`));
  check('information black follows unit BOX despite specification EA',await ev(`(()=>{const api=__inventoryColumnTest,preview=api.getPreviewDefinitions(api.state.workspace).inventory,index=preview.sourceRows.findIndex(row=>row.productCode==='INV-001');const sourceRow={...preview.sourceRows[index],unit:'BOX',specification:'EA'};const host=document.createElement('div');host.innerHTML=api.renderTableMarkup(preview,[{row:preview.rows[index],sourceRow,index}],{previewId:'inventory',editable:false,resizable:false,sortable:false});document.querySelector('#previewTable').append(host);const row=host.querySelector('tr[data-product-code="INV-001"]'),cell=row?.querySelector('td.information-value'),result=Boolean(row&&!row.classList.contains('box-unit-row')&&cell.classList.contains('information-box-unit')&&getComputedStyle(cell.querySelector('.order-information-cell')).color==='rgb(15, 23, 42)');host.remove();return result;})()`));
  ...
  for(const theme of ['light','dark']){await ev(`document.documentElement.dataset.nexusUiTheme='${theme}';true`);check(`${theme} information non-BOX unit stays red`,await ev(`getComputedStyle(document.querySelector('.preview-inventory tr[data-product-code="INV-001"] td.information-value .order-information-cell')).color==='${theme==='dark'?'rgb(128, 0, 0)':'rgb(185, 28, 28)'}'`));await shot('before-analysis-'+theme);await informationShot('before-analysis-information-'+theme);}
  ...
  await ev(`(()=>{const n=document.querySelector('.purchase-input[data-purchase-code="INV-001"]');n.focus();n.value='열이동 보존 구매처';n.dispatchEvent(new Event('input',{bubbles:true}));const a=document.querySelector('th[data-column-drag-key="${info}"]'),b=document.querySelector('th[data-column-drag-key="${purchase}"]'),dt=new DataTransfer();a.dispatchEvent(new DragEvent('dragstart',{bubbles:true,dataTransfer:dt}));b.dispatchEvent(new DragEvent('drop',{bubbles:true,cancelable:true,dataTransfer:dt,clientX:b.getBoundingClientRect().left}));a.dispatchEvent(new DragEvent('dragend',{bubbles:true,dataTransfer:dt}));return true;})()`);
  check('uncommitted purchase input survives column move with one audited commit',await ev(`document.querySelector('.purchase-input[data-purchase-code="INV-001"]').value==='열이동 보존 구매처' && ShippingManagementEngine.getPurchaseInputs(__inventoryColumnTest.state.workspace)['INV-001']==='열이동 보존 구매처' && __inventoryColumnTest.state.workspace.systemHistory.events.filter(row=>row.kind==='CELL_EDITED'&&row.productCode==='INV-001'&&row.field==='purchase'&&row.previousValue==='분석전 구매처'&&row.nextValue==='열이동 보존 구매처').length===1`));
  await click('[data-preview="allocations"]');await click('[data-preview="inventory"]');
  ...
  check('TOTAL_ONLY information read only',await ev(`document.querySelectorAll('.preview-inventory [data-substitute-order-row],.preview-inventory [data-substitution-target-product]').length===0 && document.querySelector('.preview-inventory .order-information-cell').textContent.includes('거래처') && document.querySelector('.preview-inventory .order-information-cell').getAttribute('aria-readonly')==='true'`));
