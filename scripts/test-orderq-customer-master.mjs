...
assert.match(html, /customer-master-ui\.js\?v=0\.14\.2/, 'Customer Master entry module must invalidate the deployed cache');
...
assert.match(sourceImport, /onProgress\?\.\(\{ phase: 'MATCHING', processed, total: rows\.length \}\)/, 'Excel analysis must report row progress');
assert.match(sourceImport, /setTimeout\(resolve, 0\)/, 'Excel analysis must yield between row chunks');
assert.match(ui, /Excel 읽기 완료/);
assert.match(ui, /거래처 비교 중/);
...
assert.match(ui, /customer-source-import\.js\?v=0\.14\.2/, 'Fast Source Import must invalidate the module cache');
