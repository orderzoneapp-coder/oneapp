...
const line={salesLineId:'SL',lineIdentityId:'LI',sourceLineKey:'1',productId:'P',warehouseId:'W',actualQuantity:1,baseQuantity:10,recognizedOrderQuantity:10,unitPrice:100,orderLinkMode:'ORDER_Q',sourceOrderId:'O1',sourceOrderItemId:'OI1',sourceDispatchId:'D1',sourceDispatchLineId:'DL1'};
...
assert.equal(plan.movements[0].signedBaseQuantity,-10); assert.equal(plan.document.totalAmount,100); assert.equal(plan.orderEvents[0].detail.transferredQty,10);
...
assert.deepEqual(correction.orderEvents.map(row=>[row.eventType,row.orderId,row.detail.transferredQty]),[
  ['SALES_TRANSFER_REVERSED','O1',10],['SALES_TRANSFER_ALLOCATED','O2',10]
]);
...
migrateCentralDrafts(central,{idempotencyKey:'O-MIG',deviceId:'PC',entities:[
  ...
  {entityType:'DISPATCH_LINE',entityId:'DL1',revision:1,payload:{dispatchLineId:'DL1',dispatchId:'D1',orderItemId:'OI1',productId:'P',warehouseId:'W',localOnly:true}},
...
const badDispatch=structuredClone(mutations);badDispatch.find(row=>row.entityType==='ORDER_EVENT').payload.detail.sourceDispatchLineId='DL-WRONG';
assert.throws(()=>commitCentralCommand(central,{idempotencyKey:'O-C',leaseToken:lease.leaseToken,fingerprint:lease.fingerprint,mutations:badDispatch}),/ORDER_EVENT_LINK_INVALID/);
const overAllocated=structuredClone(mutations);overAllocated.find(row=>row.entityType==='ORDER_EVENT').payload.detail.transferredQty=11;
assert.throws(()=>commitCentralCommand(central,{idempotencyKey:'O-C',leaseToken:lease.leaseToken,fingerprint:lease.fingerprint,mutations:overAllocated}),/ORDER_ALLOCATION_LIMIT/);
const centralCommit=commitCentralCommand(central,{idempotencyKey:'O-C',leaseToken:lease.leaseToken,fingerprint:lease.fingerprint,mutations});
const ledgerRows=centralCommit.changes.filter(row=>row.payload.ledgerSequence).sort((a,b)=>a.payload.ledgerSequence-b.payload.ledgerSequence);
assert.deepEqual(ledgerRows.map(row=>row.entityType),['VOUCHER_EVENT','INVENTORY_MOVEMENT','ORDER_EVENT','RECEIVABLE_ENTRY']);
const negativeDoc={...doc,salesDocumentId:'SO-NEG',sourceDocumentKey:'O-NEG'};
const negativeLine={...line,salesLineId:'SL-NEG',lineIdentityId:'LI-NEG',sourceLineKey:'NEG',actualQuantity:-1,baseQuantity:-10,recognizedOrderQuantity:-5,allocationEventId:'ALLOC-PREV'};
const negative=planOfficialVoucherCommand({document:negativeDoc,lines:[negativeLine],command:{commandContract:'VOUCHER_CORE_V1',commandType:'POST_SALE',commandId:'O-NEG-C',idempotencyKey:'O-NEG-C',expectedRevision:1,actor:'A',occurredAt:'2026-08-25T02:00:00Z',document:negativeDoc,lines:[negativeLine]}});
assert.equal(negative.movements[0].signedBaseQuantity,10); assert.equal(negative.orderEvents[0].eventType,'SALES_TRANSFER_REVERSED'); assert.equal(negative.orderEvents[0].detail.transferredQty,5);
