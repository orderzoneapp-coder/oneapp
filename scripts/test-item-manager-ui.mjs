...
assert.match(themeInit, /var VALID_MODES = \["light", "dark"\]/);
assert.match(themeInit, /return mode === "dark" \? "dark" : "light"/);
assert.doesNotMatch(themeInit, /return "system"|prefers-color-scheme/);
assert.match(masterTheme, /var\(--oneapp-bg\)/);
assert.match(masterTheme, /var\(--oneapp-surface\)/);
assert.match(uiContract, /공통헤더의 `일반 \/ 다크` 선택/);
assert.match(uiContract, /기존 `system` 저장값은 `일반`으로/);
