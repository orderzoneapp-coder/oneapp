...
const template = createTemplateRecord({ mode: 'order', name: '미출고 주문', mappings: parsed.mappings, tableFieldIds }, {
...
assert.deepEqual(template.columns.slice(0, 5).map(column => column.fieldId), ['unit', 'itemCode', 'itemName', 'specification', 'quantity']);
...
modeDraft.staging = createStaging({
  sourceHash, rows: normalizedRows, batch, mappings: parsed.mappings,
