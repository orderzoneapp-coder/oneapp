...
- Runtime verification: 7개 앱의 일반/다크 헤더와 공통 표 표시·순백색 인쇄 계약, 6개 작업영역의 독립 폭 조절·상태 보존, OrderOps 분석 전 주문 표시·동일 부모 3개 섹션·고객 배송 단위 집계·담당자 동기화·우측 재열기를 실제 브라우저에서 검증하며 운영 배포 버전은 CI와 Pages에서 별도 대조
...
- `smartinput/index.html` 스마트입력은 이 공통 작업영역 모듈의 적용 대상이 아니며 2026-09-11 사용자 승인 복원 레이아웃, 7개 글로벌헤더와 기존 입력·저장·이력 기능을 유지한다. `orderops/list.html`은 앱헤더 아래 한 작업영역의 직접 자식으로 왼쪽 주문서 요약, 중앙 작업표, 오른쪽 보조 섹션을 배치한다. 왼쪽은 항상 표시하고 중앙은 가용 폭까지 줄인 뒤 표 자체에 가로 스크롤을 제공하며, 960px 이하에서도 OrderOps 좌우 폭 조절을 유지한다.
...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않으며, 코드가 없는 원본은 같은 주문·창고 배송 단위로 제한한다. 주문서가 있으면 재고 등록·출고분석 전에도 왼쪽 주문번호·창고·담당자·지역·거래처·대표 품목·단위별 수량·공급가액·`적요(직원)` 목록과 중앙 `거래처·상품·주문수량·직원 적요` 기본 분석표를 표시한다. 실제 재고자료가 있을 때만 중앙에 `재고·잔량`을 더하고, 재고 부재를 숫자 0으로 만들지 않는다. 구매·판매 업로드는 참고 화면 입력이며 출고 배정의 필수 근거가 아니다.
- OrderOps 담당자 변경은 안정 거래처 ID/코드가 있으면 현 작업공간의 동일 거래처 키 전체 주문행에 적용하고, 코드가 없으면 현재 주문·창고 단위에만 적용하며 기존 system history와 로컬 복구 경계에 기록한다. 중앙 직원 적요는 `note1Original`/`note1`, 일반 적요는 `noteOriginal`/`note`로 분리한다. 분석 전 `ORDEROPS_PREVIEW` 작업공간의 편집과 이력은 실제 분석 성공 시 `shipping-workspace/v2` 작업공간으로 승계하고, 저장·복구·부분출고·충돌·출력 계약은 분석 완료 작업공간에 그대로 적용한다. 오른쪽은 위치·닫기·재열기·독립 폭·열림 상태만 제공하며 처리결과·이력·후속 작업 내용은 별도 승인 전 권고안이다. 사용자가 다시 연 우측은 표 갱신으로 자동 닫지 않는다.
