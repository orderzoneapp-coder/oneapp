...
The public Gateway action is fixed as `nexus_public_company_snapshot`. It accepts only a JSON-number, non-negative safe-integer `knownRevision`, uses the server-held FOUNDATION_READ credential to call only `nexus_gateway_company_public_profile_get`, and returns only a READY exact projection or `UNCHANGED`/`STALE_SERVER` with revision. Target URLs, alternate actions, tokens, protected profile operations, and management operations are denied. A 60-second ScriptCache absorbs repeated public reads; every warm-cache request resolves from cache, including a client revision ahead of the cached revision, which returns `STALE_SERVER` without upstream access. An administrator profile write refreshes that cache from its verified `publicSnapshot`, while other revision-changing company writes invalidate it.
