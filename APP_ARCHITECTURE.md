...
- SmartInput 견적 보고서는 매핑 기반으로 불러온 견적 원본에 한해 `ONEAPP_MERCHOPS_SETTINGS_READ_ADAPTER_V1`에서 현재 마진룰과 견적 매핑을 읽고, 상품 Snapshot의 기준단위와 원본 창고·입고가·외주비·노무비를 사용해 MerchOps의 출고가 자동적용 결과를 재현한다. ERP 출고가는 재계산값, 쇼핑몰 출고가는 양수 행사가 우선값이며, 소분 가격은 MerchOps 견적 매핑이 실제로 소유한 비용 필드만 사용한다. 수기 견적과 사용자가 명시적으로 편집한 출고가는 유지하고 Settings 키·상품 Master·견적 원본에는 쓰지 않는다.
