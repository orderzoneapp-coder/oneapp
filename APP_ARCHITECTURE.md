...
| OrderOps order-view presets | `oneapp.orderops.order-view-presets.v1` | ORDER Q order-status local display preference only; named search, shortage, specification, manager, warehouse, column-filter, and sort conditions are excluded from workspace recovery and cloud plans |
...
18. Quantity presentation preserves the administrator's calculation trail: the calculated column is named `수량`, blank remains blank, zero is muted, and a negative remainder is displayed as the signed numeric result such as `-3` with a restrained warning accent. `발주 N`, `정보없음`, or other explanatory text is never substituted into a numeric quantity cell; review state remains in the separate status column, while purchase-upload export may still derive the required positive quantity from the signed result. Automatic warehouse rainbow fills are disabled by default; only explicit saved color assignments are applied. Purchase-place Tab navigation centers the next verified-shortage row and selects its input. Order-information unit prices use thousands separators on screen and in the integrated workbook without changing numeric source values.
19. Order-status named view presets persist only display conditions in local browser storage: global search, shortage focus, specification, warehouse and manager filters, order-status column filters, and order-status sort. Applying a preset opens order status and discards keys that do not exist in the current workbook; it never changes analyzed rows, editable business values, workspace recovery, cloud plans, column widths, column order, or color assignments. Header-body clicks do not sort; sorting remains available only inside the header filter control.
...
- The default Order Status sequence is warehouse, customer, manager, product code, product name, specification, row state, order, unit price, all warehouse-inventory columns, notice, and purchase. Warehouse is read-only; order, unit price, notice, and purchase remain editable.
...
- `재고부족` focus is shared across F5–F7. F5 keeps verified-shortage order rows only; F6/F7 group same-six-character inventory-backed substitutes after the shortage row. The calculated column remains `수량` and displays a signed result such as `-3`; missing inventory information remains a separate non-shortage state in the status column.
