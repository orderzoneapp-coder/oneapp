...
- Architecture document version: 2.3.8
...
- Runtime verification: 7개 앱의 일반/다크 헤더와 공통 표 표시·순백색 인쇄 계약, 6개 작업영역의 독립 폭 조절·상태 보존, OrderOps 분석 전 저장·복구·동일 부모 3개 섹션·좌측 파서 내부 Excel/API 준비와 보조 복구 메뉴·중앙 작업 전용 하단바·우측 전표 날짜/금액/적요 및 선택 합계·다중 주문 Revision 조정·저장 실패 재시도·출고분석과 구매/판매 참고자료 적용 뒤 원본 기준선/다중 출고 초안 보존·재진입 후 부분 Revision 갱신을 실제 브라우저에서 검증하며 운영 배포 버전은 CI와 Pages에서 별도 대조
...
- OrderOps 오른쪽 주문 요약은 창고·담당·지역 조회 범위의 주문서 수, 단위별 수량, 원본 금액·읽기용 계산 금액·금액 자료 없음 상태를 구분한다. 계산 금액은 원본 `supplyAmount`에 쓰지 않는다. 지역 원본이 없으면 그룹명으로 추정하지 않는다. 중앙 선택행의 일반 적요·직원 전달사항·창고는 해당 작업행, 담당 변경은 기존 거래처/배송 단위 경계를 따른다. 대상별 미적용 입력은 선택 전환 중 보존하며 다른 작업으로 교체하기 전에 명시적으로 적용/취소해야 한다.
...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않는다. `창고·담당·지역` 조회와 담당 건수, `거래처·수량·금액·적요` 요약은 오른쪽에 유지한다. 중앙은 분석 전부터 기존 `allocations` 주문현황의 열 key·편집·동적 창고 열을 사용한다. `readiness`는 기존 복구본/조회 호환용이며 별도 기본 주 화면이 아니다. 구매·판매 단독 원본과 재고 원본은 읽기 전용 조회를 지원한다.
