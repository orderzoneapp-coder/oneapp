...
- Architecture document version: 2.3.2
...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않으며, 코드가 없는 원본은 같은 주문·창고 배송 단위로 제한한다. 왼쪽은 `창고·담당·지역` 조회와 담당자별 건수 선택, `거래처·수량·금액·적요` 네 열만 제공한다. 주문서가 있으면 재고 등록·출고분석 전에도 중앙 `거래처·상품·주문수량·직원 적요` 기본 분석표를 표시하고 실제 재고자료가 있을 때만 `재고·잔량`을 더한다. 구매·판매를 포함한 단독 원본도 각 읽기 전용 표로 표시한다.
- OrderOps 담당자 변경은 중앙 선택 열 또는 주문 상세에서 수행하고 안정 거래처 ID/코드가 있으면 현 작업공간의 동일 거래처 키 전체 주문행에 적용하며, 코드가 없으면 현재 주문·창고 단위에만 적용한다. 중앙 직원 적요는 `note1Original`/`note1`, 일반 적요는 `noteOriginal`/`note`로 분리한다. 분석 전 `ORDEROPS_PREVIEW`는 단일 원본 SHA-256 fingerprint, ORDER Q snapshot, 편집·이력을 검산 저장하며 복구 뒤 분석과 출고를 계속할 수 있다. 최신 검증 완료 분석본은 보관 정리에서 보호한다. 같은 상품코드의 EA·BOX처럼 주문단위가 섞이거나 명시 재고단위가 다르면 상품합계·배정·잔량·구매필요를 계산하지 않고 단위 확인으로 남긴다. 오른쪽은 위치·닫기·재열기·독립 폭·열림 상태만 제공하며 키보드 재열기에서도 중앙 전체 가로 위치를 바꾸지 않는다.
