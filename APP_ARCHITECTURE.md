...
`orderops/list.html` owns the inventory-application transaction. It cancels the pending autosave, waits for an in-flight Cloud save, shipment confirmation, or analysis result, commits the focused editor and shipment draft, verifies a latest-input recovery record, and then rebuilds the candidate against that latest workspace. The candidate is first stored as `STAGED`; automatic recovery and retention ignore this state. A last workspace digest/version check precedes promotion of the same record to `PUBLISHED` and pointer update. Failure, conflict, or timeout deletes the candidate where possible and leaves the visible workspace, user inputs, previous pointer, immutable Cloud revisions, and shipment results unchanged.
