...
20. The sixth `통합파일` uploader is a batch wrapper for the existing order, inventory, purchase, and sales inputs, not a new business-data type. Each workbook sheet is classified first by the administrator's per-kind sheet-name aliases and then must pass the existing required-header and structure validation for that kind. Valid sheets replace only their active kind; a later valid sheet or individual upload of the same kind wins. Missing, ignored, or invalid sheets never clear the previously active kind, so one bad sheet cannot block valid sheets in the same workbook.
