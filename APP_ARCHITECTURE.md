...
20. The separate `통합 Excel 업로드` drop zone is a batch wrapper for the existing order, inventory, purchase, and sales inputs, not a sixth result tab or a new business-data type. The five result tabs are ordered as order, stock ledger, inventory, purchase, and sales. Each workbook sheet is classified first by the administrator's per-kind sheet-name aliases and then must pass the existing required-header and structure validation for that kind. Valid sheets replace only their active kind; a later valid sheet or individual upload of the same kind wins. Missing, ignored, or invalid sheets never clear the previously active kind, so one bad sheet cannot block valid sheets in the same workbook.
