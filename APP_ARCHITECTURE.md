...
- SmartInput 견적·주문·구매의 초기 입력 항목·표시명·Enter 순서는 사용자 지정 불변 preset으로 보존한다. 기존 전표별 저장 설정은 유지하며 `환경설정 → 전표 선택 → 초기 입력 모드로 변경 → 설정 저장`에서 선택 전표의 입력열과 Enter 순서만 복원한다. 입력행·매핑 양식·상단/배송 설정·다른 전표·사용자 필드 정의는 보존한다. 판매 초기 구성은 항목 의미가 미확정이므로 기존값을 유지하고 복원 동작을 활성화하지 않는다.
