...
- `smartinput/index.html` 스마트입력은 이 공통 작업영역 모듈의 적용 대상이 아니며 사용자 승인 복원 레이아웃과 기존 입력·저장·이력 기능을 유지한다. `orderops/list.html`은 앱헤더 아래 작업영역의 직접 자식으로 왼쪽 파일 준비, 중앙 기존 주문현황, 오른쪽 조회·수정을 배치한다. 1100px 이상은 같은 행, 640~1099px는 조절 가능한 왼쪽과 가용 폭 중앙 및 독립 조절 가능한 오른쪽 overlay, 639px 이하는 중앙을 유지한 좌우 overlay이며 이때만 손잡이를 숨긴다. 모바일에서 한쪽을 명시적으로 열면 반대쪽을 닫는다. 데스크톱 폭과 중간 폭은 별도 로컬 키로 보존하며 화면 갱신만으로 사용자가 다시 연 오른쪽을 닫지 않는다.
- OrderOps 오른쪽 주문 목록은 `선택 / 주문일 / 거래처명·수량·금액 / 적요` 4열로 고정하고, 거래처명·단위 안전 수량·원본 또는 읽기용 계산 금액은 한 줄 텍스트로 표시한다. 목록에는 공통 열 검색 아이콘과 가로 스크롤을 만들지 않으며 날짜 상세 입력은 접고 창고·담당은 빠른 선택 버튼으로 조회한다. 선택 합계와 창고·담당 일괄 변경은 목록 바로 아래에 유지한다. 계산 금액은 원본 `supplyAmount`에 쓰지 않고 금액 자료 없음·오류를 별도 의미로 보존한다. 지역 원본이 없으면 그룹명으로 추정하지 않는다. 중앙 선택행의 일반 적요·직원 전달사항·창고는 해당 작업행, 담당 변경은 기존 거래처/배송 단위 경계를 따른다. 대상별 미적용 입력은 선택 전환 중 보존하며 다른 작업으로 교체하기 전에 명시적으로 적용/취소해야 한다.
- OrderOps 오른쪽은 최초 닫힘·열림 상태·폭을 복원하고, 승인된 기존 주문 요약과 선택행 편집·단일 출고 초안·미적용 작업 조회만 제공한다. 메뉴와 출고 확인·충돌 조정은 `document.body` popup/dialog layer를 사용한다. 열 메뉴의 마우스·터치·키보드, 바깥 클릭, Escape 초점 복귀 및 중앙 검색·필터의 기존 계약을 유지한다.
...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않는다. 날짜·창고·담당 조회와 `선택 / 주문일 / 거래처명·수량·금액 / 적요` 4열 요약은 오른쪽에 유지하고, 창고·담당·지역·원본 기준 등 상세 정보는 접힌 보조 영역에서 기존 의미를 보존한다. 중앙은 분석 전부터 기존 `allocations` 주문현황의 열 key·편집·동적 창고 열을 사용하며 열 필터 조작은 평상시 숨기고 머리글 hover·키보드 focus 또는 적용 상태에서만 표시한다. `readiness`는 기존 복구본/조회 호환용이며 별도 기본 주 화면이 아니다. 구매·판매 단독 원본과 재고 원본은 읽기 전용 조회를 지원한다.
- OrderOps 담당 변경은 안정 거래처 ID/코드가 있으면 동일 거래처 전체, 없으면 같은 주문·창고 범위에만 적용한다. 직원 적요 `note1Original`/`note1`과 일반 적요 `noteOriginal`/`note`는 분리한다. 분석 전 `ORDEROPS_PREVIEW` 원본·명시 매핑·원행·SHA·ORDER Q snapshot·이력을 검산 저장한다. 단위 혼합/공란/불일치와 재고 미확인은 정상 잔량·구매 대상으로 해석하지 않는다. 발주현황·통합/개별 구매업로드·출력 안내는 `getFinalPurchaseUploadSelection()`의 같은 작업본 최종 대상/수량을 사용한다. F10 구매업로드 대상/수량 이외 시트·열·서식·필드 계약은 유지한다.
- OrderOps 앱헤더는 주문·재고·발주·구매·판매·수불 현황 탭을 중앙에, 출고분석·초기화·설정을 우측 끝에 둔다. 좁은 화면은 단일 행 내부 가로 접근을 유지한다. 검색·필터·정렬·열 설정과 파일 준비 재열기·작업 상태는 중앙 상단 한 조작부, 저장 상태/F8은 중앙 하단 왼쪽, 자료 불러오기·출고확정·F9/F10·다음 작업은 하단 오른쪽에 둔다. 별도 파일 준비 재열기 행·중복 패널 제목·중앙 상태 행을 만들지 않는다. 확인 필요 상태는 조작부에 보이고 전체 상태는 body popover에서 열며 Escape 초점 복귀와 live 안내를 보존한다.
- OrderOps 데스크톱 작업영역의 파일 준비 패널은 좌측 끝, 조회·수정 패널은 우측 끝에 고정한다. 패널을 닫으면 해당 열만 제거하고 중앙 작업표가 남은 좌우 경계 사이 전체를 차지한다. 양쪽을 모두 닫으면 중앙 작업표가 작업영역 전체 중심이 된다. 좁은 화면의 좌·우 패널도 각각 왼쪽·오른쪽 가장자리에서 열리며 중앙 작업표의 기준 열은 바꾸지 않는다.
- 파일 선택과 파일 준비 영역의 Excel drop은 같은 준비 경로를 사용하고 기존 미적용 자료에 추가한다. 왼쪽에서 유형·시트·헤더·마지막 상품행(Excel 1-based, 포함)·열을 검증한 후 명시 적용한다. 새 준비의 시작은 헤더 다음 행이다. `explicitMapping.dataEndRowIndex`는 0-based 포함 끝행이며 주문/재고/구매/판매가 동일하게 소비한다. 지정 끝행 아래 원문은 계산·적용에서 제외하되 전체 행렬·cells·SHA·sourceEvidence를 보존하고 원래 sourceRowNumber를 유지한다. 끝행 없는 기존 매핑은 기존 시작행부터 전체 범위를 그대로 사용한다. 범위/정수 오류는 적용을 차단하고 자동 보정하지 않는다. 저장/복구·재적용·시트 전환에서 선택 범위를 유지한다. 원본 소유권·단축키·출력 양식은 배치 변경으로 바꾸지 않는다.
- 최신 주문 적용은 원본 B/작업 W/최신 N을 안정 행 ID로 조정하고 충돌 선택·대체/편집 이력·삭제행 초안을 보존한다. 예약 저장 취소 → 진행 중 저장/출고/분석 결과 확인 → 최신 입력 저장/검산 → STAGED 후보 검산 → 버전 재확인 → 짧은 입력 잠금과 원자적 PUBLISHED 게시 → 화면 교체 순서이다. STAGED는 일반 복구 대상이 아니다. 확정된 조정 기록은 `ORDER_SOURCE_RECONCILED` 이력과 작업 metadata로 로컬/F8에 보존한다. 혼합/원본 차이 창고·담당과 `REVIEW_REQUIRED`는 UI 및 명령 adapter에서 추가 출고를 차단한다. 기존 확정 결과를 수정하거나 자동 취소하지 않는다.
...
| Common inventory snapshot | `ONEAPP_INVENTORY_SNAPSHOT_V1`; `oneapp-common-inventory-read/v1`; Cloud `InventorySnapshotStaging`, `InventorySnapshotHistory`, `InventorySnapshotIndex` | OrderOps explicitly saves verified ERP warehouse inventory and can apply one indexed revision on another browser. The finalized DataOps snapshot is consumed only as `TOTAL_ONLY`: original LOT rows stay traceable, safe exact-code/name/spec/unit groups may be summed, and no warehouse, shipment allocation, or purchase proposal is synthesized. Inventory apply candidates remain `STAGED` and are excluded from recovery until the latest workspace inputs and the candidate are reverified and the record becomes `PUBLISHED`. |
...
`orderops/list.html` owns the inventory-application transaction. It cancels the pending autosave, waits for an in-flight Cloud save, shipment confirmation, or analysis result, commits the focused editor and shipment draft, verifies a latest-input recovery record, and then rebuilds the candidate against that latest workspace. The candidate is first stored as `STAGED`; automatic recovery and retention ignore this state. A last workspace digest/version check and the short edit-locked commit boundary precede promotion of the same record to `PUBLISHED`, and the record receives `inventoryApplyCommittedAt` only at that final boundary. A legacy inventory-apply record without this marker is eligible only when both the prior recovery pointer and metadata identify the same transaction, so an interrupted pre-confirmation candidate cannot become the latest recovery merely from its timestamp. Failure, conflict, or timeout deletes the candidate where possible and leaves the visible workspace, user inputs, previous pointer, immutable Cloud revisions, and shipment results unchanged.

DataOps `ONEAPP_DATAOPS_SNAPSHOT_V1` remains owned and finalized by DataOps. OrderOps may read its latest finalized FULL snapshot through the existing action, preserving all source LOT rows. It creates a separate `TOTAL_ONLY` read model only when identical product code, product name, specification, and normalized unit make aggregation safe; conflicting groups remain excluded with traceable source-row issues. `TOTAL_ONLY` clears prior warehouse overrides and purchasing rows, exposes total stock/order/remaining comparison, sets no plan ID, and disables warehouse allocation, shipment confirmation, Cloud purchase-plan save, and purchase proposal. Switching back to a verified ERP warehouse snapshot rebuilds the warehouse path without retaining total-only values.
...
23. The three-area OrderOps workbench keeps Excel selection inside the left parser/drop surface and keeps ORDER Q, inventory, Cloud-completion, and local-recovery entry points in that left source area; the center bottom bar is reserved for current-work save, output, and shipment actions. The right voucher list is a no-horizontal-scroll four-column view: selection, compact order date, one-line customer/unit-safe quantity/source-or-read-only-calculated amount, and note. Date details and source evidence remain collapsible; warehouse and manager use quick filters; selection totals and the two bulk-change inputs stay directly below the list. Existing clear-value and selected-row edit/shipment-draft functions remain available only in collapsed secondary controls. A prepared multi-order source is compared by company, document ID, Revision, and hash. An exact repeat is a no-op. A changed set reconciles stable document-line identities against an immutable `orderops-source-baselines/v1` copy, preserves compatible warehouse, manager, purchase, substitution, history, and shipment-draft work for unchanged lines, and requires an explicit B/W/N decision for same-field conflicts. Reanalysis and purchase-only or sales-only reference application preserve the immutable source baseline and `orderops-prepared-shipment-drafts/v1` data through local recovery and re-entry, so a later partial Revision update still distinguishes source values from operator work. The candidate still crosses the existing STAGED, reread, hash verification, PUBLISHED, and recovery-pointer boundary; failure leaves both current work and the prepared source pending for retry. Multi-document shipment drafts remain recovery data and do not impersonate a single ORDER Q command owner.
...
ORDER Q is registered as a Pilot on the existing `orderops/list.html` compatibility route. It owns the isolated `shipping-purchase-plan` and `shipping-result` contracts, consumes the ORDER Q-owned `orderq-shipment-candidate-read-model`, does not call `coreEngine.js`, and does not change MerchOps or DataOps business behavior. Shipment work freezes the source `orderRevision` and `snapshotHash`, repeats that check immediately before result commit, and preserves operator input on conflict. A change detected after commit marks the result `REVIEW_REQUIRED`; reversal adds a compensating result instead of deleting history. Disabling the route and reverting its PR is the code rollback path; finalized local results and cloud revisions remain append-only operational records and are not deleted by rollback.
