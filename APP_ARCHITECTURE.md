...
- 2026-09-21 운영 백업: `/orderops/list.html`은 `13c83a23a330f3319ae67005f7ff0d67dfae4d54`의 기본 흐름으로 복구하고 계산·출력 모듈을 `orderops/stable/20260917/`에 고정한다. 후속 사용자 요청으로 주문현황·창고재고의 단위 열과 비 BOX 단위 행의 빨간 글씨를 보완했다. 최신 Excel 준비·재고변동 기능은 `orderops_list.html`에 유지한다. 두 경로의 코드를 자동 동반 수정하지 않는다. 기존 저장소는 그대로이며 데이터 격리·이관은 아니다. 상세 범위는 `docs/ORDEROPS_BACKUP_ROLLBACK_2026-09-21.md`를 따른다.
- Machine-readable companion: app-manifest.json

## 1. 문서 목적

NEXUS는 각 앱이 기본 업무를 독립적으로 수행하고 필요한 정보와 결과만 계약으로 주고받는 앱 집합이다. 이 문서는 앱의 역할, 데이터 소유권, 실행·연동 경계, 공통 UI, 성능·장애 격리, 영향 범위와 복구 계약을 정의한다.

### 1.1 문서별 책임

| 구분 | 관리하는 내용 |
|---|---|
| 사용자 확정 작업 지시·명세 | 해당 작업의 목적·범위·처리규칙·승인·완료조건 |
| AGENTS.md | 승인 범위 준수, 데이터·작업 보호, 실행·검증·완료 보고의 공통 규범 |
| APP_ARCHITECTURE.md | 앱 구조·데이터 계약·공통 UI·성능·실패 격리·영향·복구 기준 |
| app-manifest.json | 등록된 앱·경로·상태·owner·공유계약 정보 |
| 적용 역할 지침 | 지정된 담당자의 책임·권한·인계 경계 |
| 채팅방 지침 | 공수·난이도·위험과 실행 환경을 고려한 작업방·도구·경로 권고 |

역할 배치와 작업방 순서는 앱 아키텍처가 정하지 않는다. 단, 어떤 작업 경로를 선택하더라도 적용되는 구조·데이터·복구 계약은 지킨다.

계획은 구현 완료가 아니고, 문서 개정은 소유권 이전이나 운영 변경 승인이 아니다. 문서·manifest·소스·배포 증거가 불일치하면 차이를 기록하고 근거 없이 한쪽을 완료 사실로 간주하지 않는다.

### 1.2 작업에 따른 읽기 범위

기본적으로 이 문서의 1~2절과 대상 앱의 manifest 항목을 확인한다. 이후 변경에 해당하는 계약과 그 계약이 참조하는 의존 범위만 확인한다.

| 변경 대상 | 추가로 확인할 범위 |
|---|---|
| 앱 책임·경로·상태 | 3~4절, 8절 및 해당 앱 manifest |
| 화면·테마·공통헤더 | 5.1절과 3.5절 |
| 저장·데이터 처리·앱 간 연동 | 3.4절, 5~6절의 해당 계약 및 영향받는 소비자 |
| 변경 영향·실패 격리·복구 | 7절, 9절의 해당 기준 |
| 문서·계약 변경 | 10절 |
| 로드맵 검토 | 11절의 해당 항목과 실제 구현 상태 |

변경 없는 상세 계약은 원문 참조로 갈음하고 매번 전체를 재작성하지 않는다. 필요한 계약을 생략하거나 관계없는 모든 앱의 문서와 테스트를 자동으로 요구하지 않는다.

---

## 2. 앱 독립성 원칙

SmartInput의 `SI-BOUNDARY-20260920-01` 적용 범위는 입력자료·견적 완성과 Excel 출력까지의 독립 업무, 명시적 전달·결과 조회의 선택 연동이다. 호스트 준비·이동 전 저장·테마·인쇄는 업무 연동과 별도로 유지한다. 기존 별칭·거래처 이력 매칭은 명시적 전체 기준정보 갱신 때 owner의 읽기 전용 Snapshot을 받아 기존 SmartInput settings에 회사·사용자별로 보관하며, 일반 입력은 검증된 로컬 복사본만 사용한다. 최초 미준비와 갱신 실패를 표시하고 기존 작업행을 자동 재분석하지 않는다. 기존 매칭 점수·공식 원장·저장소 버전·소유권·서버 활성화 조건은 변경하지 않는다.

1. 앱의 기본 화면·로컬 조회·검색·계산·작업본 편집은 가능한 로컬 범위에서 독립 실행한다. 검증된 로컬 데이터가 있으면 다른 앱·공통 Runtime·서버 응답을 선행조건으로 삼지 않는다.
2. 각 데이터의 쓰기 소유자는 하나다. 다른 앱은 소유자가 제공하는 읽기 Snapshot·조회·명령·변경요청 계약을 사용한다. 신규 타 앱 원시 저장소 쓰기를 추가하지 않는다.
3. 기존 공유 DB와 명시적으로 승인된 writer·명령 예외는 전환 전 호환 계약으로 보존한다. 독립성을 이유로 저장소를 즉시 분리·통합·이전하거나 기존 정상 기능을 일괄 차단하지 않는다. 같은 Origin의 경로 분리는 저장소 접근 통제가 아니다.
4. Read Adapter는 읽기 전용 결과를 제공하고, Integration Adapter는 계약 변환·전달·실패 격리를 담당한다. 연동 장애는 해당 기능에 한정하며 앱 전체나 관계없는 업무로 확산하지 않는다.
5. 서버는 인증·권한·공유·동기화·충돌·최종 확정처럼 서버 확인이 필요한 경계에만 개입한다. 공통 라이브러리 재사용 자체는 금지하지 않지만 앱 준비 상태·저장 권한·업무 흐름의 중앙 통제 계층으로 확대하지 않는다.
6. 공통 UI는 앱 이동·홈 이동·테마·허용된 공통 상태를 제공한다. 업무 DB 접근·초기화·Revision 조회·앱 준비 판정·이동마다 인증 호출을 수행하지 않는다. 사용자 식별·로그아웃 등 현재 로그인 경계와 상세 UI 계약은 5.1절을 따른다.
7. 조회 오류·정상 빈 데이터·확정 여부 불명을 구분한다. 마지막 정상 자료와 원본을 보존하고, 공란·숫자 0·음수·원문을 승인 없이 보정하지 않는다.
8. 열린 전표와 활성 작업본은 시작 Snapshot을 보존한다. 새 서버 Revision은 분리해 알리고, 승인된 앱 정책 또는 명시적 적용 동작 없이 현재 값을 덮어쓰지 않는다. 충돌 시 양쪽 자료를 보존하고 적용 직전 최신 Head를 재확인해 새 이력으로 기록한다.
9. 관리자 업무 판단을 지원하고 원본과 결과의 비교·검산을 제공한다. 오차·손익·단순 경고는 자동 차단 이유가 아니며, 인증·권한·실제 데이터 무결성 위험은 해당 경계에서 검증한다.
10. 변경은 영향받는 앱·소비자와 계약을 확인하고 독립 복구 가능성을 유지한다. 공식 주소·저장키·payload·owner·운영 상태는 문서 정리만으로 바꾸지 않으며, 기능 통과로 사용성 회귀를 정당화하지 않는다.
...
- ORDER Q의 `orderq-vnext`는 주문과 Revision 및 불변 출고후보 Read Model을, `orderops`는 출고 작업과 append-only 출고결과 자산을 소유한다. 2026-09-16 기본 UI 롤백 후 OrderOps는 출고후보 Adapter와 공식 출고결과 명령을 소비하지 않으며 기존 owner 모듈·DB·backend·이력은 보존한다. 두 소유 경계는 서로의 Store를 직접 공유하지 않는다.
...
- `NEXUS-SMARTINPUT-SHOPPING-UPLOAD-20260904-01` 2단계는 SmartInput 주문서 탭의 기존 `+ Excel 파일`에서 정확한 17열 쇼핑몰 원본만 별도 인식하고 `smartinput/shopping-order-upload.js`를 통해 1단계 ORDER Q command Adapter를 소비한다. 원본 셀 값·셀 주소/형식 증거·행 순서·상태·메모·주소·전화·원코드는 수정하지 않으며 누적 파일은 최신 배송일자 행만 후보로 투영한다. 거래처·상품은 기존 owner Snapshot 검색과 작업자 명시 선택을 재사용하고, 출하창고까지 owner ID가 확정된 뒤에만 실제 ORDER Q 원장을 검사한다. UI는 후보별 `신규 저장 / 기존 주문서 제외 / 확인 필요`와 원본 행·문제 이유를 표시하지만 별도 signature·원장·raw Store 접근을 만들지 않는다. 저장 버튼은 command Adapter만 호출하며 owner Repository가 같은 DB v7 후보 transaction에서 실제 개수와 unique key를 재확인한다. 기존 ORDER Q 주문은 자동 수정·삭제·취소하지 않고, 한 후보의 문제나 실패는 다른 정상 후보 저장을 막지 않는다. 일반 Excel 매핑·수기 주문·구매·판매·견적 흐름, DB schema/Store/index/version, 공식전표 V2, 재고·채권채무, 공통 Runtime과 Cloud gate는 그대로다.
- SmartInput 견적 보고서는 매핑 기반으로 불러온 견적 원본에 한해 `ONEAPP_MERCHOPS_SETTINGS_READ_ADAPTER_V1`에서 현재 마진룰과 견적 매핑을 읽고, 상품 Snapshot의 기준단위와 원본 창고·입고가·외주비·노무비를 사용해 MerchOps의 가격 자동적용 결과를 재현한다. ERP 출고가와 견적서 업로드 단가, 명시 편집이 없는 쇼핑몰 시중가는 입고가 기준 재계산값이며 쇼핑몰 출고가는 양수 행사가가 있으면 이를 우선한다. 동일 품목코드가 여러 거래처 원본에 있으면 F8은 원본행을 제거하지 않고 품목별 후보를 한 화면에 모아 대표 행 선택 또는 입고가 직접 수정을 요구한다. 모든 그룹 확정 전 출력은 0-write이고, 확정 뒤 쇼핑몰·ERP는 품목코드당 대표 상품행 하나, 견적서 업로드는 거래처별 원본 거래행 전체를 출력한다. 소분 가격은 MerchOps 견적 매핑이 실제로 소유한 비용 필드만 사용한다. 수기 견적과 사용자가 명시적으로 편집한 출고가는 유지하고 Settings 키·상품 Master·견적 원본에는 쓰지 않는다.
...
- `nexus/workspace.html`은 NEXUS 홈에서 7개 글로벌 앱을 여는 기본 통합 호스트다. 같은 문서의 공통헤더를 유지하고 활성 iframe 하나만 사용하며 등록 앱의 정확한 same-origin 진입 경로, 부모 이력, `nexus-workspace-message/v1`의 Origin·source window·transition 검증과 공식 직접 앱 fallback만 소유한다. 연결 코드의 `BRIDGE_READY`, lifecycle adapter 등록, 실제 `APP_READY|APP_ERROR`를 구분하고 취소된 준비에는 `CANCEL`을 전달한다. 각 앱 lifecycle adapter가 진행 중 작업 결과 확인과 최신 입력 보존을 마친 뒤에만 전환을 승인하며 호스트 자체는 업무 저장소·확정 처리를 소유하지 않는다. 연속 선택은 아직 시작하지 않은 중간 앱을 생략하고 이미 시작된 준비의 완료를 기다리지 않으며, 취소된 transition의 늦은 결과를 반영하지 않는다. `export-center`, `settings`, `item-manager`, `history-viewer`, `orderq-vnext` 홈 카드는 기존 직접 경로를 유지하고 7개 앱의 공식 직접 URL도 독립 실행·즉시 롤백 경계로 보존한다.
- 통합 호스트는 같은 대상 Document의 native iframe load와 `APP_READY`가 모두 확인된 뒤에만 부모 이력을 확정한다. 이전 문서의 늦은 load는 다음 대상 연결을 소비하지 않고, 같은 URL의 Document 교체에도 새 transition을 부여해 이전 READY·ERROR·timeout을 배제한다. 이미 연결된 같은 Document의 중복 load는 재연결하지 않는다.
...
실행 방식과 데이터 접근 경계는 다른 개념이다. 아래 구분은 설계·검증의 의미 기준이며 이 문서 개정만으로 manifest 필드나 허용값을 바꾸지 않는다.

| 실행 방식 | 의미 | 장애 시 동작 |
|---|---|---|
| LOCAL_OPERATION | 로컬에서 해당 작업을 완료 | 허용된 로컬 업무 유지 |
| BACKGROUND_SYNC | 로컬 작업을 먼저 완료하고 서버 확인·전송·동기화를 후행 | 마지막 정상 상태와 대기 자료를 보존하고 계약에 따라 재시도 |
| SERVER_FINALIZE | 해당 작업의 완료에 서버 확인이 필요 | 해당 확정만 보류하고 작업본과 원인을 보존 |

로컬 완료·서버 최종 확정·전송 대기 상태를 구분하며, 동기화 예약을 서버 확정 완료로 표시하지 않는다. 실행 방식을 상향해 서버 대기를 추가하거나 확정 경계를 바꾸는 것은 사용자 업무정책 변경으로 검토한다.

OWNER_COMMAND와 OWNER_REQUEST는 소유자에게 명령·변경요청을 전달한다는 접근 경계다. 해당 표기만으로 로컬 실행인지 서버 확정인지 결정할 수 없다. STATIC_UI는 정적 표시 성격이고, ADMIN_DECISION_REQUIRED는 사용자 판단이 필요한 상태이므로 세 실행 방식과 동일한 축으로 해석하지 않는다.

현재 manifest의 dependencyMode에는 실행 방식과 접근 경계 표기가 혼재한다. 이번 개정에서는 기존 키·값·소비 동작을 보존하고, 개별 기능의 실제 계약으로 실행 위치와 대기 범위를 확인한다. 기계적으로 분리해야 할 필요가 확인되면 schema·validator·소비자를 함께 대조하는 별도 작업으로 진행한다.

### 3.5 사용성·장애 격리 기준

공통 기능과 서버 연동은 다음 기본 기준을 유지한다.
...
업무 Gateway 요청 0회는 모든 정적 파일 요청을 금지한다는 뜻이 아니다. 검증된 로컬 데이터가 있는 Warm 진입에서 실제 로컬 업무 데이터 표시 전 업무 Gateway 요청이 없다는 뜻이다.

Cold와 Warm을 구분하고 같은 PC·브라우저·자료·네트워크 조건으로 기준 버전과 비교한다. 더 엄격한 사용자 확정 명세가 있으면 그 기준을 적용한다.
...
아래 표는 목표 역할이 아니라 `app-manifest.json` v1.3.13의 현재 등록 상태다.
...
| 앱 전환 | 마지막으로 선택한 유효 앱을 최종 대상으로 사용한다. 이동 전 저장 확인 중에는 현재 본문을 유지하고 대상 탭에만 진행 상태를 표시하며, 실제 문서 교체 뒤에만 본문 로딩 상태를 사용한다. 현재 앱 재선택은 문서 교체 전이면 예약 이동을 취소하고, 교체가 시작됐으면 이전 앱을 다시 연다. 실패 화면은 재시도·이전 앱 복귀·공식 직접 주소를 제공한다. |
| 준비·저장 | 호스트는 연결 코드 실행과 실제 앱 준비를 구분하고 어댑터 미등록을 자동 완료로 처리하지 않는다. 앱은 현재 편집값을 반영한 뒤 진행 중 저장과 그 이후 변경까지 보존하되, 이미 영속 저장된 변경 없는 작업을 전체 재저장·재조회하지 않는다. 기존 자료 표시·복구·초기화 정책은 앱별 현행 계약을 유지한다. |
...
| 앱헤더 | 모든 업무 앱의 공통 기준은 본문 화면모드와 무관하게 글로벌헤더와 같은 `#0b1021` 다크 배경의 정확한 56px 단일 행이다. 앱 식별의 시작선은 데스크톱 24px·700px 이하 10px이며, 오른쪽 상태·주요 작업은 줄바꿈이나 겹침 없이 가로 접근을 보존한다. 글로벌 앱 이동·NEXUS 로고·화면 모드를 중복하지 않으며 SmartParser는 ONEAPP 로고/버전 배지 없이 `스마트파서` 명칭만 표시한다. OrderOps는 2026-09-16 승인 기본단계 롤백에 따라 기존 ORDER Q 헤더와 최소 NEXUS host 연결을 유지하는 예외다. |
...
- OrderOps는 2026-09-16 승인 기본단계 롤백에 따라 아래 공통 3영역 작업영역 모듈의 적용 대상에서 제외한다. 기존 Excel/parser와 주문현황·수불현황·창고재고 표를 유지하고 좌측 API/source 준비, 우측 전표 workbench, Revision 조정과 공식 출고명령 UI는 사용하지 않는다.
...
- 다섯 앱의 좌측은 현재 선택 행과 판단에 필요한 기준정보를 항상 표시하는 보조 영역이다. 사용자가 닫을 수 없고, 검색·필터·저장·복구 같은 핵심 작업을 좌측에만 두지 않는다.
...
- `smartinput/index.html` 스마트입력은 이 공통 작업영역 모듈의 적용 대상이 아니며 사용자 승인 복원 레이아웃과 기존 입력·저장·이력 기능을 유지한다.

**공통 표·인쇄 계약**

- 공통 표 모듈을 사용하는 업무 앱은 `nexus-table-ux.css/js`로 표의 머리글 고정, 행 선택·복사·키보드 이동, 실제 자료형 정렬, 열별 값 OR·열간 AND 필터, 표 검색, 숫자 정렬과 접근 가능한 열폭 조절의 공통 계약을 적용한다. 공통 조작은 현재 DOM 표시 계층에만 적용하고 각 행의 안정 ID·입력 요소·원본 배열을 바꾸지 않는다. 기본단계로 복원된 OrderOps는 앱 소유 표·검색·필터·정렬·열 설정을 유지하며 공통 표 모듈을 로드하지 않는다. React 소유 표의 DOM 내부 변형도 금지한다.
...
| Shipping local recovery | IndexedDB `ONEAPPShippingRecoveryDB` v1 / `recoveryRecords`; legacy compatibility read from `ONEAPPShippingManagementDB` / `workspaces`; `oneapp.shipping.recovery.pointer.v1` and `oneapp.shipping.recovery.meta.v1` | OrderOps only; IndexedDB stores the analysis workspace and inputs, while localStorage stores only the recovery pointer and metadata |
| Common inventory snapshot | `ONEAPP_INVENTORY_SNAPSHOT_V1`; `oneapp-common-inventory-read/v1`; Cloud `InventorySnapshotStaging`, `InventorySnapshotHistory`, `InventorySnapshotIndex` | Owner module, backend and immutable snapshots remain preserved, but the restored basic OrderOps UI does not save/read/apply this contract. Preserved module rules keep original LOT rows traceable, allow only safe exact-code/name/spec/unit aggregation for `TOTAL_ONLY`, and do not synthesize a warehouse, shipment allocation or purchase proposal. Later workbench recovery-state compatibility is not guaranteed by the basic rollback; no existing records are deleted. |
...
| Shipping system edit history | `shipping-workspace/v2.systemHistory`; `shipping-system-history/v1` | OrderOps only; append-only cell-edit events preserve field, before/after value, actor and time in local recovery and explicit Cloud plan revisions; this audit history is not projected into the unresolved-review-only system-message column |
...
| OrderOps Excel aliases | `oneapp.orderops.excel-mappings.v1` | OrderOps local parser preference only; administrator filename, sheet, and column aliases plus explicitly remembered warehouse source-column selections are excluded from workspace recovery and cloud plans. A warehouse selection is reused only for the exact order header signature and source column. |
...
| ORDER Q shipment candidate Read Model | `ONEAPP_ORDERQ_SHIPMENT_CANDIDATE_V1`; `orderq/shipment-order-read-adapter.js` | ORDER Q owns the order Stores and emits immutable, source-line-preserving candidates. The owner assets remain preserved, but the basic OrderOps UI does not consume this Adapter or write ORDER Q Stores. |
| OrderOps shipment result | IndexedDB `ONEAPPShippingResultDB` v1; `shipmentDocuments`, `shipmentLines`, `shipmentEvents`, `shipmentCommandReceipts`; `ONEAPP_SHIPPING_RESULT_V1` | OrderOps retains ownership of confirm, hold, reversal, idempotency receipts, and review-state assets; these commands are not connected to the basic UI. ORDER Q still reads through the result Adapter only. Existing append-only results remain preserved. |
...
| POST | `inventory_snapshot_save` | Verify, deduplicate, stage, append, and index one immutable OrderOps ERP warehouse-inventory snapshot |
| POST | `inventory_snapshot_list` | Return newest indexed ERP inventory metadata with an opaque bounded cursor |
| POST | `inventory_snapshot_get` | Reread and verify one indexed ERP inventory revision and its canonical payload |
...
The common ERP inventory contract is `ONEAPP_INVENTORY_SNAPSHOT_V1`. `inventory_snapshot_save` validates the canonical source matrix, normalized product rows, warehouse-key scope, exact warehouse totals, SHA-256, row count, and scalar-cell count under the existing Shipping token and script lock. It writes and rereads `InventorySnapshotStaging`, appends immutable chunks to `InventorySnapshotHistory`, rereads them, and only then publishes `InventorySnapshotIndex`. Equal hashes are deduplicated. `inventory_snapshot_list` returns at most 100 newest indexed metadata rows and an opaque `INV1` cursor; `inventory_snapshot_get` requires both snapshot ID and revision and never returns an unindexed staging or orphaned history payload.

The expanded inventory-application UI is not active in the restored basic `orderops/list.html`. Its `common-inventory-snapshot` owner asset and immutable server snapshots remain preserved; this UI rollback does not publish, delete, rewrite, or promote existing STAGED/PUBLISHED recovery records.

DataOps `ONEAPP_DATAOPS_SNAPSHOT_V1` remains owned and finalized by DataOps, and its DataOps/MerchOps consumers and rules are unchanged. The restored basic OrderOps UI does not read that API or create/apply a `TOTAL_ONLY` inventory view. The preserved common-inventory module's source LOT and safe-aggregation rules do not imply an active product consumer.
...
- SmartInput 입력표의 첫 열은 별도 행번호와 체크박스를 분리하지 않고 `No.` 또는 행번호를 선택 컨트롤 내부에 표시한다. 현재 입력 셀의 행은 굵은 포커스 테두리로, 다중 선택 행은 별도 선택 테두리로 구분하며 기존 Enter·방향키 셀 이동과 자동저장 의미는 변경하지 않는다.
- SmartInput의 일반 ERP 주문 입력에서 원본 `담당`·`담당자` 열은 주문행별 `assigneeName`으로 보존한다. 주문번호·창고·거래처·거래유형으로 묶은 각 주문 payload는 원본 담당자를 상단 공통 담당자보다 우선하며, 같은 주문 안에서 담당자가 다르면 임의 채택하지 않고 확인 필요로 차단한다. 원본 담당자명과 상단 담당자명이 다를 때 상단 담당자 ID를 재사용하지 않는다.
- SmartInput 견적·주문·구매·판매의 초기 입력 항목·표시명·Enter 순서는 사용자 지정 불변 preset으로 보존한다. 저장된 구성이 과거 공통 기본 입력열과 순서까지 정확히 같고 전표별 사용자 변경 증거가 없으면 1회 전표별 preset으로 자동 전환하며, 항목이나 순서가 하나라도 다른 기존 사용자 설정은 유지한다. `환경설정 → 전표 선택 → 초기 입력 모드로 변경 → 설정 저장`에서도 선택 전표의 입력열과 Enter 순서를 복원할 수 있다. 입력행·매핑 양식·상단/배송 설정·다른 전표·기존 사용자 필드 정의는 보존한다. 판매의 공지=금액1, 구매처=적요3, 구매=금액2이며 날짜는 사용자지정 문자형으로 기존 슬롯을 덮어쓰지 않고 배정한다. 문자형 슬롯이 부족하면 기존 설정을 유지하고 복원 실패 사유를 표시한다.
...
2. MerchOps consumes these values through `ONEAPP_MERCHOPS_SETTINGS_READ_ADAPTER_V1`; SmartInput also consumes the pricing rules and estimate mapping through the same read-only Adapter only while generating estimate reports. Neither app seeds, persists, resets, restores, or publishes `config_sync_trigger` for Settings-owned values.
...
1. ORDER Q's left Excel preparation area exposes a 2×2 order/purchase/sales/inventory selector, an in-panel file picker/drop target, a file list, and separate file/format and column-mapping accordions. Central results and existing calculation/output/recovery contracts remain in place.
2. A single file uses the selected kind. The same drop target retains the existing one-to-four distinct-kind structural classification; a compact integrated-workbook picker retains per-sheet alias intake. This does not enable multiple-order-source merging. Valid inputs automatically display that source's view (order, inventory, purchase or sales), without an additional Apply/Analyze/Enter gate. Having both orders and inventory updates calculations but never forces the visible view to orders. The operator retains the existing view/column/preset controls. Every automatic or manual path validates duplicate mappings; failed multi-file intake retains editable candidates alongside the active recovery sources and never replaces existing work until valid application succeeds.
...
5. Order and warehouse inventory remain required for allocation calculation, not for readonly inspection of individually prepared inputs. Unknown stock stays unknown. Inventory balance is the warehouse-stock sum minus the current editable order quantity. Purchase rows populate stock-ledger inbound and purchase-place displays, while sales rows populate the stock-ledger `출고` display. Purchase/sales-only product codes remain visible with unknown stock/balance; neither optional source changes the existing inventory-balance formula.
6. Mapping aliases, reusable Excel templates, and purchase-place input history remain app-local UI preferences. Template save does not apply the current candidate. Optional mapping evidence retains the original raw/display matrices and source coordinates independently of parser projections. Parsed optional-source rows travel with the analyzed workspace so local recovery and explicit cloud loading reproduce the same stock-ledger view. Candidate read/validation/calculation must succeed before replacing that source; other source edits and evidence remain. Input failure/cancel never clears current work or the recovery pointer.
...
8. ORDER Q renders operational values as Excel-style cell editors. Warehouse, customer, group, manager, order quantity, actual shipment quantity, unit price, per-warehouse inventory, delivery note, and purchase place are directly editable; product code, product name, specification, aggregate information, and calculated balance remain fixed identity or derived cells. Enter commits the current cell and moves vertically to the same field, while arrow keys move among editable cells. Successful value changes remain inside the existing `shipping-workspace/v2` optional row or inventory-override fields, append an optional `shipping-system-history/v1` record with field, before/after value, actor, and time, and are recalculated before local recovery, explicit cloud save, stock-ledger display, purchase selection, and integrated workbook output. The edit history is preserved but not projected into the system-message column. Merely focusing, selecting, or reconfirming an unchanged cell creates no history.
...
12. Warehouse inventory retains the aggregate wide layout with `수량 = signed sum of warehouse quantity columns`. ORDER Q also accepts the administrator's single-quantity `전일재고현황` and `재고변동표` layouts without fabricating warehouse breakdowns. The movement view uses opening inventory plus purchases minus sales; uploaded transaction files replace the corresponding embedded movement totals. A transaction-only SKU starts from zero in this view under the 2026-09-20 user decision, while existing order allocation's missing-inventory behavior is unchanged. Original source cells remain intact and duplicates remain validation errors.
13. The operator-facing product brand is `ORDER Q`, owned by ONEAPP, and its stated purpose is shipment management (`출고관리`). The approved ORDER Q image asset is the visible header identity and is displayed at the same apparent cap height as the ONEAPP wordmark. User-facing navigation is `스마트입력 → 주문조회 → 출고관리`, and the former `처리현황` label is clarified as `판매처리 검증`; existing route IDs, source filenames, storage keys, workspace schemas, cloud actions, and internal compatibility keys remain unchanged.
14. The integrated ORDER Q workbook retains delivery notices, order status, stock ledger, warehouse inventory, purchase upload, and sales upload, and adds `재고변동표`. The movement sheet is also available as a standalone workbook without order input. In order status, `주문수량` remains the individual order-row value and `주문수량 합계` repeats the signed total for the same normalized product code on every matching row. The sales-upload sheet follows the administrator-provided 19-column `판매입력` contract from `일자` through `구매처`, writes `출하창고` as the text value `01`, and omits the former `날짜`, `구매`, and `생산전표생성` columns. It is generated only from current nonzero order allocation rows; previously uploaded sales-history files are never re-exported as new sales vouchers.
...
18. Quantity presentation preserves the administrator's calculation trail: the calculated column is named `잔량`, blank remains blank, zero is muted, and a negative remainder is displayed as the signed numeric result such as `-3` with the same pale-yellow emphasis as its purchase-place cell. `발주 N`, `정보없음`, or other explanatory text is never substituted into a numeric quantity cell; review state remains available to the separate status-column filter, while purchase-upload export may still derive the required positive quantity from the signed result. Ordered rows use one pale context fill from product code through order quantity unless an explicit manager row assignment supplies the complete-row fill. The restored `/orderops/list.html` order-status and warehouse-inventory views show a `단위` column; a non-BOX or blank source unit gives the whole row normal-weight red text, while BOX retains the existing high-contrast text. Other views retain their exact `EA`/`소분` and `BOX` presentation. Automatic warehouse rainbow fills are disabled by default; only explicit saved color assignments are applied. Purchase-place Tab navigation centers the next verified-shortage row and selects its input. Order-information unit prices use thousands separators on screen and in the integrated workbook without changing numeric source values.
19. Named view presets persist local display conditions for order status, stock ledger, warehouse inventory, purchases, and sales: search and filters, sort, visible columns, column order, and explicitly saved column widths. A value filter that starts from `(모두 선택)` and unchecks exceptions persists an open-ended excluded-value list, so values first seen in later workbooks remain visible while only the explicitly unchecked values stay hidden; clearing `(모두 선택)` and checking individual values retains the closed selected-value list behavior. Selecting every value removes the value constraint. One preset per view may be marked as the default and is applied automatically when that result view opens. Applying a preset discards keys absent from the current workbook and never changes analyzed rows, editable business values, workspace recovery, cloud plans, or color assignments. Header-body clicks do not sort; sorting remains available only inside the header filter control.
20. The compact integrated Excel picker in the left preparation area wraps the existing four input kinds; it is not a new input data type or full-width panel. Result tabs preserve order, stock ledger, warehouse inventory, purchase, and sales and add the editable inventory movement view. Existing settings preserve per-kind sheet aliases: exact matches outrank contained aliases, and required headers/structure must also pass. Duplicate aliases across kinds cannot be saved. More than one valid same-kind sheet in one integrated file requires explicit sheet selection rather than last-sheet overwrite. Valid unambiguous sheets replace only their active kind; missing/ignored/invalid sheets retain that kind's previous work. Individual file replacement uses the selected source and does not merge same-kind files.
21. In warehouse inventory, selecting an order-information customer chip and Ctrl-clicking only the target product's explicitly marked `정보` cell is the local substitute-shipment command. Ctrl-clicking any other cell or the product row itself does nothing. The command preserves customer, quantity, unit price, the original source matrix, and a requested-product snapshot; changes only the selected order information's actual shipment product identity; recalculates the existing workspace; and appends a `shipping-substitution-history/v1` event only after the move succeeds. Ctrl+Z appends an undo event and restores the previous product state without deleting history. Substitute and undo history is preserved but not projected into the system-message column. This remains a local operation until the administrator explicitly saves an existing Shipping Cloud revision.
22. The rightmost warehouse-inventory system-message column is a derived unresolved-review projection with only two exact messages in stable order: `주문서 중복 여부 확인`, then `수량 입력 오류 확인`. It is empty when neither condition exists and never displays edit, substitute, save, or other success history. Suspected duplication requires the same strong original order identity and source-line identity; customer/product/date/quantity similarity, normal multi-line orders, and repeated-product lines are insufficient and no automatic merge, deletion, or correction occurs. Blank and non-finite order quantities remain editable raw values and are excluded from calculations; zero, negative, decimal, and parenthesized negative quantities remain valid. An unresolved quantity error blocks explicit Cloud-plan save and every Excel-workbook build, while analysis, editing, local recovery, and direct table printing remain available. Correcting the value immediately re-derives review state and calculations without changing `shipping-workspace/v2` or its existing history schemas.
23. The 2026-09-16 authorized basic rollback restores the four runtime files from `a596cbbdc1f8df52ee2b7f8f0dcf094cc372a8ab` (2026-09-07), retaining only the minimal NEXUS host adapter. Excel/parser intake, order status, stock ledger, warehouse inventory, local recovery, explicit Shipping Cloud save and outputs remain. The expanded three-area/source workbench, direct ORDER Q candidate reads, common-inventory reader and official shipment-result commands are not active UI consumers. Phase 6B unresolved review remains a read-only state in the existing result area. Owner modules, backend APIs, existing records and other apps are preserved; this is not a data migration or rollback of owner contracts.

24. Inventory movement edits and stocktake values remain optional fields in `shipping-workspace/v2`, with original workbook evidence preserved. Blank stocktake means use calculated balance; zero is an explicit count. The view shows stocktake minus book balance and the resulting final quantity. Negative balances from sales without stock or overselling remain valid. Non-BOX/non-박스 units use red text and negative numeric cells use a yellow background in the table and workbook. These local edits do not finalize official inventory checkpoints or write other apps' repositories.
...
이 표는 실제 변경에 해당하는 최소 영향 검토 범위다. 작업방·모델·공수 등급으로 결정하지 않으며, 필요한 역할을 생략해도 관련 계약의 최소 검증을 생략하지 않는다.

실제 공유계약이 바뀌면 모든 영향받는 소비자와 복구 경계를 확인한다. 상세 계약이 특정 전체 검사나 실패 주입 검증을 최소 요건으로 지정하면 해당 변경에서 유지한다. 그 외 관계없는 앱의 전체 재검증은 구체적인 위험 근거가 있을 때만 추가한다.

아래 독립성 질문은 기존 계약을 참조해 답할 수 있다. 단순 변경마다 동일한 설명을 새로 작성할 필요는 없지만, 관련 경계를 확인하지 않은 채 변경 없음이나 검증 통과로 표시하지 않는다.
...
ORDER Q remains a Pilot on the existing `orderops/list.html` compatibility route. The 2026-09-16 basic rollback retains Excel/parser shipment work, `shipping-purchase-plan` local/cloud recovery and Phase 6B unresolved-review reads, without `coreEngine.js` or changes to MerchOps/DataOps behavior. `orderq-shipment-candidate-read-model`, `common-inventory-snapshot` and official `shipping-result` commands are disconnected from this product UI; their owners, modules, backend and existing data remain preserved. ORDER Q's existing shipment-result read consumer is unchanged. Code rollback uses a reviewed PR; finalized local results, immutable cloud revisions and recovery records are not deleted or rewritten by the UI rollback.

---

## 9. 아키텍처 검증과 manifest 전환

실행·승인·검증·완료 보고의 공통 규범은 AGENTS.md를 따른다. 공수·난이도·작업방 배치는 채팅방 권고와 해당 작업의 지정 사항이며, 이 절은 앱 구조·데이터 경계와 완료 증거만 정의한다.

### 9.1 앱 개발명세 필수 경계

신규 앱과 구조·저장·연동 경계를 변경하는 작업은 아래 항목 중 적용되는 내용과 근거를 명세에 기록한다. 기존 계약을 그대로 쓰는 항목은 해당 절·계약·버전을 참조하고 변경 없음을 명시하면 된다. 경계를 바꾸지 않는 단순 작업에 전체 구조 명세의 재작성을 요구하지 않는다.
...
app-manifest.json의 현재 등록 계약과 호환성을 유지한다. 선택 필드는 실제 구현·소비자 전환·검증이 완료된 해당 앱에 한해 등록한다. 설명 문서의 버전이나 목표 역할을 근거로 현재 상태·소유권을 일괄 갱신하지 않는다.
...
| `dependencyMode` | 기능 의존 경계의 현재 표기. 실행 방식과 OWNER_COMMAND·OWNER_REQUEST 등 접근 경계를 구분해 읽는다(3.4절 참조). | 실제 경계 구현·검증 시. 형식 변경은 schema·validator·소비자 검토와 별도 승인 후 수행 |
...
- 문서별 책임은 1.1절을 따른다. 공통 규범·역할·난이도별 절차를 여러 문서에 반복해서 적지 않는다.
- 이 문서는 구조·데이터·UI·성능·실패 격리·영향·복구를 관리하고, 특정 모델·작업방·프로젝트 ID·PC 경로·필수 전달 순서를 규정하지 않는다.
- 앱의 추가·경로 변경·상태 변경·폐기는 승인된 실제 변경과 검증에 맞춰 manifest와 해당 상세 설명을 함께 갱신한다. 미구축 앱을 운영 의존성으로 등록하지 않는다.
- 공유계약 변경에는 검토한 모든 영향 소비자와 호환·이전·롤백 근거를 남긴다. 담당 역할이나 작업 경로를 이유로 데이터 보호와 최소 검증을 낮추지 않는다.
- 원본 상세 계약과 승인된 예외는 해당 절을 단일 참조점으로 유지한다. 문서 이동 시에는 원문·적용 조건·참조를 보존하고, 이동을 정책 폐기나 재승인으로 해석하지 않는다.
- 목표 구조·현재 구현·manifest 등록·검증·배포 상태는 구분한다. 서로 다르면 차이와 확인 근거를 기록하고, 검증하지 않은 상태를 운영 완료로 승격하지 않는다.
- 무관한 기능 개발에 이 문서의 전면 개정을 끼워 넣지 않는다. 실제 변경으로 영향을 받는 계약·참조·현재 사실만 갱신하며 정책 변경은 사용자 판단을 받는다.
- 역할 문서는 지정된 책임·권한·인계만 정의한다. 공통 지침 개정으로 충돌이 생기면 충돌 조항을 같은 문서 변경에서 정리한다.
- 문서 개정은 운영 코드·데이터·권한·서버·배포 설정의 변경을 자동 승인하지 않는다. 저장소 문서 적용만으로 채팅방·워크·로컬 실행환경 지침까지 자동 반영됐다고 보고하지 않는다.

---

## 11. 현재 계약과 앱 독립성 전환 로드맵

이 절에는 유지 중인 계약과 전환 방향이 함께 있다. 현행 기능·데이터·출력·복구 계약은 계속 적용하며, 로드맵은 그 자체로 신규 개발 명령이 아니다. 실제 작업 시 최신 소스와 승인 이력으로 이미 구현된 내용과 남은 내용을 구분해 중복 개발을 피한다.

승인된 전환 작업은 영향과 독립 복구 경계에 맞춰 PR·검증 범위를 정한다. 데이터·owner·호환성의 선행조건은 보존하되, 여기의 기술 전환 순서를 채팅·워크·검증PM·개발자의 필수 작업방 순서로 해석하지 않는다.
...
- The default Order Status sequence is warehouse, customer, group, manager, product code, product name, specification, product information, order, unit price, all warehouse-inventory columns, notice, and purchase. Product information aggregates only the same product code and explicit unit; mixed units are shown separately with unit labels, never converted from specification text. A single-unit single-row product remains blank, and product-name differences do not split the aggregate. Warehouse is read-only; order, unit price, notice, and purchase remain editable.
...
- Purchase input supplies stock-ledger inbound/purchase-place displays and sales input supplies the `출고` display. The ledger includes inventory, orders, purchases and sales by product code plus explicit unit. Missing inventory is unknown (blank stock/balance), not verified zero; missing transaction quantity stays blank, distinct from numeric zero. The inventory balance remains warehouse stock minus the current same-unit order quantity; reference purchases/sales are not applied a second time.
- Warehouse quantity edits recalculate allocations against effective edited quantities while keeping source rows/matrices immutable. Incompatible order/inventory units suspend automatic allocation and aggregate balance with a warning. Screen selection and purchase Excel use the same final signed warehouse-balance calculation, excluding missing inventory and incompatible units. The order workbook distinguishes per-order allocation shortage from final product-level replenishment (shown once per product), which can differ for negative stock or additional warehouses.
- Repeated shipment analysis recalculates a copy of the current edited or restored workspace, preserving order quantity/price, inventory overrides, purchase-place inputs and source evidence. Only the explicit current-file recovery replacement reanalyzes original inputs. Individual, classified and integrated Excel reads validate candidates before replacing active inputs; read/validation failure or picker cancellation retains the previous work and recovery. File reads, analysis and session reset cannot overlap.
