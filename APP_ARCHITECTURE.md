...
- SmartInput 견적·주문·구매·판매의 초기 입력 항목·표시명·Enter 순서는 사용자 지정 불변 preset으로 보존한다. 기존 전표별 저장 설정은 유지하며 `환경설정 → 전표 선택 → 초기 입력 모드로 변경 → 설정 저장`에서 선택 전표의 입력열과 Enter 순서를 복원한다. 입력행·매핑 양식·상단/배송 설정·다른 전표·기존 사용자 필드 정의는 보존한다. 판매의 공지=금액1, 구매처=적요3, 구매=금액2이며 날짜는 사용자지정 문자형으로 기존 슬롯을 덮어쓰지 않고 배정한다. 문자형 슬롯이 부족하면 기존 설정을 유지하고 복원 실패 사유를 표시한다.
