...
- `smartinput/index.html` 스마트입력은 이 공통 작업영역 모듈의 적용 대상이 아니며 2026-09-11 사용자 승인 복원 레이아웃, 7개 글로벌헤더와 기존 입력·저장·이력 기능을 유지한다. `orderops/list.html`은 앱헤더 아래 한 작업영역의 직접 자식으로 왼쪽 주문서 요약, 중앙 작업표, 오른쪽 보조 섹션을 배치한다. OrderOps는 1100px 이상에서 세 섹션을 같은 행에 두고, 640~1099px에서는 조절 가능한 왼쪽과 최소 320px 중앙을 같은 행에 두면서 오른쪽을 독립 폭 조절 overlay로 표시한다. 639px 이하에서는 왼쪽을 최대 38dvh 상단, 중앙을 하단에 쌓고 오른쪽을 overlay로 유지하며 이때만 좌우 조절 손잡이를 숨긴다. breakpoint 전환은 저장된 데스크톱 좌우 폭과 오른쪽 열림 상태를 덮어쓰지 않는다.
- OrderOps 왼쪽 주문 요약은 현재 창고·담당·지역 조회 범위의 주문서 수, 단위별 수량, 원본 금액·읽기용 계산 금액·금액 자료 없음 상태를 구분한다. 읽기용 계산 금액은 원본 `supplyAmount`를 변경하거나 저장하지 않는다. 지역 원본이 없으면 그룹명으로 자동 추정하지 않고 필요한 연결자료를 안내한다. 거래처 코드가 없는 담당 변경은 같은 주문·창고 범위에만 적용한다. 선택한 주문은 현재 중앙 보기를 유지한 채 연관 행만 표시하고, 기본 분석표 이동은 명시적 버튼으로만 수행한다.
- OrderOps 오른쪽 보조 섹션은 최초 빈 상태에서 닫혀 있고 사용자가 저장한 열림 상태와 폭만 복원한다. 승인되지 않은 처리결과·이력·후속 작업 표는 표시하지 않는다. 열 표시 메뉴는 `document.body` popup layer로 올려 viewport 안에 배치하며 마우스·터치·키보드, 바깥 클릭, Escape 초점 복귀를 지원한다. 중앙 공통 검색은 보기 전환 뒤에도 유지하고 현재 적용 조건과 `표시/전체` 건수를 밝히며, 검색 해제는 검색어만 지운다.
...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않으며, 코드가 없는 원본은 같은 주문·창고 배송 단위로 제한한다. 왼쪽은 `창고·담당·지역` 조회와 현재 창고·지역 범위의 담당자별 건수 선택, `거래처·수량·금액·적요` 네 열만 제공한다. 주문서가 있으면 재고 등록·출고분석 전에도 중앙 `거래처·상품·주문수량·직원 적요` 기본 분석표를 표시하고 실제 재고자료가 있을 때만 `재고·잔량`을 더한다. 이 기본 열은 `shipping:readiness:<role>` 안정 key를 사용한다. 구매·판매를 포함한 단독 원본도 각 읽기 전용 표로 표시하며 재고 단독 표는 검증된 동적 창고수량 열과 재고 합계를 원본 순서로 표시한다.
- OrderOps 담당자 변경은 왼쪽의 선택 거래처 담당 변경, 중앙 선택 열 또는 주문 상세에서 수행하고 안정 거래처 ID/코드가 있으면 현 작업공간의 동일 거래처 키 전체 주문행에 적용하며, 코드가 없으면 현재 주문·창고 단위에만 적용한다. 중앙 직원 적요는 `note1Original`/`note1`, 일반 적요는 `noteOriginal`/`note`로 분리한다. 분석 전 `ORDEROPS_PREVIEW`는 단일 원본 SHA-256 fingerprint, ORDER Q snapshot, 편집·이력을 검산 저장하며 복구 뒤 분석과 출고를 계속할 수 있다. 최신 검증 완료 분석본은 보관 정리에서 보호한다. 같은 상품코드의 EA·BOX처럼 주문단위가 섞이거나 주문·재고 어느 한쪽 단위가 공란이거나 명시 재고단위가 다르면 상품합계·배정·잔량·구매필요를 계산하지 않고 단위 확인으로 남긴다. 오른쪽은 위치·닫기·재열기·독립 폭·열림 상태만 제공하며 키보드 재열기에서도 중앙 전체 가로 위치를 바꾸지 않는다.
- OrderOps 앱헤더는 통합·주문서·창고재고 불러오기, 출고분석, 더보기와 환경설정을 소유한다. 검색·보기 전환·필터 초기화와 단일 `표 설정` 메뉴는 중앙 상단에, 스마트입력·Cloud 저장·화면인쇄·Excel 출력·이전복구와 저장 상태는 중앙 하단 작업바에 둔다. 가시 UI에 `System.IO` 콘솔 표현을 사용하지 않으며 버튼 이동은 기존 ID·handler·단축키·저장 경계를 바꾸지 않는다.
