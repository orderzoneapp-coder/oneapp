...
2. A single file uses the selected kind. The same drop target retains the existing one-to-four distinct-kind structural classification; a compact integrated-workbook picker retains per-sheet alias intake. This does not enable multiple-order-source merging. Valid inputs automatically display the available raw rows, or the calculated order list when order/inventory are available, without an additional Apply/Analyze/Enter gate. Ambiguous mappings remain pending on the left and never replace existing work until valid application succeeds.
