...
- OrderOps 배송건은 상품행 수가 아니라 안정 거래처 ID/코드와 주문·창고 배송 단위로 계산한다. 같은 상호의 다른 거래처 코드를 합치지 않으며, 코드가 없는 원본은 같은 주문·창고 배송 단위로 제한한다. 왼쪽은 주문번호·창고·담당자·지역·거래처·대표 품목·단위별 수량·공급가액·`적요(직원)`의 주문서형 목록이며 전체 업무량과 담당자/지역 필터 결과를 구분한다. `적요(직원)`은 Excel `적요1|적요(직원)|직원적요` 또는 ORDER Q line `description`만 사용하고 일반 `적요`/배송지시를 대체값으로 사용하지 않는다.
- OrderOps 담당자 변경은 안정 거래처 ID/코드가 있으면 현 작업공간의 동일 거래처 키 전체 주문행에 적용하고, 코드가 없으면 현재 주문·창고 단위에만 적용하며 기존 system history와 로컬 복구 경계에 기록한다. 혼합·일부 미지정 상태를 숨기지 않고 동일 값 재입력은 이력을 추가하지 않는다. 오른쪽 예상 잔량은 현재 적재 창고재고 합계에서 주문수량을 뺀 표시 계산이며 실제 출고 확정이나 DataOps 재고 쓰기를 수행하지 않는다. 선택 상품의 재고 원본행이 없으면 선택을 유지하고 `재고자료 없음`을 표시하며 재고·예상잔량을 0으로 대체하지 않는다.
