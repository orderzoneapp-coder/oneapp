...
if (!window.React || !window.ReactDOM) { throw new Error('React/ReactDOM 라이브러리 로드 실패'); }
        ...
        const readLegacyItemMasterSnapshot = async () => {
            if (!window.indexedDB || typeof window.indexedDB.databases !== 'function') {
                return { status: 'unsupported', exists: false, masterMap: {}, revision: null };
            }
            const databases = await window.indexedDB.databases();
            const exists = databases.some(entry => entry && entry.name === LEGACY_ITEMMASTER_DB.NAME);
            if (!exists) return { status: 'absent', exists: false, masterMap: {}, revision: null };
            ...
            try {
                if (!db.objectStoreNames.contains(LEGACY_ITEMMASTER_DB.PRODUCTS_STORE)) {
                    return { status: 'error', exists: true, masterMap: {}, revision: null, error: '레거시 상품 Store를 찾지 못했습니다.' };
                }
                ...
                const result = await new Promise((resolve, reject) => {
                    ...
                    const revisionRequest = storeNames.includes(LEGACY_ITEMMASTER_DB.STATE_STORE)
                        ? tx.objectStore(LEGACY_ITEMMASTER_DB.STATE_STORE).get(LEGACY_ITEMMASTER_DB.REVISION_KEY)
                        : null;
                    const snapshotRequest = storeNames.includes(LEGACY_ITEMMASTER_DB.STATE_STORE)
                        ? tx.objectStore(LEGACY_ITEMMASTER_DB.STATE_STORE).get('itemMasterSnapshot_v1')
                        : null;
                    tx.oncomplete = () => resolve({
                ...
                const rawItems = result.items.length > 0
                    ? result.items
                    : Object.values(result.snapshot && typeof result.snapshot === 'object' ? result.snapshot : {});
                ...
                return { status: 'error', exists: true, masterMap: {}, revision: null, error: error.message };
            } finally {
                db.close();
            }
        };
        ...
        const buildInitialMasterImport = (parsed = {}, fileName = '') => {
            ...
            rows.forEach((row, index) => {
                const item = {};
                headers.forEach(header => { item[header] = row[header]; });
        ...
        // ==========================================
        const AppHeader = ({ itemCount, isProcessing, processMsg, onOpenSettings }) => (
            <div className="nexus-app-header shrink-0 z-40 w-full bg-white border-b border-slate-200 shadow-sm" data-nexus-app-header="master-lookup">
                <header className="min-h-[56px] w-full px-3 sm:px-6 py-2 flex flex-wrap items-center justify-between gap-2">
                    <div className="min-w-0 flex items-center gap-3" data-nexus-app-identity>
                        <span className="w-9 h-9 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100" aria-hidden="true">
                            <SafeIcon name="database" size={17} />
                        </span>
                        <div className="min-w-0">
                            <div className="flex items-center gap-2">
                                <h1 className="text-[15px] font-black text-slate-800 whitespace-nowrap" data-nexus-app-title>상품관리</h1>
                                <span className="h-7 px-3 rounded-md bg-slate-100 text-slate-700 text-[11px] font-bold hidden sm:flex items-center" aria-current="page">조회</span>
                            </div>
                            <p className="hidden sm:block text-[10px] text-slate-500 mt-0.5 truncate">상품 조회·관리자 검토</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-2">
                        <div className="h-9 px-3 rounded-lg bg-slate-50 border border-slate-200 flex items-center gap-2" title={isProcessing ? processMsg : '상품관리 준비'}>
                            {isProcessing
                                ? <i className="fas fa-circle-notch fa-spin text-amber-500 text-[11px]"></i>
                                : <span className="inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>}
                            <span className="text-[11px] font-mono font-bold text-slate-600">{itemCount.toLocaleString()}건</span>
                        </div>
                        <button type="button" onClick={onOpenSettings} className="w-9 h-9 rounded-lg bg-white hover:bg-slate-50 flex items-center justify-center border border-slate-300 text-slate-600" title="환경설정 열기" aria-label="환경설정 열기"><SafeIcon name="settings" size={16} /></button>
                    </div>
                </header>
            </div>
        );
                ...
                <div className="fixed inset-0 z-[500] flex bg-slate-900/60 backdrop-blur-sm fade-in">
                    <div className="w-full max-w-[1400px] h-full bg-white shadow-[-10px_0_40px_rgba(0,0,0,0.3)] flex flex-col slide-in-right border-l border-slate-300 relative overflow-hidden ml-auto">
                        ...
                        <div className="flex-1 w-full bg-slate-100 relative">
                            ...
                            <iframe src="settings.html?mode=iframe&returnApp=master-lookup" className="absolute inset-0 w-full h-full border-0 bg-white relative z-10" title="Settings Center"></iframe>
                        </div>
                    </div>
                </div>
            );
        ...
        const InitialImportConfirmModal = ({ draft, onSave, onCancel, isSaving }) => {
            if (!draft) return null;
            return (
                <div className="fixed inset-0 z-[130] bg-slate-950/60 flex items-center justify-center p-5">
                    <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-6 py-5 bg-indigo-50 border-b border-indigo-200">
                            <h2 className="text-lg font-black text-indigo-900">상품관리 최초 Excel 등록</h2>
                            <p className="text-xs text-indigo-700 mt-1">공식 상품 DB가 0건일 때만 실행되며 revision과 공식 history를 함께 기록합니다.</p>
                        </div>
                        <div className="p-6 space-y-4">
                            <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                                <div className="text-[11px] font-bold text-slate-500">파일</div>
                                <div className="mt-1 text-sm font-bold text-slate-800 break-all">{draft.fileName}</div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                                    <div className="text-[11px] font-bold text-slate-500">검증 완료 상품</div>
                                    <div className="mt-1 text-2xl font-black text-indigo-700">{draft.count.toLocaleString()}건</div>
                                </div>
                                <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                                    <div className="text-[11px] font-bold text-slate-500">저장 기준 Revision</div>
                                    <div className="mt-1 text-sm font-black text-slate-800 break-all">{safeStr(draft.baseRevision, '초기 상태')}</div>
                                </div>
                            </div>
                            <p className="text-xs leading-5 text-slate-600">상품코드 중복과 품목명·규격·단위 누락 검사를 통과했습니다. 저장 직전 DB가 여전히 비어 있고 revision이 같은지 다시 확인합니다.</p>
                        </div>
                        <div className="px-6 pb-6 flex justify-end gap-2">
                            <button onClick={onCancel} disabled={isSaving} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 text-xs font-bold disabled:opacity-50">취소</button>
                            <button onClick={onSave} disabled={isSaving} className="px-5 py-2 rounded-lg bg-indigo-600 text-white text-xs font-black disabled:bg-slate-400">
        ...
        const PRODUCT_FORM_FIELDS = [
            { key: '코드', label: '상품코드', required: true },
            { key: '품목명', label: '품목명', required: true },
            { key: '규격', label: '규격', required: true },
            { key: '단위', label: '단위', required: true },
            { key: '1코드', label: '대분류 코드' },
            { key: '1그룹명', label: '대분류명' },
            { key: '2코드', label: '중분류 코드' },
            { key: '2그룹명', label: '중분류명' },
            { key: '3코드', label: '소분류 코드' },
            { key: '3그룹명', label: '소분류명' },
            { key: '오더즈', label: '오더즈분류3' },
            { key: '구매처', label: '구매처' },
            { key: '브랜드', label: '브랜드' },
            { key: '상품설명', label: '상품설명' },
            { key: '검색명', label: '검색명' },
            { key: '구분', label: '구분' },
            { key: '입고가', label: '입고가', type: 'number' },
            { key: '출고가', label: '출고가', type: 'number' },
            { key: '도매A', label: '도매A', type: 'number' },
            { key: '최종입고', label: '최종입고', type: 'number' },
            { key: '행사가', label: '행사가', type: 'number' },
            { key: '준비기간', label: '준비기간' },
            { key: '주문마감시간', label: '주문마감시간' },
            { key: '외주비단가', label: '외주비단가', type: 'number' },
            { key: '표준노무시간', label: '표준노무시간', type: 'number' },
            { key: '경비가중치', label: '경비가중치', type: 'number' },
            { key: '1종', label: '1종 관련 정보' },
            { key: '2종', label: '2종 관련 정보' },
            { key: '비과세', label: '비과세' }
        ];

        const DETAIL_PRODUCT_FIELDS = [
            { category: '품목정보', key: '바코드', label: '바코드' },
            { category: '품목정보', key: '원산지', label: '원산지' },
            { category: '수량', key: '최소주문수량', label: '최소주문수량', type: 'number' },
            { category: '수량', key: '최대주문수량', label: '최대주문수량', type: 'number' },
            { category: '단가', key: '입고B', label: '입고B', type: 'number' },
            { category: '단가', key: '도매B', label: '도매B', type: 'number' },
            { category: '원가', key: '표준원가', label: '표준원가', type: 'number' },
            { category: '부가정보', key: '보관방법', label: '보관방법' },
            { category: '관리대상', key: '관리대상', label: '관리대상' },
            { category: '관리대상', key: '사용여부', label: '사용여부' },
        ];
        ...
        const isDerivedGroupCodeField = (field, productCode) => {
            const requiredLength = { '1코드': 2, '2코드': 4, '3코드': 6 }[field];
        ...
        const ProductEditorModal = ({ item, masterProducts, onCancel, onSave, isSaving }) => {
            ...
            useEffect(() => {
                if (!isOpen) return;
                const source = item ? { ...item, 코드: safeStr(item.코드 || item.품목코드) } : {};
            ...
            const handleSubmit = async () => {
                try {
                    ...
                    PRODUCT_FORM_FIELDS.concat(DETAIL_PRODUCT_FIELDS).filter(field => field.type === 'number').forEach(({ key: field }) => {
                        if (safeStr(normalized[field]).trim() !== '') normalized[field] = Number(normalized[field]);
                    });
                    await onSave(normalized, { isEditing });
                    try { localStorage.setItem(PRODUCT_EDITOR_FIELD_LAYOUT_KEY, JSON.stringify(fieldKeys)); } catch {}
                } catch (error) {
                    setErrorMessage(error.message);
                }
            };
            const visibleFields = fieldKeys.map(key => PRODUCT_FORM_FIELDS.find(field => field.key === key) || DETAIL_PRODUCT_FIELDS.find(field => field.key === key) || { key, label: key });
            return (
                <div className="fixed inset-0 z-[135] bg-slate-950/60 flex items-center justify-center p-5">
                    <div className="w-full max-w-3xl max-h-[92vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                        <div className="px-6 py-5 bg-slate-900 text-white">
                            <h2 className="text-lg font-black">{isEditing ? '상품 수정' : '상품 단건 등록'}</h2>
                            <p className="text-xs text-slate-300 mt-1">공식 master·revision·history 계약으로 저장됩니다.</p>
                        </div>
                        <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4">
                            {visibleFields.map((field, index) => (
                                <div key={field.key} className="relative">
                                    <label className="block">
                                        <span className="min-h-6 flex items-center pr-24 text-[11px] font-bold text-slate-600">{field.label}{field.required ? ' *' : ''}</span>
                                        <input
                                            ...
                                            onChange={event => setForm(current => {
                                                const next = field.key === '코드'
                                                    ? { ...current, 코드: event.target.value, 품목코드: event.target.value }
                                                    : { ...current, [field.key]: event.target.value };
                                                return !isEditing && field.key === '코드'
                                                    ? window.ONEAPP_MASTER_ADD_UPDATE.deriveProductCategoryFields(next, masterProducts)
                                                    : next;
                                            })}
                                    ...
                                    <span className="absolute top-0 right-0 flex items-center gap-1">
                                        <button type="button" onClick={() => moveField(field.key, -1)} disabled={index === 0} className="w-6 h-6 rounded border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 disabled:opacity-30" aria-label={`${field.label} 앞으로 이동`}>↑</button>
                                        <button type="button" onClick={() => moveField(field.key, 1)} disabled={index === visibleFields.length - 1} className="w-6 h-6 rounded border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 disabled:opacity-30" aria-label={`${field.label} 뒤로 이동`}>↓</button>
                                        {!PRODUCT_FORM_FIELDS.some(baseField => baseField.key === field.key) && <button type="button" onClick={() => setFieldKeys(current => current.filter(key => key !== field.key))} className="w-6 h-6 rounded border border-rose-200 bg-white text-rose-600 hover:bg-rose-50" aria-label={`${field.label} 기본항목에서 제외`}>×</button>}
                                    </span>
                                </div>
                            ))}
                            <div className="md:col-span-2 border-t border-slate-200 pt-4">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                    <select value={detailSelection} onChange={event => setDetailSelection(event.target.value)} className="h-10 flex-1 px-3 rounded-lg border border-slate-300 text-sm bg-white">
                                        <option value="">상세항목 선택</option>
                                        {DETAIL_PRODUCT_FIELDS.filter(field => !fieldKeys.includes(field.key)).map(field => <option key={field.key} value={field.key}>{field.category} · {field.label}</option>)}
                                        {Object.keys(form).filter(key => !PRODUCT_FORM_FIELDS.some(field => field.key === key) && !DETAIL_PRODUCT_FIELDS.some(field => field.key === key) && !fieldKeys.includes(key)).map(key => <option key={key} value={key}>등록되지 않은 항목 · {key}</option>)}
                                    </select>
                                    <button type="button" onClick={() => { if (detailSelection) setFieldKeys(current => [...current, detailSelection]); setDetailSelection(''); }} disabled={!detailSelection} className="h-10 px-4 rounded-lg border border-indigo-300 text-indigo-700 text-xs font-black disabled:opacity-40">+ 기본항목 추가</button>
                                </div>
                                <p className="text-[10px] text-slate-500 mt-2">추가한 항목과 표시 순서는 상품 저장 후 다음 수정창에도 유지됩니다. 화면에서 제외한 기존 필드값은 삭제하지 않습니다.</p>
                            </div>
                        </div>
                        {errorMessage && <div className="mx-6 mb-4 px-4 py-3 rounded-lg bg-rose-50 border border-rose-200 text-xs font-bold text-rose-700">{errorMessage}</div>}
                        <div className="px-6 pb-6 flex justify-end gap-2">
                            <button onClick={onCancel} disabled={isSaving} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 text-xs font-bold disabled:opacity-50">취소</button>
                            <button onClick={handleSubmit} disabled={isSaving} className="px-5 py-2 rounded-lg bg-indigo-600 text-white text-xs font-black disabled:bg-slate-400">
        ...
        const requestProposedItem = (entry, masterProducts = {}) => {
            ...
            if (entry?.review?.applyTarget?.targetProduct) {
                const prepared = { ...entry.review.applyTarget.targetProduct };
                return request.operation === 'CREATE'
                    ? window.ONEAPP_MASTER_ADD_UPDATE.deriveProductCategoryFields(prepared, masterProducts)
                    : prepared;
            }
            const base = request.operation === 'CREATE' ? {} : (masterProducts[request.entityId] || {});
            const item = { ...base, 코드: safeStr(base.코드 || request.entityId), 품목코드: safeStr(base.품목코드 || request.entityId) };
            (request.changes || []).forEach(change => { item[change.field] = change.proposedValue; });
            return request.operation === 'CREATE'
                ? window.ONEAPP_MASTER_ADD_UPDATE.deriveProductCategoryFields(item, masterProducts)
                : item;
        };

        const ProductRequestReviewModal = ({ entry, masterProducts, onClose, onApply, onLink, onReject, isProcessing }) => {
            ...
            useEffect(() => { setForm(proposed); setLinkCode(''); setReason(''); }, [proposed]);
            ...
            const request = entry.request || {};
            const original = request.source?.original || request.changes?.map(change => ({ field: change.field, value: change.beforeValue })) || {};
            return (
                <div className="fixed inset-x-0 bottom-0 z-[145] bg-slate-950/65 flex items-center justify-center p-3 sm:p-5" style={{ top: 'var(--nexus-ui-header-height, 64px)' }}>
                    <div className="w-full max-w-6xl max-h-full bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                        <div className="px-5 sm:px-6 py-4 bg-slate-900 text-white flex items-start justify-between gap-4">
                            <div><h2 className="text-lg font-black">{requestTypeLabel(entry)}</h2><p className="text-xs text-slate-300 mt-1">{requestSourceLabel(entry)} · {request.actor?.actorName || '요청자 미확인'} · {new Date(request.requestedAt || entry.receivedAt).toLocaleString('ko-KR')}</p></div>
                            <button type="button" onClick={onClose} disabled={isProcessing} className="w-9 h-9 rounded-lg border border-slate-600 text-slate-200">×</button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-5 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-5">
                            <section className="space-y-3">
                                <h3 className="text-xs font-black text-slate-800">작업자 원본 · 읽기 전용</h3>
                                <pre className="min-h-32 max-h-56 overflow-auto whitespace-pre-wrap break-all rounded-xl border border-slate-200 bg-slate-50 p-4 text-[11px] text-slate-700">{JSON.stringify(original, null, 2)}</pre>
                                <h3 className="text-xs font-black text-slate-800">동일·유사 상품 후보</h3>
                                <div className="rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
                                    {similarProducts.length ? similarProducts.map(item => {
                                        const code = safeStr(item.코드 || item.품목코드);
                                        return <button type="button" key={code} onClick={() => setLinkCode(code)} className={`w-full p-3 text-left text-xs hover:bg-indigo-50 ${linkCode === code ? 'bg-indigo-50 text-indigo-800' : ''}`}><strong>{safeStr(item.품목명, '-')}</strong> · {safeStr(item.규격, '-')} · {code}</button>;
                                    }) : <div className="p-4 text-xs text-slate-500">동일·유사 상품 후보가 없습니다.</div>}
                                </div>
                                <input value={linkCode} onChange={event => setLinkCode(event.target.value)} placeholder="연결할 기존 상품코드" className="w-full h-10 px-3 rounded-lg border border-slate-300 text-sm" />
                            </section>
                            <section>
                                <h3 className="text-xs font-black text-slate-800 mb-3">공식 상품 반영 예정값</h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {PRODUCT_FORM_FIELDS.map(field => <label key={field.key} className="block"><span className="text-[10px] font-bold text-slate-600">{field.label}{field.required ? ' *' : ''}</span><input type={field.type || 'text'} value={form[field.key] ?? ''} disabled={(request.operation !== 'CREATE' && field.key === '코드') || (request.operation === 'CREATE' && isDerivedGroupCodeField(field.key, form.코드 || form.품목코드))} onChange={event => setForm(current => {
                                        const next = field.key === '코드'
                                            ? { ...current, 코드: event.target.value, 품목코드: event.target.value }
                                            : { ...current, [field.key]: event.target.value };
                                        return request.operation === 'CREATE' && field.key === '코드'
                                            ? window.ONEAPP_MASTER_ADD_UPDATE.deriveProductCategoryFields(next, masterProducts)
                                            : next;
                                    })} className="mt-1 w-full h-9 px-3 rounded-lg border border-slate-300 text-xs disabled:bg-slate-100" /></label>)}
                                </div>
                            </section>
                        </div>
                        <div className="px-5 sm:px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col lg:flex-row gap-2 lg:items-center">
                            <input value={reason} onChange={event => setReason(event.target.value)} placeholder="반려 사유 또는 처리 메모" className="h-10 flex-1 px-3 rounded-lg border border-slate-300 text-xs" />
                            <button type="button" onClick={() => onReject(reason)} disabled={isProcessing || !reason.trim()} className="h-10 px-4 rounded-lg border border-rose-300 text-rose-700 text-xs font-black disabled:opacity-40">반려</button>
                            <button type="button" onClick={() => onLink(linkCode, reason)} disabled={isProcessing || !linkCode.trim()} className="h-10 px-4 rounded-lg border border-indigo-300 text-indigo-700 text-xs font-black disabled:opacity-40">기존 상품 연결</button>
                            <button type="button" onClick={() => onApply(form, reason)} disabled={isProcessing} className="h-10 px-5 rounded-lg bg-indigo-600 text-white text-xs font-black disabled:bg-slate-400">{request.operation === 'CREATE' ? '확인 및 등록' : '확인 및 반영'}</button>
                        </div>
                    </div>
                </div>
            );
                ...
                <div className="fixed inset-0 z-[120] bg-slate-950/55 flex items-center justify-center p-5">
                    <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                        <div className="px-6 py-5 bg-amber-50 border-b border-amber-200">
                            <h2 className="text-lg font-black text-amber-900">2. 정보수정 Excel 비교 결과 요약</h2>
                            <p className="text-xs text-amber-800 mt-1">아직 master에는 아무 값도 저장하지 않았습니다. 이슈 화면에서 예외 상품을 확인하세요.</p>
                        </div>
                        ...
                        <div className="px-6 pb-6 flex justify-end gap-2">
                            ...
                            <button onClick={onReview} className="px-5 py-2 rounded-lg bg-indigo-600 text-white text-xs font-black hover:bg-indigo-700">3. 이슈 확인으로 이동</button>
                        </div>
                    </div>
                </div>
            );
        ...
        const MasterAddUpdatePreviewModal = ({ preview, onBack, onSave, isSaving }) => {
            if (!preview) return null;
            const counts = preview.counts;
            const rows = [
                ['일괄 승인 준비', counts.approvedCount],
                ['신규 적용 예정', counts.createCount],
                ['기존 상품 수정 예정', counts.updateCount],
                ['적용 예정 변경 필드', counts.appliedFieldCount],
                ['상품 전체 제외', counts.excludedCount],
                ['필드 반영 제외', counts.fieldExcludedCount],
                ['미승인', counts.unapprovedCount],
                ['저장 차단', counts.blockedCount],
                ['반영 필드 없음', counts.noAppliedFieldProductCount],
                ['누락 상품 · 기존 유지', counts.missingRetainedCount]
            ];
            return (
                <div className="fixed inset-0 z-[124] bg-slate-950/55 flex items-center justify-center p-5">
                    <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                        <div className="px-6 py-5 bg-indigo-50 border-b border-indigo-200">
                            <h2 className="text-lg font-black text-indigo-900">6. 적용 예정 결과 확인</h2>
                            <p className="text-xs text-indigo-800 mt-1">현재 승인 결정을 기존 master에 적용한다고 가정해 계산한 읽기 전용 결과입니다. 아직 master에는 저장하지 않았으며 history도 기록하지 않았습니다.</p>
                        </div>
                        <div className="p-6">
                            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                                {rows.map(([label, count]) => (
                                    <div key={label} className={`rounded-xl border p-3 ${label === '저장 차단' && count > 0 ? 'bg-rose-50 border-rose-200' : 'bg-slate-50 border-slate-200'}`}>
                                        <div className="text-[11px] font-bold text-slate-500">{label}</div>
                                        <div className="text-lg font-black text-slate-900 mt-1">{count.toLocaleString()}건</div>
                                    </div>
                                ))}
                            </div>
                            <div className="mt-4 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-xs font-bold text-amber-900">
                                이 화면의 수치는 저장 전 미리보기입니다. 아래 저장 버튼을 누르기 전에는 상품 master·revision·history가 변경되지 않습니다.
                            </div>
                        </div>
                        <div className="px-6 pb-6 flex justify-end gap-2">
                            <button type="button" onClick={onBack} disabled={isSaving} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-700 text-xs font-bold disabled:opacity-40">이슈 확인으로 돌아가기</button>
                            <button type="button" onClick={onSave} disabled={isSaving || counts.savedProductCount === 0} className="px-5 py-2 rounded-lg bg-emerald-600 text-white text-xs font-black disabled:bg-slate-400">
                ...
                <div className="fixed inset-0 z-[125] bg-slate-950/55 flex items-center justify-center p-5">
                    <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-6 py-5 bg-emerald-50 border-b border-emerald-200">
                            <h2 className="text-lg font-black text-emerald-900">정보수정 Excel 저장·검증 완료</h2>
        ...
        const MasterAddUpdateReview = ({
            ...
            const visible = analysis && api
                ? api.filterCandidates(analysis, selectedTags, { includeSame })
                : [];
            const page = api
                ? api.paginateCandidates(visible, pageIndex, REVIEW_PAGE_SIZE)
                : { items: [], totalCount: 0, pageIndex: 0, pageCount: 1, startIndex: 0, endIndex: 0, hasPrevious: false, hasNext: false };
            useEffect(() => {
            ...
            return (
                <div className="fixed inset-0 z-[110] bg-slate-100 flex flex-col">
                    <div className="h-16 px-6 bg-slate-900 text-white flex items-center justify-between shrink-0 shadow-lg">
                        <div>
                            <h2 className="text-lg font-black">3. 이슈 확인 · 4. 예외 상품 반영 제외</h2>
                            <p className="text-[11px] text-slate-300">{analysis.fileName} · 예외만 제외한 뒤 전체 일괄 적용으로 저장 전 결과를 확인합니다.</p>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={onBulkApply} disabled={isSaving || analysis.masterMismatch} className="px-5 py-2 rounded-lg bg-indigo-500 disabled:bg-slate-600 text-white text-xs font-black">
                                5. 전체 일괄 적용
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="px-5 py-3 bg-white border-b border-slate-200 shrink-0">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <label className="ml-auto flex items-center gap-2 text-[11px] font-bold text-slate-600">
                                <input type="checkbox" checked={includeSame} onChange={e => {
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div className="text-xs font-bold text-slate-500">
                                표시 {page.totalCount === 0 ? '0' : `${(page.startIndex + 1).toLocaleString()}-${page.endIndex.toLocaleString()}`} / 필터 결과 {page.totalCount.toLocaleString()}건 / 전체 비교 {analysis.summary.compareCount.toLocaleString()}건 / 누락 유지 {analysis.summary.missingCount.toLocaleString()}건
                            </div>
                            <div className="flex items-center gap-2 text-[11px] font-bold text-slate-600">
                                ...
                                >이전</button>
                                <span>{page.pageIndex + 1} / {page.pageCount} 페이지</span>
                                <button
                                    type="button"
                                                        ...
                                                        return (
                                                            <tr key={field.field} className={`border-t border-slate-100 ${field.excluded ? 'bg-slate-100 opacity-70' : ''}`}>
                                                                <td className="px-3 py-3 align-top">
                                                                    ...
                                                                    {(field.sourceHeader !== field.field || field.sourceCellAddress) && (
                                                                        <div className="mt-0.5 text-[9px] text-slate-500">원본 {field.sourceHeader}{field.sourceCellAddress ? ` · ${field.sourceCellAddress}` : ''}</div>
                                                                    )}
                                                                ...
                                                                <td className="px-3 py-3 align-top break-all">{formatReviewValue(field.uploadDisplay, field.uploadPresent)}</td>
                                                                <td className="px-3 py-3 align-top">
                                                                    {canDirectEdit && (
                                                                        <input
                                                                            type="text"
        ...
        function App() {
            ...
            const [legacyState, setLegacyState] = useState({ status: 'checking', exists: false, masterMap: {}, rawItems: [], revision: null });
            const [changeRequestInbox, setChangeRequestInbox] = useState({ status: 'LOADING', requests: [], error: null });
            const [selectedChangeRequest, setSelectedChangeRequest] = useState(null);
            const [selectedProductCode, setSelectedProductCode] = useState('');
            const [productResultOpen, setProductResultOpen] = useState(() => {
                try { return sessionStorage.getItem('nexus:master:right-panel:v1') !== 'closed'; } catch { return true; }
            });
                    ...
                    unregister = bridge.registerAdapter({
                        beforeLeave: async () => {
                            ...
                            if (workspaceLifecycleStateRef.current?.editingOpen || workspaceLifecycleStateRef.current?.reviewOpen || workspaceLifecycleStateRef.current?.settingsOpen) {
                                return { result: 'BLOCKED', message: '열려 있는 상품 편집·검토 화면을 저장하거나 닫은 뒤 이동해 주세요.' };
                            }
                            return { result: 'READY' };
                        },
                        print: () => window.print()
            ...
            const refreshChangeRequestInbox = useCallback(async () => {
                try {
                    ...
                    const result = await module.productMasterChangeRequestAdapter.listChangeRequests({ status: ['PENDING', 'IN_REVIEW'], limit: 200, collapseDuplicates: true });
                    setChangeRequestInbox(result);
                } catch (error) {
                    setChangeRequestInbox({ status: 'ERROR', requests: [], error: { code: error.message || String(error) } });
                }
            }, []);
            ...
            useEffect(() => {
                    ...
                    if (data && typeof data === 'object') {
                        ...
                        try {
                            ...
                            if (saved) {
                                ...
                                setTimeout(() => { if (productTableScrollRef.current) productTableScrollRef.current.scrollTop = Number(saved.scrollTop || 0); }, 0);
                ...
                readLegacyItemMasterSnapshot().then(setLegacyState).catch(error => {
                    setLegacyState({ status: 'error', exists: true, masterMap: {}, revision: null, error: error.message });
                });
            ...
            const openSkuManagement = () => {
                try {
                    sessionStorage.setItem('oneapp.master.sku-return-state.v1', JSON.stringify({
                        globalSearch, activeC1, activeC2, activeC3,
            ...
            const openChangeRequest = async (entry) => {
                ...
                try {
                    const module = await window.__loadProductChangeRequestAdapter();
                    const result = await module.productMasterChangeRequestAdapter.beginReview({ requestId: entry.request?.requestId, actor: null });
                    if (result.entry) setSelectedChangeRequest({ ...result.entry, duplicateRequestIds: entry.duplicateRequestIds || [], repeatedRequestCount: entry.repeatedRequestCount || 1 });
            ...
            const completeSelectedRequest = async ({ resolution, productCode = '', reason = '', result = null }) => {
                const module = await window.__loadProductChangeRequestAdapter();
                const completed = await module.productMasterChangeRequestAdapter.completeChangeRequest({
                    ...
                    resolution, productCode, reason, actor: null, result,
                });
            ...
            const handleApplyChangeRequest = async (input, reason) => {
                ...
                try {
                    const normalized = window.ONEAPP_MASTER_ADD_UPDATE.validateSingleProductInput(request.operation === 'CREATE'
                        ? window.ONEAPP_MASTER_ADD_UPDATE.deriveProductCategoryFields(input, masterProducts)
                        : input);
                    PRODUCT_FORM_FIELDS.concat(DETAIL_PRODUCT_FIELDS).filter(field => field.type === 'number').forEach(({ key }) => {
                    ...
                    if (preparedAlreadyApplied) {
                        ...
                        const module = await window.__loadProductChangeRequestAdapter();
                        const prepared = await module.productMasterChangeRequestAdapter.prepareApply({ requestId: request.requestId, productCode: code, targetProduct: normalized, actor: null });
                    ...
                    await completeSelectedRequest({ resolution: 'APPLIED', productCode: code, reason, result: { revision: commitResult?.revision ?? masterPageRevision } });
                    ...
                    for (let index = 0; index < targets.length; index += 1) {
                        ...
                        try {
                            ...
                            const completed = await module.productMasterChangeRequestAdapter.completeChangeRequest({
                                ...
                                reason: '신규 상품 전체 등록',
                                actor: null,
                                result: { revision: commitResult?.revision ?? masterPageRevision },
                            });
                            ...
                            console.error(error);
                            failures.push({ name: displayName, reason: error.message || String(error) });
                            if (error?.code === 'MERCH_MASTER_REVISION_CONFLICT') {
            ...
            const handleLinkChangeRequest = async (productCode, reason) => {
                ...
                setIsProcessing(true);
                try {
                    await completeSelectedRequest({ resolution: 'LINKED', productCode, reason });
                    showToast(`✅ 기존 상품 [${productCode}]에 연결했습니다.`);
                } catch (error) { showToast(`❌ 기존 상품 연결 실패: ${error.message}`); }
            ...
            const handleRejectChangeRequest = async (reason) => {
                setIsProcessing(true);
                try {
                    await completeSelectedRequest({ resolution: 'REJECTED', reason });
                    showToast('✅ 요청을 반려하고 사유를 기록했습니다.');
                } catch (error) { showToast(`❌ 반려 처리 실패: ${error.message}`); }
                finally { setIsProcessing(false); }
            };
            ...
            const legacySummary = useMemo(() => {
                ...
                Object.entries(legacyMap).forEach(([code, item]) => {
                    ...
                    else if (stableSerialize(current) === stableSerialize(item)) sameCount += 1;
                    else conflictCount += 1;
                });
                return { total: Object.keys(legacyMap).length, newCount, sameCount, conflictCount };
            ...
            const handleSaveProduct = async (input, { isEditing } = {}) => {
                setIsProcessing(true);
            ...
            const handlePrepareBulkApply = async () => {
                ...
                try {
                    ...
                    setAddUpdatePreview({ counts: preview.counts, analysis: prepared.analysis });
            ...
            const setProductResultVisibility = useCallback((open) => {
                ...
                try { sessionStorage.setItem('nexus:master:right-panel:v1', open ? 'open' : 'closed'); } catch {}
            }, []);

            return (
                <div className="w-full flex flex-col font-sans overflow-hidden bg-slate-100 relative select-none" style={{ height: 'calc(100vh - var(--nexus-ui-header-height, 64px))' }}>
                    <AppHeader
                    ...
                    <main className="w-full max-w-[1440px] mx-auto flex-1 flex flex-col overflow-hidden bg-slate-50 border-x border-slate-200">
                        <div className="min-h-[50px] px-3 sm:px-6 py-2 bg-white border-b border-slate-200 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 shrink-0 z-10 shadow-sm">
                            <div className="w-full lg:w-auto flex flex-wrap items-center gap-2 sm:gap-4">
                                <strong className="text-[12px] text-slate-700">상품 조회</strong>
                                {Object.keys(masterProducts).length === 0 && (
                                    <span className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
                                        상품 DB가 비어 있습니다. Excel 최초 등록 또는 상품 단건 등록으로 시작하세요
                                    </span>
                                )}
                            </div>
                            <div className="w-full lg:w-auto grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 min-w-0">
                                <button onClick={() => setEditingProduct({})} disabled={isProcessing} className="w-full sm:w-auto h-9 px-3 sm:px-4 rounded-lg text-[12px] font-bold bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-slate-300 shadow-sm flex items-center justify-center">
                                    <SafeIcon name="plus" size={13} className="mr-2" /> 상품 등록
                                </button>
                                <label className={`w-full sm:w-auto h-9 px-3 sm:px-4 rounded-lg text-[12px] font-bold border transition-colors flex items-center justify-center shadow-sm ${isProcessing ? 'cursor-not-allowed bg-slate-100 text-slate-400 border-slate-200' : 'cursor-pointer bg-white text-slate-700 hover:bg-slate-100 border-slate-300'}`}>
                                    <SafeIcon name="upload" size={14} className="mr-2 text-slate-500" /> {Object.keys(masterProducts).length === 0 ? '최초 등록 Excel' : '1. 정보수정 Excel'}
                                    <input type="file" className="hidden" accept=".xlsx,.xls" onChange={handleExcelUpload} disabled={isProcessing} />
                                </label>
                                <button type="button" onClick={openSkuManagement} disabled={isProcessing} className="w-full sm:w-auto h-9 px-3 sm:px-4 rounded-lg text-[12px] font-bold border border-indigo-300 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:opacity-40">SKU 관리</button>
                                {addUpdateAnalysis && !showAddUpdateConfirm && !showAddUpdateReview && (
                                    <button onClick={() => setShowAddUpdateReview(true)} className="h-9 px-4 rounded-lg text-[12px] font-black bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm">
                                        검토 이어보기
                                    </button>
                                )}
                                <div className="relative col-span-2 w-full sm:w-64 h-9 sm:ml-2">
                        ...
                        {legacyState.status === 'ready' && legacySummary.total > 0 && (
                            <div className="px-3 sm:px-6 py-3 bg-amber-50 border-b border-amber-200 flex flex-col lg:flex-row lg:items-center gap-3 shrink-0" data-legacy-itemmaster-notice="ready">
                                <div className="flex-1 min-w-0">
                                    <div className="text-xs font-black text-amber-900">이 브라우저에서 폐기된 ItemMaster 격리 DB 데이터 {legacySummary.total.toLocaleString()}건을 찾았습니다.</div>
                                    <div className="text-[11px] text-amber-800 mt-1">신규 {legacySummary.newCount.toLocaleString()}건 · 동일 {legacySummary.sameCount.toLocaleString()}건 · 충돌 {legacySummary.conflictCount.toLocaleString()}건. 자동 반영·덮어쓰기·삭제하지 않습니다.</div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={downloadLegacyBackup} className="h-8 px-3 rounded-lg border border-amber-300 bg-white text-amber-900 text-[11px] font-bold hover:bg-amber-100">JSON 백업</button>
                                    <button onClick={startLegacyReview} disabled={isProcessing} className="h-8 px-3 rounded-lg bg-amber-700 text-white text-[11px] font-black hover:bg-amber-800 disabled:bg-slate-400">정보수정 Excel 검토</button>
                                </div>
                            </div>
                        )}
                        {legacyState.status === 'error' && legacyState.exists && (
                            <div className="px-3 sm:px-6 py-2 bg-rose-50 border-b border-rose-200 text-[11px] font-bold text-rose-800 shrink-0" data-legacy-itemmaster-notice="error">
                                레거시 ItemMaster 격리 DB가 존재하지만 읽지 못했습니다. 원본은 변경하지 않았습니다: {legacyState.error}
                            </div>
                        )}

                        <section className="px-3 sm:px-6 py-3 bg-white border-b border-slate-200 shrink-0" data-product-change-request-inbox={changeRequestInbox.status}>
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                <div className="flex-1 min-w-0">
                                    <div className="text-[12px] font-black text-slate-800">신규 상품 등록 요청 {(changeRequestInbox.requests || []).filter(entry => entry.request?.operation === 'CREATE').length.toLocaleString()}건 · 상품정보 수정 요청 {(changeRequestInbox.requests || []).filter(entry => entry.request?.operation !== 'CREATE').length.toLocaleString()}건</div>
                                    <div className={`text-[10px] mt-0.5 ${changeRequestInbox.status === 'ERROR' || changeRequestInbox.status === 'NOT_AVAILABLE' ? 'text-rose-700' : 'text-slate-500'}`}>
                                        {changeRequestInbox.status === 'READY'
                                            ? '관리자가 원본과 반영 예정값을 확인한 뒤 처리합니다.'
                                            : changeRequestInbox.status === 'EMPTY'
                                                ? '접수된 요청이 없습니다.'
                                                : changeRequestInbox.status === 'LOADING'
                                                    ? '요청 확인 중'
                                                    : `요청함 확인 실패: ${changeRequestInbox.error?.code || changeRequestInbox.status}`}
                                    </div>
                                </div>
                                {(changeRequestInbox.requests || []).some(entry => entry.request?.operation === 'CREATE') && (
                                    <button type="button" onClick={handleRegisterAllCreateRequests} disabled={isProcessing} className="h-9 px-5 rounded-lg bg-indigo-600 text-white text-[11px] font-black shadow-md hover:bg-indigo-700 disabled:bg-slate-400 whitespace-nowrap">전체 등록</button>
                                )}
                            </div>
                            {(changeRequestInbox.requests || []).length > 0 && (
                                <div className="mt-3 max-h-52 overflow-auto rounded-lg border border-slate-200" aria-label="상품 등록·수정 요청 목록">
                                    <table className="w-full min-w-[860px] text-[11px] text-left">
                                        <thead className="sticky top-0 z-10 bg-slate-100 text-slate-600 border-b border-slate-200">
                                            <tr>
                                                <th className="px-3 py-2 font-black w-36">요청 구분</th>
                                                <th className="px-3 py-2 font-black">품목명</th>
                                                <th className="px-3 py-2 font-black w-32">규격</th>
                                                <th className="px-3 py-2 font-black w-24">단위</th>
                                                <th className="px-3 py-2 font-black w-44">요청자·출처</th>
                                                <th className="px-3 py-2 font-black w-24">상태</th>
                                                <th className="px-3 py-2 font-black text-center w-20">관리</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100 bg-white">
                                            {changeRequestInbox.requests.map((entry) => (
                                                <tr key={`${entry.request?.requestId}-${entry.receivedAt}`} className="hover:bg-amber-50/60">
                                                    <td className="px-3 py-2 font-black text-amber-900">{requestTypeLabel(entry)}</td>
                                                    <td className="px-3 py-2 font-bold text-slate-800">{entry.request?.changes?.find(change => change.field === '품목명')?.proposedValue || entry.request?.entityId || '-'}{entry.repeatedRequestCount > 1 && <span className="ml-2 inline-flex px-2 py-0.5 rounded-full bg-slate-100 text-[10px] text-slate-600">동일 요청 {entry.repeatedRequestCount}회</span>}</td>
                                                    <td className="px-3 py-2 text-slate-600">{entry.request?.changes?.find(change => change.field === '규격')?.proposedValue || '-'}</td>
                                                    <td className="px-3 py-2 text-slate-600">{entry.request?.changes?.find(change => change.field === '단위')?.proposedValue || '-'}</td>
                                                    <td className="px-3 py-2 text-slate-600"><div className="truncate">{entry.request?.actor?.actorName || '요청자 미확인'} · {requestSourceLabel(entry)}</div></td>
                                                    <td className="px-3 py-2"><span className={`inline-flex px-2 py-1 rounded-full font-black ${entry.status === 'IN_REVIEW' ? 'bg-indigo-50 text-indigo-700' : 'bg-amber-50 text-amber-800'}`}>{entry.status === 'IN_REVIEW' ? '검토 중' : '검토 대기'}</span></td>
                                                    <td className="px-3 py-2 text-center"><button type="button" onClick={() => openChangeRequest(entry)} className="h-7 px-3 rounded-md bg-white border border-indigo-300 text-indigo-700 font-black hover:bg-indigo-50">수정</button></td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        <div className="flex-1 flex overflow-hidden relative" data-nexus-workspace="master-lookup">
                            <aside className="min-w-0 flex shrink-0 flex-col overflow-y-auto border-r border-slate-200 bg-white p-3" data-nexus-pane="reference" aria-label="선택 상품 기준정보">
                                <span className="text-[9px] font-black tracking-[0.16em] text-indigo-600">REFERENCE</span>
                                <h2 className="mt-1 text-[13px] font-black text-slate-800">선택 상품 참고</h2>
                                <div className="mt-3 space-y-2" data-nexus-selection-reference>
                                    <p className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-500">중앙 목록에서 상품을 선택하면 핵심 기준정보가 고정 표시됩니다.</p>
                                </div>
                            </aside>

                            <section className="flex-1 min-w-0 bg-white flex flex-col overflow-hidden relative z-0" data-nexus-pane="work" aria-label="상품 목록 작업표">
                                <div className="grid max-h-[190px] shrink-0 grid-cols-[180px_minmax(220px,1fr)] overflow-hidden border-b border-slate-200 bg-slate-50">
                                    <div className="overflow-y-auto border-r border-slate-200 bg-white py-2 custom-scrollbar">
                                        {c2List.map(c2 => (
                                            <button key={c2.code} onClick={() => { setActiveC2(c2.code); setActiveC3(''); setGlobalSearch(''); }} className={`w-full text-left px-5 py-2.5 text-[12px] font-bold transition-all border-l-4 ${activeC2 === c2.code ? 'bg-indigo-50/50 text-indigo-700 border-indigo-600' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}>{c2.name}</button>
                                        ))}
                                    </div>
                                    <div className="flex min-w-0 flex-col overflow-hidden">
                                        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-2 shrink-0"><h3 className="text-[12px] font-black text-slate-800">소분류 (3차)</h3><button type="button" onClick={handlePull} className="h-8 rounded-lg border border-indigo-200 bg-indigo-50 px-3 text-[11px] font-black text-indigo-700 hover:bg-indigo-100"><SafeIcon name="cloud-download" size={12} className="mr-1" /> 데이터 불러오기</button></div>
                                        <div className="grid min-h-0 flex-1 grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-1 overflow-y-auto p-2 custom-scrollbar">
                                            {c3List.map(c3 => (
                                                <button key={c3.code} onClick={() => { setActiveC3(c3.code); setGlobalSearch(''); }} className={`min-w-0 text-left px-3 py-2 rounded-md text-[12px] font-bold transition-all flex justify-between items-center ${activeC3 === c3.code ? 'bg-white text-indigo-700 border border-indigo-200 shadow-sm' : 'border border-transparent text-slate-600 hover:bg-white hover:border-slate-200'}`}><span className="truncate pr-2">{c3.name}</span><span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeC3 === c3.code ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>{c3.count}</span></button>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div ref={productTableScrollRef} className="flex-1 overflow-auto custom-scrollbar relative">
                                    <table className="w-max min-w-full text-left text-[11px] whitespace-nowrap border-separate border-spacing-0" aria-label="상품관리 Excel형 목록">
                                        <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200 shadow-sm">
                                            <tr>
                                                <th className="sticky left-0 z-20 py-2 px-2 w-12 min-w-[48px] text-center text-slate-500 bg-slate-100 border-r border-b border-slate-200">행</th>
                                                ...
                                                <th className="py-2.5 px-3 text-slate-600 w-20">관리</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100">
                                            {displayRows.length > 0 ? displayRows.map((row, idx) => {
                                                ...
                                                return (
                                                    <tr key={code} onClick={() => setSelectedProductCode(code)} className={`cursor-pointer transition-colors ${selectedProductCode === code ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                                                        <td className="sticky left-0 z-[1] py-2 px-2 text-center text-slate-500 bg-slate-50 border-r border-b border-slate-200">{idx + 1}</td>
                                                        ...
                                                        <td className="py-2 px-3">
                                                            <button onClick={() => setEditingProduct(row)} className="h-7 px-3 rounded-md border border-slate-300 text-[11px] font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-300">
                                                                <SafeIcon name="edit" size={11} className="mr-1" /> 수정
                                                            </button>
                                                        </td>
                                                    </tr>
                                                );
                                                ...
                                                <tr>
                                                    <td colSpan={visMasterCols.length + 5} className="py-20 text-center text-slate-400">{Object.keys(masterProducts).length === 0 ? '상품 DB가 비어 있습니다. 상단의 Excel 최초 등록 또는 상품 등록을 사용하세요.' : '해당 카테고리에 데이터가 없거나 검색 결과가 없습니다.'}</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                                <div className="shrink-0 flex items-center justify-center gap-2 px-4 py-3 border-t-2 border-slate-300 bg-white shadow-[0_-8px_18px_rgba(15,23,42,0.08)]" data-nexus-completion-bar="master-lookup">
                                    <button type="button" onClick={handlePush} disabled={isProcessing || Object.keys(masterProducts).length === 0} className="h-9 px-4 rounded-lg bg-indigo-600 text-white text-[11px] font-black hover:bg-indigo-700 disabled:opacity-40"><SafeIcon name="cloud-upload" size={12} className="mr-1" /> 데이터 백업</button>
                                </div>
                            </section>
                            {productResultOpen ? (
                                <aside className="flex min-w-0 shrink-0 bg-white border-l border-slate-200 flex-col overflow-hidden" data-nexus-pane="result" aria-label="선택 상품 결과">
                                    <div className="h-11 px-3 flex items-center justify-between border-b border-slate-200 bg-slate-50 shrink-0">
                                        <div><span className="block text-[9px] font-black tracking-widest text-indigo-600">RESULT</span><strong className="block text-[12px] text-slate-800">처리 결과·다음 단계</strong></div>
                                        <button type="button" onClick={() => setProductResultVisibility(false)} className="w-7 h-7 rounded-md border border-slate-300 bg-white text-slate-500" aria-label="선택 상품 결과 닫기">×</button>
                                    </div>
                                    <div className="flex-1 overflow-y-auto p-3 space-y-2 text-[11px]">
                                        {selectedProduct ? <>
                                            <div className="p-3 rounded-lg border border-slate-200 bg-slate-50"><span className="block text-[9px] font-bold text-slate-400 mb-1">상품</span><strong className="block text-[13px] text-slate-800 break-words">{safeStr(selectedProduct['품목명'], '-')}</strong><span className="font-mono text-slate-500">{safeStr(selectedProduct['코드'] || selectedProduct['품목코드'], '-')}</span></div>
                                            <div className="p-3 rounded-lg border border-slate-200 bg-slate-50"><span className="block text-[9px] font-bold text-slate-400 mb-1">규격·단위</span><strong>{safeStr(selectedProduct['규격'], '-')} · {safeStr(selectedProduct['단위'], '-')}</strong></div>
                                            <div className="p-3 rounded-lg border border-slate-200 bg-slate-50"><span className="block text-[9px] font-bold text-slate-400 mb-1">단가</span><strong>입고 {Number(selectedProduct['입고가'] || 0).toLocaleString()} · 출고 {Number(selectedProduct['출고가'] || 0).toLocaleString()}</strong></div>
                                            <button type="button" onClick={() => window.ONEAPP_NEXUS_NAVIGATE_ROUTE(`history_viewer.html?code=${encodeURIComponent(safeStr(selectedProduct['코드'] || selectedProduct['품목코드']))}&returnApp=master-lookup`, 'master-lookup')} className="w-full h-9 rounded-lg border border-slate-300 bg-white text-slate-700 font-bold">변경이력 보기</button>
                                        </> : <div className="p-4 text-center text-slate-400 leading-relaxed">중앙 목록에서 상품을 선택하면 상세 결과가 표시됩니다.</div>}
                                    </div>
                                    <div className="p-3 border-t border-slate-200"><button type="button" onClick={openSkuManagement} className="w-full h-9 rounded-lg border border-indigo-300 bg-indigo-50 text-indigo-700 font-black">SKU 관리</button></div>
                                </aside>
                            ) : (
                                <button type="button" onClick={() => setProductResultVisibility(true)} className="absolute right-3 top-3 z-20 h-9 px-3 rounded-lg border border-slate-300 bg-white text-[11px] font-black text-slate-700 shadow-lg" data-nexus-result-reopen="master-lookup">선택 상품 결과 열기</button>
                            )}
                        </div>
                    </main>
