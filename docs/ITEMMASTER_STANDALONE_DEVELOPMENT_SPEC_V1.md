# ItemMaster 독립 상품관리 앱 개발명세서 v1.0

- 작업 ID: `ITEMMASTER-STANDALONE-20260829`
- 작성일: 2026-08-29
- 기준 소스: `origin/main` `7aebfb23128a0a4b5a8e9cd0cec59e60319c5536`
- 상태: 확정·개발 진행
...
3. 저장 DB 이름만 ItemMaster 전용 격리 DB로 변경한다.
4. 복사본 안에서 직접 조회·저장하며 운영 Master 저장 함수를 호출하지 않는다.
5. 구형 15개 메뉴, 설정 iframe, 클라우드 Push·Pull 코드를 제거한다.
6. Pipeline·BOM·카탈로그 기능을 포함하지 않는다.
7. 독립 실행, 저장과 새로고침 후 유지 여부만 검증한다.
...
현재 `Master.html`에 없는 단건 입력 UI는 이번 복사 단계에 새로 만들지 않는다. 이번 명세의 등록·수정은 기존 Excel 추가·갱신 기능을 의미한다.
...
- 상단에는 `ITEMMASTER · 격리 검증`과 전용 DB 상태만 표시한다.
