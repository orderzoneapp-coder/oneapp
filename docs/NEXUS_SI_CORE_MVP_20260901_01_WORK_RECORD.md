...
## 2026-09-02 · 승인되지 않은 SmartInput UI 변경 복구

- 입력표 상단 버튼으로 여는 중앙 팝업 방식을 제거하고 기존 우측 패널 UI를 복구했다.
- 우측 패널은 사용자가 열고 닫을 수 있으며, 열린 상태에서는 입력표와 나란히 배치되어 연속 작업할 수 있다.
- 견적서 모드에서는 기존처럼 `견적서 목록 / 연동견적서 / 다중 선택` UI만 우측 패널에 표시하고 전표 활동 영역은 숨긴다.
- 우측 패널 폭 조절, 닫기 버튼, 견적 카드 순서 변경과 하단 작업 버튼을 기존 동작 그대로 유지한다.
- 승인된 후속 UI 조정으로 중앙 입력표와 Excel 매핑표의 `No.`·선택 열을 하나로 합치고 행 번호 옆 체크박스를 21px로 확대했다.
- 공식 전표, 회사별 원장, 스냅샷과 IndexedDB 스키마는 변경하지 않았다.
