...
## 4단계 순차 통합
사용자: “4단계 빠른개발로 배포해”, 앞선 추가 검증 중단 지시 유지. main bcf5dcab(3단계 배포 완료)을 PR #600의 기존 06ed1f0e에 병합한 뒤 조회 cache·summary·registry·실제 세 표의 viewport를 연결했다. 기존 선택/파일 순서와 데이터 보존 경로를 유지한다. syntax 확인 외 반복/확대 회귀검사와 30회 성능 측정은 수행하지 않으며, CI 결과를 앱 통합 검증이나 독립 PM 승인으로 표시하지 않는다. 실제 배포 완료는 GitHub Pages 해당 merge SHA로 확인한다.
