...
function orderQM9SituationBegin(ss, payload) {
  ...
  const closePurpose=orderQM9Text(payload.readPurpose)==='DATAOPS_CLOSE';let closeContext=null;if(closePurpose){const expectedSeries='CLOSE-'+dataOpsSituationDigest(['CLOSE_SERIES',companyId,orderQM9Text(payload.businessDate),'KRW']).slice(0,40);if(orderQM9Text(payload.closeSeriesId)!==expectedSeries)throw new Error('CLOSE_CONTEXT_SCOPE_MISMATCH');closeContext=dataOpsCloseContext(ss,{closeSeriesId:expectedSeries,companyId:companyId,closeBusinessDate:orderQM9Text(payload.businessDate)},{actorId:actorId,roleIds:['DATAOPS_CLOSE_READ'],scopeDigest:orderQM9Digest({companyId:companyId})},properties);if(orderQM9Text(payload.closeContextDigest)!==orderQM9Text(closeContext.contextDigest))throw new Error('CLOSE_CONTEXT_CHANGED');}
  const head = orderQM9SituationHeadState(ss);
  const readSessionId = `OQS-${Utilities.getUuid()}`;
  const allowed = { ORDER:true, ORDER_ITEM:true, ORDER_EVENT:true, INVENTORY_MOVEMENT:true, PURCHASE_DOCUMENT:true, PURCHASE_LINE:true,
    SALES_DOCUMENT:true, SALES_LINE:true, VOUCHER_EVENT:true, PAYABLE_ENTRY:true, RECEIVABLE_ENTRY:true, CUSTOMER:true, PRODUCT:true, WAREHOUSE:true };
  ...
  const allMovementEntities=allRows.filter(row=>orderQM9Text(row.entityType).toUpperCase()==='INVENTORY_MOVEMENT'),reportedMovementEntities=allMovementEntities.filter(row=>{const value=row.payload||{},key=`${value.productId}\u001f${value.warehouseId}\u001f${value.baseUnit}`,sequence=Number(value.ledgerSequence||0);return cutoffByKey.has(key)&&sequence>cutoffByKey.get(key)&&sequence<=head.revision;});
  const bridgeLowerByKey=new Map();if(closeContext){(closeContext.priorInventoryRows||[]).forEach(row=>bridgeLowerByKey.set(orderQM9Text(row.inventoryKey)||`${row.productId}\u001f${row.warehouseId}\u001f${row.baseUnit}`,Number(row.bridgeUpperInclusive||0)));(closeContext.approvedBaselines||[]).forEach(row=>{const key=orderQM9Text(row.inventoryKey)||`${row.productId}\u001f${row.warehouseId}\u001f${row.baseUnit}`;if(!bridgeLowerByKey.has(key))bridgeLowerByKey.set(key,Number(row.orderqLedgerUpperBound||0));});}
  if(closePurpose)cutoffByKey.forEach((cutoff,key)=>{if(!bridgeLowerByKey.has(key))bridgeLowerByKey.set(key,cutoff);});
  const bridgeMovementEntities=closePurpose?allMovementEntities.filter(row=>{const value=row.payload||{},key=`${value.productId}\u001f${value.warehouseId}\u001f${value.baseUnit}`,sequence=Number(value.ledgerSequence||0);return bridgeLowerByKey.has(key)&&sequence>bridgeLowerByKey.get(key)&&sequence<=head.revision;}):reportedMovementEntities;
  const closeDate=orderQM9Text(payload.businessDate),dateMovedDocumentIds=new Set(allRows.filter(row=>orderQM9Text(row.entityType).toUpperCase()==='VOUCHER_EVENT').filter(row=>{const value=row.payload||{},before=value.beforeSnapshot||value.before||{},after=value.afterSnapshot||value.after||{};return orderQM9Text(before.businessDate||before.purchaseDate||before.saleDate)===closeDate||orderQM9Text(after.businessDate||after.purchaseDate||after.saleDate)===closeDate;}).map(row=>orderQM9Text((row.payload||{}).documentId))),closeDocumentIds=new Set(allRows.filter(row=>['PURCHASE_DOCUMENT','SALES_DOCUMENT'].includes(orderQM9Text(row.entityType).toUpperCase())).filter(row=>{const value=row.payload||{};return orderQM9Text(value.businessDate||value.purchaseDate||value.saleDate)===closeDate||dateMovedDocumentIds.has(orderQM9Text(row.entityId));}).map(row=>orderQM9Text(row.entityId))),evidenceMovementIds=new Set(allRows.filter(row=>orderQM9Text(row.entityType).toUpperCase()==='VOUCHER_EVENT'&&closeDocumentIds.has(orderQM9Text((row.payload||{}).documentId))).flatMap(row=>((row.payload||{}).lineEffects||[]).filter(effect=>effect.entityType==='INVENTORY_MOVEMENT').map(effect=>orderQM9Text(effect.entityId)))),evidenceOnlyMovementEntities=closePurpose?allMovementEntities.filter(row=>evidenceMovementIds.has(orderQM9Text(row.entityId))&&!bridgeMovementEntities.some(item=>item.entityId===row.entityId)&&!reportedMovementEntities.some(item=>item.entityId===row.entityId)):[];
  const roleByMovementId=new Map(),addRole=(row,role)=>{const id=orderQM9Text(row.entityId),roles=roleByMovementId.get(id)||new Set();roles.add(role);roleByMovementId.set(id,roles);};reportedMovementEntities.forEach(row=>addRole(row,'REPORTED_BRIDGE'));bridgeMovementEntities.forEach(row=>addRole(row,'CALCULATED_BRIDGE'));evidenceOnlyMovementEntities.forEach(row=>addRole(row,'EVIDENCE_ONLY'));const movementById=new Map();allMovementEntities.filter(row=>roleByMovementId.has(orderQM9Text(row.entityId))).forEach(row=>movementById.set(row.entityId,{...row,payload:{...(row.payload||{}),closeEvidenceRoles:[...roleByMovementId.get(orderQM9Text(row.entityId))].sort()}}));const movementEntities=[...movementById.values()].sort((a,b)=>Number(a.payload&&a.payload.ledgerSequence||0)-Number(b.payload&&b.payload.ledgerSequence||0)||orderQM9Text(a.entityId).localeCompare(orderQM9Text(b.entityId)));
  ...
  const closeManifest=entities=>{const rows=entities.map(row=>({...row.payload,status:row.status,revision:row.revision})).sort((a,b)=>Number(a.ledgerSequence)-Number(b.ledgerSequence)||orderQM9Text(a.movementId).localeCompare(orderQM9Text(b.movementId))),ids=rows.map(row=>orderQM9Text(row.movementId)),tombstones=rows.filter(row=>['TOMBSTONED','DELETED','INACTIVE'].includes(orderQM9Text(row.status).toUpperCase())),active=rows.filter(row=>tombstones.indexOf(row)<0),base={movementCount:ids.length,movementIds:ids,effectKeys:rows.map(row=>orderQM9Text(row.effectKey)).filter(Boolean).sort(),inventoryKeys:[...new Set(rows.map(row=>`${row.productId}\u001f${row.warehouseId}\u001f${row.baseUnit}`))].sort(),minLedgerSequence:rows.length?Math.min.apply(null,rows.map(row=>Number(row.ledgerSequence))):0,maxLedgerSequence:rows.length?Math.max.apply(null,rows.map(row=>Number(row.ledgerSequence))):0,activeDigest:orderQM9Digest(active),tombstoneIds:tombstones.map(row=>orderQM9Text(row.movementId)).sort(),tombstoneDigest:orderQM9Digest(tombstones),digest:orderQM9Digest(rows)};return{...base,manifestDigest:orderQM9Digest(base)};};
  const reportedBridgeManifest=closeManifest(reportedMovementEntities),calculatedBridgeManifest=closeManifest(bridgeMovementEntities),evidenceOnlyManifest=closeManifest(evidenceOnlyMovementEntities),compositePredicate={readPurpose:closePurpose?'DATAOPS_CLOSE':'CURRENT_SITUATION',dataOpsCutoffs:[...cutoffByKey.entries()].sort((a,b)=>a[0].localeCompare(b[0])),closeBridgeLower:[...bridgeLowerByKey.entries()].sort((a,b)=>a[0].localeCompare(b[0])),closeContextDigest:closeContext?closeContext.contextDigest:'',ledgerUpperBound:head.revision,reportedBridgeManifestDigest:reportedBridgeManifest.manifestDigest,calculatedBridgeManifestDigest:calculatedBridgeManifest.manifestDigest,evidenceOnlyManifestDigest:evidenceOnlyManifest.manifestDigest};
  const movementManifest = {...movementManifestBase,predicateDigest:orderQM9Digest(compositePredicate),reportedBridgeManifest,calculatedBridgeManifest,evidenceOnlyManifest,closeContextDigest:closeContext?closeContext.contextDigest:''};movementManifest.manifestDigest=orderQM9Digest(movementManifest);
