(() => {
  ...
  const APP = 'smart-input';
  if ((document.documentElement.dataset.nexusUiApp || '') !== APP) return;
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      #voucherInputTable:not([data-smartinput-force-hidden="true"]) { display:table !important; }
      ...
      .source-preparation-mapping__head { padding:9px 10px; display:flex; align-items:flex-start; justify-content:space-between; gap:8px; border-bottom:1px solid var(--border); background:var(--surface-strong); }
      ...
      .source-preparation-mapping__empty { padding:12px; color:var(--text-faint); font-size:10px; text-align:center; }
  ...
  function mappingHeaders() {
    return [...document.querySelectorAll('#mappingTableHeaders th[data-mapping-column]')];
  }

  function guessedTargetId(stateText) {
    const normalized = String(stateText || '').replace(/\(사용자\)/g, '').trim();
    const candidates = targets.filter(target => normalized.includes(target.label));
    return candidates.sort((a, b) => b.label.length - a.label.length)[0]?.id || '';
  }

  async function probeTargets() {
    if (targets.length || probing || applying) return targets;
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (dialog) {
          const buttons = [...dialog.querySelectorAll('[data-mapping-target]')];
          if (buttons.length) {
            targets = buttons.map(button => ({
              id: button.dataset.mappingTarget,
              label: button.querySelector('strong')?.textContent?.trim() || button.dataset.mappingTarget,
  ...
  function renderPanel() {
    ...
    panel.hidden = false;
    const list = byId('sourcePreparationList');
    ...
    list.innerHTML = headers.map((header, order) => {
      ...
      let value = staged.has(column) ? staged.get(column) : '';
      if (!staged.has(column)) {
        if (/비매핑/.test(stateText)) value = '__UNMAPPED__';
        else value = guessedTargetId(stateText);
        if (state === 'MAPPED' || state === 'UNMAPPED') staged.set(column, value);
      }
      const options = [
        `<option value="" ${value === '' ? 'selected' : ''}>항목 선택</option>`,
        `<option value="__UNMAPPED__" ${value === '__UNMAPPED__' ? 'selected' : ''}>비매핑 · 제외</option>`,
        ...targets.map(target => `<option value="${esc(target.id)}" ${value === target.id ? 'selected' : ''}>${esc(target.label)}</option>`),
      ].join('');
      return `<label class="source-preparation-mapping__row" data-state="${esc(state)}"><span><b>${order + 1}. ${esc(source)}</b><small>${esc(stateText)}</small></span><select data-source-column="${column}" aria-label="${esc(source)} 사용할 항목">${options}</select></label>`;
    }).join('');
    updateStatus();
  }
  ...
  function updateStatus() {
    ...
    status.textContent = duplicate
      ? '같은 사용할 항목이 중복 지정되었습니다. 중복을 해제하세요.'
      : undecided
        ? `매핑 ${mapped} · 제외 ${excluded} · 확인 필요 ${undecided}`
        : `매핑 ${mapped} · 제외 ${excluded} · ${headers.length}열 확인 완료`;
    apply.disabled = applying || duplicate || undecided > 0 || headers.length === 0;
  }

  async function openDialogForColumn(column) {
  ...
  async function choose(column, value) {
    const dialog = await openDialogForColumn(column);
    if (value === '__UNMAPPED__') {
      ...
      if (!button) {
        ...
        if (search && target) {
          ...
          await sleep(20);
          button = [...dialog.querySelectorAll('[data-mapping-target]')].find(node => node.dataset.mappingTarget === value);
        }
      }
      if (!button || button.disabled) throw new Error(`${column + 1}열의 선택 항목을 적용할 수 없습니다.`);
      button.click();
    }
    await sleep(25);
    if (dialog.open) dialog.close();
  ...
  async function applyStaged() {
    if (applying) return;
    const headers = mappingHeaders();
    const desired = headers.map(header => ({
      column: Number(header.dataset.mappingColumn),
      state: header.dataset.mappingState || '',
      value: staged.get(Number(header.dataset.mappingColumn)) || '',
      currentText: header.querySelector('small')?.textContent?.trim() || '',
    }));
    ...
    try {
      ...
      // Two phase prevents target swaps from colliding with the currently used target.
      for (const item of changed) {
        if (item.state === 'MAPPED' || item.state === 'RECOMMENDED') await choose(item.column, '__UNMAPPED__');
      }
      for (const item of changed) {
        if (item.value !== '__UNMAPPED__') await choose(item.column, item.value);
      }
      document.querySelector('[data-table-view="input"]')?.click();
      await sleep(40);
      staged = new Map();
      status.textContent = '매핑 저장 완료 · 중앙 작업표에 적용했습니다.';
      window.dispatchEvent(new CustomEvent('smartinput:source-mapping-applied'));
      queueRefresh(true);
    } catch (error) {
      status.textContent = `적용 중단 · ${error.message || '매핑을 적용하지 못했습니다.'}`;
    } finally {
      applying = false;
      updateStatus();
    }
  ...
  async function refresh(forceProbe = false) {
    ...
    renderPanel();
    const voucher = byId('voucherInputTable');
    const mapping = byId('mappingWorktable');
    if (mapping) mapping.hidden = true;
    if (voucher && !applying) voucher.hidden = false;
  }

  function queueRefresh(forceProbe = false) {
    if (refreshQueued) return;
    refreshQueued = true;
    setTimeout(() => { void refresh(forceProbe); }, 60);
  }
  ...
  function boot() {
    ...
    const observer = new MutationObserver(mutations => {
      if (applying || probing) return;
      if (mutations.some(mutation => mutation.target?.closest?.('#mappingWorktable, #sourceInputPanel, #tableScroll') || mutation.target?.id === 'mappingTableHeaders')) queueRefresh();
    });
    observer.observe(document.body, { subtree: true, childList: true, attributes: true, attributeFilter: ['hidden', 'data-mapping-state'] });
    queueRefresh(true);
  }
