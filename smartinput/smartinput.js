...
const OPTIONAL_FEATURE_MODULES = Object.freeze({
  ...
  voucherOutput: Object.freeze({ feature: 'voucher-output-module', assetVersion: '0.1.2', specifier: './purchase-sales-output.js?v=0.1.2', unavailableMessage: '구매 보고서 기능을 불러오지 못했습니다. 현재 입력은 유지됩니다.' }),
...
function loadDraft() {
  ...
  if (bootstrapDraft?.reused) {
    try {
      return resetResumedShoppingInspection(contract.normalizeDraft(bootstrapDraft.value));
    } catch (_) {}
  }
  try {
    return resetResumedShoppingInspection(contract.normalizeDraft(JSON.parse(localStorage.getItem(contract.DRAFT_STORAGE_KEY) || 'null')));
  } catch (_) {
...
function resetResumedShoppingInspection(draft) {
  const upload = draft?.modes?.order?.shoppingOrderImport;
  if (upload?.schemaVersion === SMARTINPUT_SHOPPING_ORDER_UPLOAD_SCHEMA) {
...
const estimateWorkspace = createEstimateWorkspace({
  store: estimateStore,
  // Existing local draft authority remains local; master uses the authenticated owner context separately.
  readContext: () => ({ status: 'READY', companyId: state.companyId,
    actor: { actorId: resolveSmartInputActor(), actorState: 'LOCAL_EDITOR' } }),
  readMasterContext: readSmartInputEstimateContext,
  prepareMaster: async () => {
    const context = await prepareSmartInputEstimateContext();
    if (context.status !== 'READY') return;
    await import('../coreEngine.js?v=smartinput-stage3-v1');
    if (typeof window.ONEAPP?.STORAGE?.commitMasterState !== 'function') throw new Error('상품관리 저장 기능을 불러오지 못했습니다.');
  },
  readProducts: getProductSnapshot,
  reviewMappings: async (plan, mappings) => {
    const decisions = [];
    for (const target of plan.targets) for (const issue of target.issues) {
      if (!['ESTIMATE_MAPPING_STALE', 'ESTIMATE_MAPPING_CONFLICT', 'ESTIMATE_MULTIPLE_ROWS_REQUIRE_MAPPING'].includes(issue.code)) continue;
      const record = target.expectedPreimage;
      const rows = issue.candidateRowIds.map(id => record.ownedRows.find(row => row.ownedRowId === id));
      const choice = window.prompt(`${estimateTitle(record)}에서 업데이트할 상품행 번호를 선택하세요. 여러 행은 쉼표로 구분합니다. 취소하면 해당 상품은 제외됩니다.\n${rows.map((row, index) => `${index + 1}. ${row.itemCode} ${row.itemName} · ${row.rowCustomerCode || ''} · ${row.ownedRowId}`).join('\n')}`);
      if (choice === null) continue;
      const indices = choice.split(',').map(value => Number(value.trim()) - 1);
      if (!indices.length || indices.some(index => !Number.isInteger(index) || !rows[index])) continue;
      const ownedRowIds = [...new Set(indices.map(index => rows[index].ownedRowId))];
      const previous = mappings.filter(mapping => mapping.companyId === plan.companyId && mapping.targetEstimateId === target.estimateId && mapping.sourceKey === issue.sourceKey);
      for (const before of previous.length ? previous : [null]) decisions.push({
        aliasMappingId: before?.aliasMappingId || estimateTechnicalKey('rowMapping', plan.companyId, plan.templateId, target.estimateId, ownedRowIds[0], issue.sourceKey),
        mappingType: 'ESTIMATE_DIRECT_ROW_V1', companyId: plan.companyId, keyVersion: DIRECT_ROW_KEY_VERSION,
        templateId: plan.templateId, templateRevision: plan.templateRevision, templateSignature: plan.templateSignature,
        targetEstimateId: target.estimateId, sourceKey: issue.sourceKey, ownedRowIds,
        mappingRevision: Number(before?.mappingRevision || 0) + 1, confirmedBy: plan.actor.actorId,
        confirmedAt: new Date().toISOString(), decisionBasis: 'ADMIN_SELECTED_ROWS' });
    }
    return decisions;
  },
  owner: createProductMasterCommandAdapter(),
  onChange: snapshot => {
    state.noticeEstimateIds = snapshot.selectedIds;
    state.estimateOperationBusy = snapshot.busy;
  }
});
...
function estimateExcelFile(current = modeDraft()) {
  ...
  return { companyId: state.companyId, fileGeneration: mapping.sessionId, fileFingerprint: mapping.fileFingerprint,
    sourceDocumentId: current.documentId, sourceRevision: current.updatedAt || mapping.updatedAt,
    templateId: mapping.templateId, templateRevision: mapping.templateRevision, templateSignature: mapping.signature };
}

function captureSelectedEstimateWork() {
  if (state.draft.activeMode !== 'estimate' || estimateExcelFile()) return;
  const current = modeDraft();
  if (!current.stage3SelectedTable) {
    if (current.catalogRecordId) estimateWorkspace.remember(current.catalogRecordId, current);
    return;
  }
  const original = state.estimateTableBaseline || [];
  const live = new Map(current.rows.map(row => [row.rowId, row]));
  const changed = new Set(original.filter(row => !estimateValuesEqual(row, live.get(row.rowId))).map(row => row.estimateOwner?.estimateId).filter(Boolean));
  for (const estimateId of changed) {
    const record = estimateWorkspace.getRecord(estimateId);
    if (!record) continue;
    const draft = structuredClone(estimateWorkspace.getWork(estimateId) || record.draft);
    const owned = new Map((draft.ownedRows || record.ownedRows).map(row => [row.ownedRowId, row]));
    for (const before of original.filter(row => row.estimateOwner?.estimateId === estimateId)) {
      const after = live.get(before.rowId);
      const pointer = before.estimateOwner;
      if (!after) { owned.delete(pointer.ownedRowId); continue; }
      if (estimateValuesEqual(before, after)) continue;
      const saved = { ...after, rowId: pointer.originalRowId, ownedRowId: pointer.ownedRowId };
      delete saved.estimateOwner; delete saved.estimateOwnerName;
      owned.set(pointer.ownedRowId, saved);
    }
    const displayGroups = (draft.displayGroups || record.displayGroups).map(group => ({ ...group,
      ownedRowIds: group.ownedRowIds.filter(key => owned.has(key)) })).filter(group => group.ownedRowIds.length);
    const working = projectIndependentEstimateDraft({ ...record, draft, ownedRows: [...owned.values()], displayGroups });
    estimateWorkspace.remember(estimateId, working);
  }
}

async function changeEstimateSelection(ids) {
  if (state.busy || state.estimateOperationBusy) return;
  ...
  if (!estimateExcelFile()) {
    ...
    const next = contract.normalizeModeDraft('estimate', { ...contract.createDraft().modes.estimate,
      header: previous.header, rows: [...loaded.rows, ...preservedRows], stage3SelectedTable: true, stage3LegacyPreview: legacy.length > 0, activeMethod: 'direct' });
    if (ids.length === 1 && legacy.length === 1) Object.assign(next,
      structuredClone(legacy[0].draft), { stage3SelectedTable: true, stage3LegacyPreview: true, catalogRecordId: legacy[0].estimateId });
    state.draft.modes.estimate = next;
    state.estimateTableBaseline = structuredClone(next.rows);
    state.selectedRowIds.clear(); sourceTextInput.value = '';
  }
  saveDraftNow(); renderMode(); renderEstimateExclusions();
}

async function flushDraftBeforeWorkspaceChange() {
  state.draft.ui.estimateWorkspace = { ...estimateWorkspace.journal(), tableBaseline: state.estimateTableBaseline };
  const ticket = queueDocumentCheckpoint('estimate', { trackDirty: false, bypassLoading: true });
...
async function runSelectedEstimateUpdate() {
  const current = modeDraft();
  if (!estimateWorkspace.selected().length) return toast('업데이트할 견적서를 선택하세요.', 'warn');
  const file = estimateExcelFile(current);
  ...
  try {
    ...
    const custom = mapped.filter(target => target.custom).map(target => ({ id: target.projectionFieldId || target.id,
      storageKey: target.projectionFieldId || target.id, custom: true, valueType: target.valueType }));
    const allowed = estimateUpdateFieldDefinitions(custom);
    const fields = [...new Set(mapped.map(target => target.custom ? `custom:${target.projectionFieldId || target.id}` : target.projectionFieldId || target.id))].filter(key => allowed.has(key));
    const result = await estimateWorkspace.updateExcel({ draft: structuredClone(current), file, allowedFieldIds: fields,
      fieldDefinitions: custom, masterFields: $('estimateApplyMasterOption').checked ? ['purchasePriceB', 'wholesaleA', 'wholesaleB', 'promoPrice'] : [] });
    const saved = result.receipts.filter(receipt => receipt.committed).length;
    setAppStatus(`견적서 ${saved}개 처리 · ${result.receipts.length - saved}개 확인 필요`, saved === result.receipts.length ? '' : 'warn');
    if ($('estimateApplyMasterOption').checked && saved) {
      const master = await estimateWorkspace.applyMaster({ update: result });
      setAppStatus(`견적서 ${saved}개 처리 · 마스터 ${master.status}`, ['APPLIED', 'UNCHANGED'].includes(master.status) ? '' : 'warn');
    }
    await hydrateEstimateLibrary();
  } catch (error) { toast(error.code || error.message, 'error'); }
  finally { state.busy = false; saveDraftNow(); renderCatalogControls(); renderDelivery(); renderEstimateExclusions(); }
}

async function runSelectedMasterUpdate() {
  if (state.busy) return;
  captureSelectedEstimateWork();
  state.busy = true; renderDelivery();
  try { const result = await estimateWorkspace.applyMaster(); setAppStatus(`마스터 상품관리: ${result.status}`, ['APPLIED', 'UNCHANGED'].includes(result.status) ? '' : 'warn'); }
  catch (error) { toast(error.code || error.message, 'error'); }
...
async function initializeAutosave() {
  ...
  try {
    ...
    if (bootstrapGeneration === state.optionalInputGeneration && !state.draftDirty) {
      ...
      if (adoptedJournal) { resetResumedShoppingInspection(state.draft); estimateWorkspace.restore(workspace?.ui?.estimateWorkspace); state.estimateTableBaseline = workspace?.ui?.estimateWorkspace?.tableBaseline || []; renderMode(); }
    }
...
async function restoreLatestAutosave() {
  ...
  try {
    ...
    state.draft = resetResumedShoppingInspection(restoredDraft);
    estimateWorkspace.restore(workspace?.ui?.estimateWorkspace || restoredDraft.ui?.estimateWorkspace);
    state.estimateTableBaseline = workspace?.ui?.estimateWorkspace?.tableBaseline || restoredDraft.ui?.estimateWorkspace?.tableBaseline || [];
...
function createRowFieldDisplayReader() {
  let targetsByProjection;
  return (row, projectionFieldId, fallback = '') => {
    if (!row?.fieldValues || !Object.keys(row.fieldValues).length) return fallback;
    if (!targetsByProjection) {
      targetsByProjection = new Map();
      inputMappingDefinitions().forEach(target => {
        const projection = target.projectionFieldId || target.id;
        if (!targetsByProjection.has(projection)) targetsByProjection.set(projection, target);
      });
    }
    const target = targetsByProjection.get(projectionFieldId);
...
function applyVoucherColumnOrder() {
  ...
  containers.forEach(container => {
    ...
    [...container.children].forEach(child => {
      if (!cells.has(child.dataset.column)) cells.set(child.dataset.column, child);
    });
    const status = cells.get('status');
    if (!status) return;
    let next = status;
    for (let index = ordered.length - 1; index >= 0; index -= 1) {
      const cell = cells.get(ordered[index]);
      if (!cell) continue;
      // Reparenting an already ordered cell drops focus from its active input.
      if (cell.nextElementSibling !== next) container.insertBefore(cell, next);
      next = cell;
    }
  });
}
...
function applyCustomerRelationship(header = modeDraft().header) {
  ...
  $('customerRelationHint').textContent = group
    ...
    : '연결되지 않은 배송 거래처입니다.';
  $('customerRelationHint').dataset.tone = '';
}
  ...
  try {
    const customer = await new Promise(resolve => {
      ...
      dialog.innerHTML = `<div class="smart-dialog__shell">
        ...
        <footer class="smart-link-footer" hidden><span>배송처를 1곳 이상 지정하세요. 세무거래처는 선택사항입니다.</span><button type="button" class="button button--quiet" data-tax-clear>세무 지정 안 함</button><button type="button" class="button button--quiet" data-link-cancel>취소</button><button type="button" class="button button--primary" data-link-save>관계 저장</button></footer>
      </div>`;
      document.body.append(dialog);
      ...
      const render = () => {
        ...
        visibleCustomers = normalizedCustomerCandidates(state.customers)
          ...
          .filter(customerItem => !query || normalizedKey(customerSearchText(customerItem)).includes(normalizedKey(query)))
            ...
            .slice(0, 80);
        if (linkMode && visibleCustomers.length === 1 && !selected.size) {
      ...
      dialog.querySelector('[data-tax-clear]').addEventListener('click', () => {
        selectedTaxCustomerId = '';
        render();
      });
      ...
      dialog.querySelector('[data-link-save]').addEventListener('click', async () => {
        ...
        if (selectedTaxCustomerId && temporaryMeta(selectedTaxCustomerId)) {
        ...
        const memberCustomerIds = [...new Set([...deliveryCustomerIds, ...(selectedTaxCustomerId ? [selectedTaxCustomerId] : [])])];
        const timestamp = new Date().toISOString();
        const group = {
...
function mappingValidationIssueText(issue, session = inputMappingSession()) {
  ...
  if (issue?.code === 'RECOMMENDATION_APPROVAL_REQUIRED') return `${column + 1}열 ${source} — 추천 매핑을 승인하거나 다른 항목을 선택하세요.`;
  if (issue?.code === 'REVIEW_REQUIRED') return `${column + 1}열 ${source} — 매핑을 다시 선택하여 확정하세요.`;
...
function estimateRecordsForKind() {
...
function beginEstimateMultiSelect() {
  return changeEstimateSelection([...new Set([...estimateWorkspace.selected(), ...availableCatalogs().map(record => record.estimateId)])]);
}

function cancelEstimateMultiSelect() {
  return changeEstimateSelection([]);
}
...
function renderVoucherActivitySnapshot() {
  ...
  if (activity.status === 'IDLE') {
    $('voucherContextSummary').textContent = '연동 전표를 아직 조회하지 않았습니다.';
    $('voucherContextList').innerHTML = '<div class="voucher-activity-state"><strong>전표 조회 대기</strong><span>목록 열기 또는 다시 불러오기를 실행하면 조회합니다. 현재 입력과 자동저장은 계속 사용할 수 있습니다.</span></div>';
    $('voucherContextDelivery').textContent = '조회 전';
    return;
  }
...
async function loadVoucherActivity({ force = false } = {}) {
  ...
  const date = voucherActivityDate(state.draft.activeMode);
  const companyId = state.companyId;
  if (!force && state.voucherActivity.companyId === companyId && state.voucherActivity.mode === mode && state.voucherActivity.date === date && ['LOADING', 'READY', 'EMPTY', 'ERROR'].includes(state.voucherActivity.status)) {
    return renderVoucherActivitySnapshot();
  }
  const requestId = ++state.voucherActivity.requestId;
  state.voucherActivity = { ...state.voucherActivity, requestId, status: 'LOADING', companyId, mode, sourceMode: mode, date, rows: [], error: null };
  renderVoucherActivitySnapshot();
  let snapshot;
  try {
    snapshot = mode === 'estimate'
      ? readEstimateVoucherActivity(date)
      : await (await loadOptionalFeature('voucherActivity')).readVoucherActivity({ mode, date, companyId });
  } catch (error) {
    snapshot = { status: 'ERROR', rows: [], error: { code: error.code || 'VOUCHER_ACTIVITY_READ_FAILED', message: error.message || String(error) } };
  }
...
function renderEstimateWorkspace() {
  ...
  $('catalogPickerList').hidden = !estimateMode;
  $('estimateMultiSelectButton').disabled = state.busy || !state.smartDataReady;
  $('estimateDeselectAllButton').disabled = state.busy;
  $('estimateLibrarySummary').textContent = '선택한 견적서만 업데이트합니다. 엑셀을 불러온 상태에서는 대상만 선택합니다.';
  parserCard.hidden = false;
  renderEstimateExclusions();
}
...
function syncEstimateLibraryActionState() {
  ...
  $('selectedEstimateDeleteButton').disabled = state.busy || !count;
  $('estimateRenameButton').disabled = state.busy || count !== 1;
  $('estimateMasterApplyButton').disabled = state.busy || !count;
...
function linkedEstimateUpdatesAfterSourceDeletion(records, deletedIds, timestamp) {
  const sourceRecords = records.filter(record => record.estimateKind !== 'LINKED_GROUP' && !deletedIds.has(record.estimateId));
  return new Map(records
    .filter(record => record.estimateKind === 'LINKED_GROUP'
      && (record.linkedEstimateSources || []).some(source => deletedIds.has(source.estimateId)))
    .map(record => [record.estimateId, removeLinkedEstimateSources({
      linkedRecord: record,
      sourceRecords,
      removedEstimateIds: [...deletedIds],
      occurredAt: timestamp
    })]));
}

function pruneLinkedEstimateWorkingCopies(linkedUpdates, deletedIds, timestamp) {
  if (!linkedUpdates.size) return;
  const sourceRecords = state.estimates.filter(record => record.estimateKind !== 'LINKED_GROUP' && !deletedIds.has(record.estimateId));
  linkedUpdates.forEach((updatedRecord, linkedEstimateId) => {
    const workingDraft = state.estimateWorkingCopies.get(linkedEstimateId);
    if (!workingDraft) {
      state.estimateWorkingCopyBaselines.set(linkedEstimateId, JSON.parse(JSON.stringify(updatedRecord.draft)));
      return;
    }
    const pruned = removeLinkedEstimateSources({
      linkedRecord: { ...updatedRecord, draft: workingDraft },
      sourceRecords,
      removedEstimateIds: [...deletedIds],
      occurredAt: timestamp
    });
    pruned.draft.rows = restoreLinkedEstimateWorkingRowEdits({
      materializedRows: pruned.draft.rows,
      workingRows: workingDraft.rows || []
    });
    state.estimateWorkingCopies.set(linkedEstimateId, pruned.draft);
    state.estimateWorkingCopyBaselines.set(linkedEstimateId, JSON.parse(JSON.stringify(updatedRecord.draft)));
  });
}
...
async function deleteSelectedEstimates() {
  ...
  if (!selected.length || !window.confirm(`선택한 견적서 ${selected.length}개만 삭제할까요? 다른 견적서는 유지됩니다.`)) return;
  captureSelectedEstimateWork(); await flushDraftBeforeWorkspaceChange();
  state.busy = true; renderDelivery();
  const removed = [];
  try {
    await estimateWorkspace.loadSelection();
    for (const estimateId of selected) {
      await estimateWorkspace.edit({ estimateId, action: 'DELETE' }); removed.push(estimateId);
    }
  } catch (error) { toast(error.code || error.message, 'error'); }
  finally {
    state.busy = false;
    estimateWorkspace.select(selected.filter(id => !removed.includes(id)));
    await hydrateEstimateLibrary(); saveDraftNow(); renderMode();
  }
}
...
function updatedEstimateInformationBundle(record, { catalogName, customer }, timestamp) {
  ...
  if (customer) {
    for (const [target, value] of Object.entries({ customerId: customer.customerId || '', customerCode: customer.customerCode || customer.code || '', customerName: customer.customerName || customer.name || '' })) {
      next[target] = value; next.draft.header[target] = value;
    }
  }
...
async function openSelectedEstimateInformationDialog() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const next = { ...bundle[0], dataRevision: record.dataRevision + 1 };
      next.ownedRows = next.ownedRows.map(row => ({ ...row, matchIdentity: estimateIdentityFromRow(row, next.draft.header) }));
      ...
      await estimateStore.commitIndependentEstimateEdit({ companyId: state.companyId,
        ...
        estimateId: record.estimateId, expectedPreimage: record, candidate: next, action: 'METADATA' });
      await estimateStore.completePendingEstimateOperation({ companyId: state.companyId, kind: 'edit', id: operationId });
      bundle[0] = next;
...
async function persistEstimateLibrary(records = state.estimates) {
  ...
  for (const candidate of records) {
    ...
    await estimateStore.commitIndependentEstimateEdit({ companyId: state.companyId,
      actor: { actorId: resolveSmartInputActor(), actorState: 'LOCAL_EDITOR' }, operationId,
      estimateId: before.estimateId, expectedPreimage: before, candidate: next, action: 'METADATA' });
    await estimateStore.completePendingEstimateOperation({ companyId: state.companyId, kind: 'edit', id: operationId });
  }
  await hydrateEstimateLibrary();
}
...
async function persistEstimateCardOrder(payload, target) {
  ...
  const sourceRecords = availableCatalogs();
  ...
  const next = reordered.map((record, index) => ({ ...record, sortOrder: index + 1 }));
...
async function rematchRowsForCustomer(customer) {
  ...
  try {
    ...
    const matched = await withTimeout(
      rematchExtractedLinesForCustomer(before, customer, 'SMART_INPUT', inputMatchingContext()),
      OPTIONAL_OPERATION_TIMEOUT_MS.externalReference,
...
function applyWarehouseMatch() {
  ...
  renderVoucherContext();
  if (shoppingOrderImport()) invalidateShoppingOrderInspection();
  scheduleSave();
}
...
async function deleteSelectedGridRows() {
  ...
  renderRows();
  saveDraftNow();
  toast(linkedRows.length
    ? `선택한 품목을 작업표에서 삭제했습니다. 저장은 선택한 견적서에만 반영됩니다.`
    : '선택한 품목을 삭제했습니다.', 'success');
}
...
function invalidateShoppingOrderInspection() {
...
async function refreshShoppingOrderInspection({ persist = true, announce = false, isCurrent = null } = {}) {
  ...
  const discardStaleResult = () => {
    ...
    if (requestId === state.shoppingInspectionRequestId
      ...
      invalidateShoppingOrderInspection();
    }
    return null;
  };
...
function shoppingDecisionLabel(decision, commitDecision) {
  ...
  return { status: 'PENDING', label: '전달 시 원장 확인' };
}
...
function renderShoppingOrderPanel() {
  ...
  const resolvedSummary = upload.status === 'ERROR'
    ? '원장 판정 오류'
    : (upload.status !== 'READY'
      ? (upload.status === 'ANALYZING' ? '실제 원장 확인 중' : '입력 준비됨 · 전달 시 원장 확인')
...
async function completeShoppingOrderImport() {
  ...
  try {
    if (upload.status !== 'READY') {
      const inspection = await refreshShoppingOrderInspection({ persist: true });
      if (!inspection || shoppingOrderImport() !== upload || upload.status !== 'READY') return toast('실제 ORDER Q 원장 판정을 확인한 뒤 저장하세요.', 'error');
    }
...
function updateSummaries() {
  ...
  if (shopping) {
    ...
    $('matchedCount').textContent = shopping.inspection ? `신규 ${Number(summary.newCount || 0).toLocaleString('ko-KR')}` : '신규 여부 미조회';
    $('similarCount').textContent = shopping.inspection ? `확인 ${Number(summary.reviewRequiredCount || 0).toLocaleString('ko-KR')}` : '전달 전';
    $('failedCount').textContent = shopping.inspection ? `기존 제외 ${Number(summary.duplicateCount || 0).toLocaleString('ko-KR')}` : '중복 여부 미조회';
    $('duplicateCount').textContent = shopping.inspection ? '실제 원장 기준' : '로컬 입력 준비됨';
...
function renderMode({ persistCleanup = true, scheduleAnalysis = true } = {}) {
  ...
  if (!referencesReady()) {
    ...
    setAppStatus(selected.id === 'order'
      ? '주문서 입력을 시작할 수 있습니다.'
      : (selected.id === 'estimate' ? '엑셀과 견적서는 어느 순서로든 선택할 수 있습니다. 선택한 견적서만 업데이트합니다.' : `${selected.label} 입력 화면입니다. 전달 연결은 준비 중입니다.`));
  }
  ...
  try {
    ...
    if (mode === 'estimate') {
      ...
      if (estimateCreationActive()) {
        ...
        state.noticeEstimateIds = estimateWorkspace.selected();
        state.estimateSelectionReturnDraft = null;
      }
    }
    saveDraftNow();
...
function materializeDefaultRow(tr, sourceInput = document.activeElement) {
  ...
  // Publish the new key to the viewport before a later blur/resize paints it.
  // Normal rendering preserves the active input and defers during composition.
  queueMicrotask(() => renderRows({ restoreFocus: false }));
...
async function handleFile(file) {
  // A newly supplied file is an upload, even after viewing preserved legacy evidence.
  modeDraft().stage3LegacyPreview = false;
  if (!file) return;
  ...
  try {
    ...
    if (/\.(xlsx|xls|csv|tsv)$/i.test(file.name)) {
      ...
      if (shoppingSelected) {
        ...
        assertOptionalOperationCurrent(operationToken);
        const totals = shoppingUploadTotals(upload);
        setAppStatus(`쇼핑몰 주문내역 ${totals.rowCount}행 · 후보 ${Number(upload.candidates?.length || 0)}건 · 입력 준비됨`);
        toast('쇼핑몰 17열 원본을 불러왔습니다. 주문 확인·전달을 실행하면 원장을 확인합니다.', 'success');
        return;
      }
      if (!selected) throw new Error('읽을 수 있는 Excel 시트가 없습니다.');
      if (modeId === 'estimate') { captureSelectedEstimateWork(); await flushDraftBeforeWorkspaceChange(); $('estimateApplyMasterOption').checked = false; }
      captureGridPasteUndo();
      const current = state.draft.modes[modeId];
      const fresh = contract.createDraft({ activeMode: modeId }).modes[modeId];
      current.header = cloneGridValue(fresh.header);
...
function tryMatchRow(row, changedField = '', { focusTarget = null } = {}) {
  ...
  if (focusTarget) inputViewport.invalidate();
...
function trySearchProductRow(row, query = '', { focusTarget = null } = {}) {
  ...
  // Enter publishes its result while the source row still owns focus. Change
  // events and ordinary typing retain the normal active-input protection.
  if (focusTarget) inputViewport.invalidate();
  if (match.autoConfirm) {
...
function merchOpsEstimateOutputConfig() {
  ...
  if (result?.status !== 'READY' || !values || !Array.isArray(values.marginRules)
    || !values.mappings || typeof values.mappings !== 'object' || Array.isArray(values.mappings)) {
    const error = new Error('견적 가격·매핑 설정을 읽지 못했습니다. 설정을 확인한 뒤 보고서를 다시 실행하세요. 현재 입력과 견적서는 유지됩니다.');
    error.code = result?.error?.code || 'SETTINGS_READ_FAILED';
    throw error;
  }
  return {
...
function estimateF8ReusableIndependentCopy(diagnosis, records = state.estimates) {
  if (diagnosis?.status !== 'ALL_MISSING') return null;
  return records.find(record => record?.estimateKind !== 'LINKED_GROUP'
    && record?.draft && Array.isArray(record.draft.rows)
    && record?.recoveryOrigin?.type === 'LINKED_SNAPSHOT_WITHOUT_SOURCES'
    && record.recoveryOrigin.sourceLinkedEstimateId === diagnosis.targetEstimateId
...
async function exportEstimateExcel() {
  ...
  try {
    ...
    let selectedRecords = selectedEstimateRecords().map(record => ({ ...record,
      draft: estimateWorkspace.getWork(record.estimateId) || record.draft }));
    ...
    try {
      ...
      toast('복구된 견적서는 저장됐지만 Excel 파일은 생성되지 않았습니다. F8을 다시 실행하면 같은 사본으로 재시도합니다.', 'error');
      return;
    }
    ...
    setAppStatus(`견적 F8 Excel 생성 실패 · ${error.message || '출력 자료를 확인하세요.'}`, 'error');
    toast(error.message || '보고서를 만들지 못했습니다. 현재 견적은 유지됩니다.', 'error');
  } finally {
...
function validateEstimateDocument() {
  ...
  if (!current.rows.length) {
    ...
    const deletingAllLinkedRows = current.estimateKind === 'LINKED_GROUP'
      && loadedRecord?.estimateKind === 'LINKED_GROUP'
      && (loadedRecord.draft?.rows || []).some(rowHasLinkedSource);
    if (deletingAllLinkedRows) return true;
...
function linkedSourceOptionMarkup(row, source, preselected) {
  ...
  const deletion = row.operation === 'DELETE';
  ...
  const tag = deletion ? 'article' : 'label';
  const metricAfter = deletion ? { quantity: '삭제', unitPrice: '삭제', amount: '삭제' } : after;
  return `<${tag} class="linked-source-edit-option ${deletion ? 'is-selected' : ''}" data-source-option>
    ${deletion ? '' : `<input type="radio" name="linked-source-${esc(row.rowId)}" value="${esc(source.key)}" data-source-choice data-working-row-id="${esc(row.rowId)}" ${preselected ? 'checked' : ''}>`}
    <span class="linked-source-edit-option__body">
      <span class="linked-source-edit-option__title"><strong>${esc(source.estimateName)}</strong><em>${deletion ? '삭제 대상' : (before ? `${differenceCount}개 차이` : '신규 추가')}</em></span>
      <small class="linked-source-edit-option__id">견적 ID ${esc(source.estimateId)} · ${esc(sourceRow)}</small>
      <span class="linked-source-edit-identity"><small>원본 품목</small><b>${esc(linkedSourceIdentity(before))}</b><small>${deletion ? '처리' : '작업 품목'}</small><strong>${deletion ? '삭제' : esc(linkedSourceIdentity(after))}</strong></span>
      <span class="linked-source-edit-metrics">
        ${linkedSourceMetricMarkup('수량', before, metricAfter, 'quantity')}
        ${linkedSourceMetricMarkup('단가', before, metricAfter, 'unitPrice')}
        ${linkedSourceMetricMarkup('금액', before, metricAfter, 'amount')}
      </span>
...
function linkedSourceRowMarkup(row) {
  ...
  const deletion = row.operation === 'DELETE';
  const actionLabel = row.operation === 'ADD' ? '신규 행 추가' : (deletion ? '기존 행 삭제' : '기존 행 수정');
  const changedLabels = row.changedFields.map(field => LINKED_ESTIMATE_FIELD_LABELS[field] || field).join(' · ');
  return `<section class="linked-source-edit-row" data-linked-source-row="${esc(row.rowId)}" data-selection-required="${singleSource || deletion ? 'false' : 'true'}">
    <header><div><small>작업행 ${row.rowNo} · ${actionLabel}</small><h3>${esc(linkedSourceIdentity(row.after || row.before))}</h3></div><span>${esc(deletion ? '연결 원본 품목 삭제' : (changedLabels || '신규 품목'))}</span></header>
    <p>${deletion ? `이 품목을 연결된 원본 ${row.sources.length}개에서 삭제합니다. 품목이 0개가 되는 원본 견적서는 함께 삭제됩니다.` : (singleSource ? '연결된 원본 한 건을 확인하고 적용하세요.' : (row.operation === 'ADD' ? '이 행을 추가할 원본 견적서를 직접 선택하세요.' : '값을 바꿀 원본 견적서와 원본 행을 직접 선택하세요.'))}</p>
...
function showLinkedEstimateSourceEditDialog(evidence) {
  return new Promise(resolve => {
    ...
    dialog.innerHTML = `<div class="smart-dialog__shell">
      ...
      <p class="smart-dialog__message">${esc(evidence.linkedEstimateName)}의 ${evidence.rows.length}개 작업행입니다. 수정은 선택한 원본에, 삭제는 연결된 모든 원본에 반영되며 하나의 원자적 저장으로 처리됩니다.</p>
      <div class="linked-source-edit-body">${evidence.rows.map(linkedSourceRowMarkup).join('')}</div>
      <footer><small data-selection-status aria-live="polite"></small><button type="button" class="button button--quiet" data-close>취소</button><button type="button" class="button button--primary" data-confirm-source>변경 저장</button></footer>
    </div>`;
    document.body.append(dialog);
    let settled = false;
    const selections = {};
    ...
    const sync = () => {
      ...
      const unresolved = evidence.rows.filter(row => row.operation !== 'DELETE' && !(row.operation === 'UPDATE' && row.sources.length === 1) && !selections[row.rowId]);
    ...
    sync();
    dialog.showModal();
    (dialog.querySelector('[data-source-choice]') || dialog.querySelector('[data-confirm-source]'))?.focus({ preventScroll: true });
  });
}

function estimateBulkCustomerLabel(group = {}) {
...
function estimateBulkRowReviewHtml(entry = {}) {
  const targetRows = entry.target?.draft?.rows || [];
  return (entry.issues || [])
    .filter(issue => issue.code === 'ESTIMATE_BULK_ROW_MATCH_AMBIGUOUS')
    .map(issue => {
      const sourceRowId = issue.sourceRowId || issue.sourceRowIds?.[0] || '';
      const current = entry.decision?.rowResolutions?.[sourceRowId];
      const value = current?.action === 'USE_EXISTING'
        ? `USE_EXISTING:${current.targetRowId}`
        : (current?.action || '');
      const candidateOptions = (issue.candidateRowIds || []).map(rowId => {
        const row = targetRows.find(candidate => String(candidate.rowId || '') === String(rowId)) || {};
        const label = row.itemName || row.itemCode || rowId;
        return `<option value="USE_EXISTING:${esc(rowId)}"${value === `USE_EXISTING:${rowId}` ? ' selected' : ''}>기존 행 · ${esc(label)}</option>`;
      }).join('');
      return `<label class="estimate-bulk-row-review"><span>${esc(issue.message)}</span><select data-bulk-row-resolution data-source-row-id="${esc(sourceRowId)}" aria-label="${esc(issue.message)}"><option value="">처리 방법을 선택하세요</option>${candidateOptions}<option value="ADD_NEW"${value === 'ADD_NEW' ? ' selected' : ''}>신규 행 추가</option><option value="EXCLUDE"${value === 'EXCLUDE' ? ' selected' : ''}>이번 품목 제외</option></select></label>`;
    }).join('');
}
...
async function applyEstimatePerCustomerUpdates(plan, selectedGroupIds, onProgress) {
  ...
  for (const component of components) {
    ...
    try {
      ...
      const expectedEstimatePreimages = collectEstimateBulkReadPreimages({
        estimates: state.estimates,
        changedEstimateIds: [...changedById.keys()],
        linkedRecords: linkedUpserts
      });
...
function showEstimateBulkUpdateDialog(classification) {
  ...
  const selections = Object.fromEntries(currentPlan.entries.map(entry => [entry.groupId, {
    ...
    rowResolutions: cloneGridValue(entry.decision?.rowResolutions || {})
  }]));
  ...
  dialog.innerHTML = `<div class="smart-dialog__shell">
    ...
    <div class="estimate-bulk-body">${currentPlan.entries.map(entry => `<section class="estimate-bulk-row" data-bulk-group="${esc(entry.groupId)}" data-bulk-state-value="${esc(entry.status)}">
        ...
        <div class="estimate-bulk-row__reviews" data-bulk-row-reviews></div>
      </section>`
    ).join('')}</div>
  ...
  let settled = false;
  let applying = false;
  let view = currentPlan.entries.some(entry => ['PENDING', 'FAILED'].includes(entry.status)) ? 'review' : 'all';
  const finish = () => {
  ...
  const sync = () => {
    ...
    currentPlan.entries.forEach((entry, index) => {
      ...
      section.querySelector('[data-bulk-row-reviews]').innerHTML = estimateBulkRowReviewHtml(entry);
  ...
  dialog.addEventListener('change', event => {
    const select = event.target.closest?.('[data-bulk-row-resolution]');
    if (!select) return;
    const section = select.closest('[data-bulk-group]');
    const groupId = section.dataset.bulkGroup;
    const sourceRowId = select.dataset.sourceRowId;
    const base = selections[groupId] || { action: 'NONE', targetEstimateId: '', catalogName: '', matchMethod: 'MANUAL' };
    const rowResolutions = cloneGridValue(base.rowResolutions || {});
    if (!select.value) delete rowResolutions[sourceRowId];
    else if (select.value.startsWith('USE_EXISTING:')) rowResolutions[sourceRowId] = { action: 'USE_EXISTING', targetRowId: select.value.slice(13) };
    else rowResolutions[sourceRowId] = { action: select.value, targetRowId: '' };
    selections[groupId] = { ...base, rowResolutions, matchMethod: 'MANUAL' };
    selectionTouched.delete(groupId);
    sync();
  });
  ...
  dialog.querySelector('[data-confirm-bulk]').addEventListener('click', async () => {
    ...
    try {
      ...
      applied.results.filter(result => result.targetEstimateId).forEach(result => {
        selections[result.groupId] = {
          action: 'UPDATE',
          targetEstimateId: result.targetEstimateId,
          catalogName: '',
          matchMethod: 'MATCH_DICTIONARY_SAVED',
          rowResolutions: cloneGridValue(selections[result.groupId]?.rowResolutions || {})
        };
      });
      applying = false;
...
async function runAutomaticEstimateBulkUpdates(classification) {
  let plan = createEstimatePerCustomerPlanForCurrent(classification);
  const executableIds = new Set(plan.entries
  ...
  if (executableIds.size) {
    ...
    try {
      await applyEstimatePerCustomerUpdates(plan, executableIds);
    } finally {
      state.busy = false;
      renderDelivery();
    }
...
async function saveSelectedEstimateTable() {
  ...
  if (newRows.length) {
    let targetId = selected.length === 1 ? selected[0] : '';
    if (!targetId) {
      const names = selected.map((id, index) => `${index + 1}. ${estimateTitle(estimateWorkspace.getRecord(id))}`);
      const choice = window.prompt(`추가한 ${newRows.length}개 행을 저장할 견적서 번호를 입력하세요.\n${names.join('\n')}`);
      if (choice === null) return;
      targetId = selected[Number(choice) - 1];
      if (!targetId) return toast('견적서를 선택하지 않아 추가 행은 작업표에 유지됩니다.', 'warn');
    }
    const record = estimateWorkspace.getRecord(targetId);
    const draft = structuredClone(estimateWorkspace.getWork(targetId) || record.draft);
    const owned = newRows.map(row => ({ ...row, ownedRowId: row.rowId, matchIdentity: estimateIdentityFromRow(row, draft.header) }));
    draft.ownedRows = [...(draft.ownedRows || record.ownedRows), ...owned];
    draft.displayGroups = [...(draft.displayGroups || record.displayGroups), ...owned.map(row => ({ rowId: row.rowId, ownedRowIds: [row.ownedRowId], visible: true }))];
    draft.rows.push(...owned);
    estimateWorkspace.remember(targetId, draft);
    for (const row of newRows) row.estimateOwner = { companyId: state.companyId, estimateId: targetId,
      ownedRowId: row.rowId, originalRowId: row.rowId, sourceRevision: record.dataRevision };
  }
  captureSelectedEstimateWork();
  state.busy = true; renderDelivery();
  let count = 0;
  try {
    for (const estimateId of selected) {
      const draft = estimateWorkspace.getWork(estimateId);
      if (!draft) continue;
      await estimateWorkspace.edit({ estimateId, draft }); count += 1;
    }
    await hydrateEstimateLibrary();
    setAppStatus(`선택 견적서 ${count}개 저장 · 마지막 엑셀 결과 유지`);
  } catch (error) { toast(error.code || error.message, 'error'); }
  finally { state.busy = false; saveDraftNow(); renderDelivery(); }
}
...
async function saveEstimateDocument(catalogName) {
  ...
  try {
    ...
    const draft = createCatalogOnlyDraft(structuredClone(current), estimateId);
    if (previous) {
      estimateWorkspace.adopt(previous);
      await estimateWorkspace.edit({ estimateId, draft, name: requestedName, expectedRecord: previous });
    } else {
      const rows = draft.rows.filter(rowHasMeaningfulInput);
      const ownedRows = rows.map(row => ({ ...structuredClone(row), ownedRowId: row.rowId,
        matchIdentity: estimateIdentityFromRow(row, draft.header) }));
      const summary = contract.summarizeRows(rows);
      const record = { estimateId, companyId: state.companyId, schemaVersion: INDEPENDENT_ESTIMATE_SCHEMA,
        estimateKind: 'INDIVIDUAL', linkedEstimateSources: [], dataRevision: 0, ownedRows,
        displayGroups: ownedRows.map(row => ({ rowId: row.rowId, ownedRowIds: [row.ownedRowId], visible: true })),
        catalogName: requestedName, customerId: draft.header.customerId, customerCode: draft.header.customerCode,
        customerName: draft.header.customerName, rowCount: summary.total, amount: summary.amount,
        sortOrder: state.estimates.length + 1, createdAt: timestamp, updatedAt: timestamp, draft };
      record.draft = projectIndependentEstimateDraft(record);
      const operationId = createRecordId('SIEDIT');
      await estimateStore.commitIndependentEstimateEdit({ companyId: state.companyId,
        actor: { actorId: resolveSmartInputActor(), actorState: 'LOCAL_EDITOR' }, operationId, estimateId,
        expectedPreimage: null, candidate: record });
      await estimateStore.completePendingEstimateOperation({ companyId: state.companyId, kind: 'edit', id: operationId });
    }
    const saved = await estimateStore.loadEstimateForUpdate({ companyId: state.companyId, estimateId });
    if (saved.status !== 'READY') throw new Error('저장 결과 재조회가 필요합니다. 견적서 재시도로 확인하세요.');
    estimateWorkspace.adopt(saved.record);
    if (current === modeDraft()) {
      state.draft.modes.estimate = contract.normalizeModeDraft('estimate', saved.record.draft);
      estimateWorkspace.select([estimateId]);
    }
    await hydrateEstimateLibrary(); saveDraftNow(); renderMode();
    setAppStatus(`“${requestedName}” 견적서를 저장했습니다.`); return true;
  } catch (error) { toast(error.code || error.message, 'error'); return false; }
...
async function hydrateReferences() {
  ...
  const results = await Promise.allSettled([
    ...
    withTimeout(loadLocalWarehouseCatalog(companyId), OPTIONAL_OPERATION_TIMEOUT_MS.capability, '로컬 창고 기준자료 로딩 시간 초과')
  ]);
  if (!optionalOperationIsLatest(referenceToken)) return;
  ...
  if (referencesReady() && [results[2], smartDataResult[0]].some(result => result.status === 'rejected')) {
    setAppStatus(results[2].status === 'rejected'
      ? `로컬 입력 사용 가능 · ${results[2].reason?.message || '창고 기준정보 갱신이 필요합니다.'}`
      : '기준정보 준비됨 · 설정 자료 일부를 불러오지 못했습니다.', 'warn');
  } else if (referencesReady() && state.warehouseCatalog.aliasesAvailable === false) {
    setAppStatus('기준정보 준비됨 · 기존 로컬 창고 자료에는 별칭이 없습니다. 별칭을 사용하려면 기준정보 갱신을 실행하세요.', 'warn');
  }
}
...
async function refreshAllReferencesFromToolbar() {
  ...
  try {
    ...
    state.warehouseCatalog = {
      warehouses: rowsByDomain.warehouse,
      aliases: rowsByDomain.warehouse.flatMap(row => row.referenceAliases || []),
      aliasesAvailable: true,
      revision: result.generation.domains.warehouse.ownerRevision
    };
    renderWarehouseOptions();
    state.purchaseCapability = { ready: false, deferred: true, code: 'ORDERQ_PURCHASE_STAGE3_CAPABILITY_DEFERRED', detail: '저장 시 확인' };
    state.saleCapability = { ready: false, deferred: true, code: 'ORDERQ_SALE_STAGE4_CAPABILITY_DEFERRED', detail: '저장 시 확인' };
...
function selectEstimateLibraryKind(kind, { focusSnapshot = null } = {}) {
  state.estimateLibraryKind = 'individual';
  syncEstimateLibraryView(); restoreEstimateLibrarySwitchFocus(focusSnapshot);
}
const estimateLibraryIndividualButton = $('estimateLibraryIndividualButton');
[estimateLibraryIndividualButton].forEach(button => {
...
function handleEstimateCardSelection(event) {
  if (state.estimateDragSuppressed || event.target.closest('[data-estimate-drag-handle]')) return;
  ...
  state.estimateSelectionQueue = state.estimateSelectionQueue.then(() => {
    const selected = new Set(estimateWorkspace.selected());
    if (selected.has(card.dataset.estimateId)) selected.delete(card.dataset.estimateId); else selected.add(card.dataset.estimateId);
    return changeEstimateSelection([...selected]);
  }).catch(error => toast(error.code || error.message, 'error'));
}
...
$('estimateRetryButton').addEventListener('click', async () => {
  try {
    ...
    await hydrateEstimateLibrary();
    setAppStatus(result.status === 'NO_PENDING_OPERATION' ? '재시도할 견적서 작업이 없습니다.' : `견적서 재시도: ${result.outcomes.map(item => item.status).join(", ")}`); saveDraftNow(); renderEstimateExclusions();
  } catch (error) { toast(error.code || error.message, 'error'); }
});
