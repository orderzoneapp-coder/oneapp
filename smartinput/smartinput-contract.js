(function initSmartInputContract(global) {
  ...
  const PRODUCT_FIELD_DEFINITIONS = Object.freeze([
    ...
    productField('rowCustomerCode', '거래처코드', 'ADDITIONAL', { inputAliases: ['고객코드', '구매처코드', '판매처코드'] }),
    productField('rowCustomerName', '거래처명', 'ADDITIONAL', { inputAliases: ['배송처', '구매처명', '판매처명', '고객명'] }),
    productField('deliveryCustomerId', '배송처ID', 'ADDITIONAL'),
    productField('deliveryCustomerCode', '배송처코드', 'ADDITIONAL'),
    productField('deliveryCustomerName', '배송처명', 'ADDITIONAL'),
    productField('billingCustomerId', '세무거래처ID', 'ADDITIONAL'),
    productField('billingCustomerCode', '세무거래처코드', 'ADDITIONAL'),
    productField('billingCustomerName', '세무거래처명', 'ADDITIONAL'),
    productField('supplierCustomerId', '공급처ID', 'ADDITIONAL'),
    productField('supplierCustomerCode', '공급처코드', 'ADDITIONAL'),
    productField('supplierCustomerName', '공급처명', 'ADDITIONAL'),
    productField('salesCustomerId', '판매업무처ID', 'ADDITIONAL'),
    productField('salesCustomerCode', '판매업무처코드', 'ADDITIONAL'),
    productField('salesCustomerName', '판매업무처명', 'ADDITIONAL'),
  ...
  function normalizeHeader(value = {}, fallback = {}) {
    ...
    return {
      ...
      customerCode: text(value.customerCode || fallback.customerCode),
  ...
  function normalizeRow(input = {}, fallbackBatchId = '') {
    ...
    const row = {
      ...
      sourceRowNo: Number(input.sourceRowNo || input.sourceLineNo || 0),
      sourceLineKey: text(input.sourceLineKey),
      sourceId: text(input.sourceId),
      sourceBatchId: text(input.sourceBatchId || input.batchId || fallbackBatchId),
      sourceDocumentKey: text(input.sourceDocumentKey),
      sourceVoucherIndex: numberOrNull(input.sourceVoucherIndex) ?? 1,
      sourceSheetName: text(input.sourceSheetName),
      manualSplitKey: text(input.manualSplitKey),
      sourceFingerprint: text(input.sourceFingerprint),
      sourceType: text(input.sourceType),
      contractKind: text(input.contractKind),
      originSystem: text(input.originSystem),
      originTransactionId: text(input.originTransactionId),
      directOriginSystem: text(input.directOriginSystem),
      directOriginTransactionId: text(input.directOriginTransactionId),
      ...
      metaStatus: text(input.metaStatus),
      rowDigest: text(input.rowDigest),
      suggestedQuantity: numberOrNull(input.suggestedQuantity),
      suggestedUnit: text(input.suggestedUnit),
      suggestedBaseQuantity: numberOrNull(input.suggestedBaseQuantity),
      ...
      rawQuantity: numberOrNull(input.rawQuantity ?? input.quantity ?? input.finalQuantity),
      rawUnit: text(input.rawUnit || input.unit || input.finalUnit),
      baseQuantity: numberOrNull(input.baseQuantity ?? input.quantity ?? input.finalQuantity ?? input.rawQuantity),
      baseUnit: text(input.baseUnit || input.unit || input.finalUnit || input.rawUnit),
      unitConversionSource: text(input.unitConversionSource) || 'SAME_UNIT',
      rowCustomerCode: text(input.rowCustomerCode),
      rowCustomerId: text(input.rowCustomerId),
      rowCustomerName: text(input.rowCustomerName),
      deliveryCustomerId: text(input.deliveryCustomerId),
      deliveryCustomerCode: text(input.deliveryCustomerCode),
      deliveryCustomerName: text(input.deliveryCustomerName),
      billingCustomerId: text(input.billingCustomerId),
      billingCustomerCode: text(input.billingCustomerCode),
      billingCustomerName: text(input.billingCustomerName),
      supplierCustomerId: text(input.supplierCustomerId),
      supplierCustomerCode: text(input.supplierCustomerCode),
      supplierCustomerName: text(input.supplierCustomerName),
      salesCustomerId: text(input.salesCustomerId),
      salesCustomerCode: text(input.salesCustomerCode),
      salesCustomerName: text(input.salesCustomerName),
      rowVoucherDate: text(input.rowVoucherDate),
      rowDeliveryDate: text(input.rowDeliveryDate),
      rowWarehouseId: text(input.rowWarehouseId),
  ...
  function normalizeModeDraft(mode, input = {}, fallback = createModeDraft(mode)) {
    ...
    return {
      ...
      sources: Array.isArray(input.sources) ? input.sources.map(source => ({
        ...
        kind: ['IMAGE', 'TEXT', 'EXCEL_FILE', 'EXCEL_PASTE', 'VOICE'].includes(source?.kind) ? source.kind : 'TEXT',
        displayName: text(source?.displayName) || '원본',
        status: ['REGISTERED', 'ANALYZING', 'STAGED', 'PARTIALLY_APPLIED', 'APPLIED', 'FAILED', 'REMOVED'].includes(source?.status)
          ? source.status
          : 'REGISTERED',
        createdAt: text(source?.createdAt) || new Date().toISOString(),
        analysisRevision: Math.max(0, Number(source?.analysisRevision || 0)),
        parserVersion: text(source?.parserVersion),
        previewRef: text(source?.previewRef),
        previewText: String(source?.previewText ?? ''),
        removedAt: source?.removedAt ? text(source.removedAt) : null,
        failureMessage: text(source?.failureMessage),
        analysisMeta: source?.analysisMeta && typeof source.analysisMeta === 'object' ? { ...source.analysisMeta } : null
      })).filter(source => source.sourceId) : [],
      activeSourceId: text(input.activeSourceId),
      stagedSourceRows: Array.isArray(input.stagedSourceRows) ? input.stagedSourceRows.map(row => ({
        sourceId: text(row?.sourceId),
        sourceRowKey: text(row?.sourceRowKey),
        logicalRowDigest: text(row?.logicalRowDigest),
        analysisRevision: Math.max(1, Number(row?.analysisRevision || 1)),
        state: ['PENDING', 'ERROR', 'ALREADY_APPLIED', 'EXCLUDED', 'APPLIED'].includes(row?.state) ? row.state : 'PENDING',
        values: row?.values && typeof row.values === 'object' ? { ...row.values } : {},
        appliedWorkRowId: text(row?.appliedWorkRowId),
        errorMessage: text(row?.errorMessage)
      })).filter(row => row.sourceId && row.sourceRowKey) : [],
      sourceApplicationLedger: Array.isArray(input.sourceApplicationLedger) ? input.sourceApplicationLedger.map(entry => ({
        sourceId: text(entry?.sourceId),
        sourceRowKey: text(entry?.sourceRowKey),
        logicalRowDigest: text(entry?.logicalRowDigest),
        workRowId: text(entry?.workRowId),
        appliedAt: text(entry?.appliedAt),
        deletedFromWorkTableAt: entry?.deletedFromWorkTableAt ? text(entry.deletedFromWorkTableAt) : null,
        reapplyAuthorizedAt: entry?.reapplyAuthorizedAt ? text(entry.reapplyAuthorizedAt) : null
      })).filter(entry => entry.sourceId && entry.sourceRowKey) : [],
      ...
      purchaseSubmissions: Array.isArray(input.purchaseSubmissions) ? input.purchaseSubmissions.map(pointer => ({
        ...
        receiptKey: text(pointer.receiptKey),
        voucherGroupKey: text(pointer.voucherGroupKey),
        lastErrorCode: text(pointer.lastErrorCode)
      })) : [],
