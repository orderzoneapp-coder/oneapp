...
- 견적서: 거래처를 먼저 선택하지 않아도 전체 견적서를 헤더에서 불러올 수 있다. 기존 견적서를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정하며 `신규·저장/수정·삭제`를 같은 영역에서 처리한다. 품목은 필수지만 수량은 선택값이므로 수량이 비어 있어도 저장할 수 있다. 견적서명은 `거래처명(품목수)`로 생성한다. 별도 상단 견적 목록은 사용하지 않는다. `공지 묶음`에서는 저장 견적서를 최대 3개까지 다중 선택하고 위·아래로 업체 공지 순서를 바꾼다. 첫 번째 업체 견적을 편집표에 표시하며 `카톡 공지 복사`는 선택 순서대로 업체별 공지 페이지를 연속 생성한다. 미리보기 이미지 상단의 단가 필터에서 판매가를 최대 2개 선택해 `품명·규격 / 판매가1 / 판매가2` 열로 출력한다. 마지막 선택값은 다음 미리보기에도 유지하며 필터 자체는 PNG에 포함하지 않는다. 품목 20개까지는 한 줄, 21~40개는 좌우 두 줄, 41개부터는 다음 페이지로 생성한다. `견적 Excel`은 현재 작성값만으로 MerchOps F8의 `쇼핑몰업로드·ERP업데이트·확인요청` 형식을 생성한다.
