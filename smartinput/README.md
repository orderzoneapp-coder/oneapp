...
- 견적서: 거래처를 먼저 선택하지 않아도 전체 카탈로그를 헤더에서 불러올 수 있다. 기존 카탈로그를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정하며 `신규·저장/수정·삭제`를 같은 영역에서 처리한다. 카탈로그명은 `거래처명(품목수)`로 생성한다. 상단 견적 목록은 사용하지 않는다. `카톡 공지 복사`는 직전 저장 공지단가와 비교한 `품명·규격·단가·변동액·적요` 이미지를 페이지별로 만들고, `견적 Excel`은 현재 작성값만으로 MerchOps F8의 `쇼핑몰업로드·ERP업데이트·확인요청` 형식을 생성한다.
