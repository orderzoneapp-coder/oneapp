...
- 표준 그리드: 전체/행 선택, 품목코드, 품목명, 규격, 수량, 단위, 단가, 공급가액, 메모, 적요(직원), 공지단가. Enter는 값 유무와 관계없이 다음 표시 셀로, 방향키는 상하좌우 인접 셀로 이동한다. 헤더 경계 드래그는 열 너비를, 헤더 드래그앤드롭은 현재 전표의 열 순서를 변경해 즉시 저장한다. 표 전체 폭은 표시 열의 실제 합계만 사용해 부족한 우측 공간은 비워 두고, 초과 폭은 입력표 내부 가로 스크롤로만 제공한다.
...
- 전표 표시 열 상품정보에는 `입고B`, `단가D`, `최종입고`, `단가H`, `단가I`를 제공하며 `입고B`는 모든 전표에서 숨길 수 없는 필수 열이다. 마스터 상품을 선택하면 해당 마스터 가격값을 함께 불러온다.
- 전표 작업: 전표 선택은 배경 채움 없이 주황색 라인으로만 현재 전표를 구분한다. 거래처 선택이 완료되면 첫 미입력 품목코드 셀에 커서를 유지한다. `전표 초기화·저장`은 입력표 하단에 배치한다.
