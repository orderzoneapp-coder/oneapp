...
- 견적서: 거래처를 먼저 선택하지 않아도 견적서 필터에서 전체 견적서를 불러올 수 있다. 기존 견적서를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정한다. `저장`은 견적서명 입력 모달을 열며 기본값은 새 견적서의 거래처명 또는 불러온 견적서명이다. 불러온 이름을 유지하면 같은 견적서를 수정하고, 다른 이름이면 목록 최하단에 신규 복사본을 생성한다. 거래처명이나 견적서명의 중복은 허용한다. 필터 목록은 일괄출력 체크박스와 편집 버튼을 제공하며, 편집 모달에서 견적서명·순서를 변경하거나 삭제한다. 카톡 공지는 체크한 견적서만 현재 목록 순서대로 연속 출력한다. 품목은 필수지만 수량은 선택값이므로 수량이 비어 있어도 저장할 수 있다. 미리보기 이미지 상단의 단가 필터에서 판매가를 최대 2개 선택해 `품명·규격 / 판매가1 / 판매가2` 열로 출력한다. 마지막 선택값은 다음 미리보기에도 유지하며 필터 자체는 PNG에 포함하지 않는다. 품목 20개까지는 한 줄, 21~40개는 좌우 두 줄, 41개부터는 다음 페이지로 생성한다. `견적 Excel`은 현재 작성값만으로 MerchOps F8의 `쇼핑몰업로드·ERP업데이트·확인요청` 형식을 생성한다.
