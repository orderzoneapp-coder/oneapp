...
- 일체형 입력: 배송일자·출하창고·거래유형은 상단 좌측, 전표 선택은 상단 우측, 배송 거래처는 전표 입력표 헤더에 둔다. 세무 거래처는 본문에 노출하지 않고 다중 연결 거래처 검색 결과에서만 `세무거래처`로 구분한다.
...
- 견적서: 거래처를 먼저 선택하지 않아도 전체 카탈로그를 헤더에서 불러올 수 있다. 기존 카탈로그를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정하며, 신규 입력 후 저장하면 생성하고 기존 카탈로그는 행 추가·선택 삭제 후 수정 저장한다.
