...
- 입력 방식: 파서 상단은 `음성 / 일치·유사·불일치 상태칩 / 지우기`만 한 줄로 표시한다. 별도 입력 방식 버튼 없이 텍스트 직접입력·Ctrl+V, 텍스트·이미지·Excel 파일 끌어놓기를 파서 전체에서 받아 자동 판별한다. 일반 Excel은 기존 구조화 가져오기를 유지한다. 양식 작업에서는 Excel 파일과 HTML 표/TSV 붙여넣기를 동일한 `ImportMatrix`로 정규화하고 헤더 행을 분석한 뒤 공통 표준 필드에 매핑한다. 사진 OCR은 명암·고대비 전처리와 표 영역 재인식을 수행하고 `수량 × 단가 = 금액` 및 합계가 모두 일치한 행만 전표에 입력한다.
...
- 네 전표 모드의 파일·HTML 표·TSV 붙여넣기는 같은 Matrix Adapter와 Mapping Core를 거쳐 기존 WorkTable, 다중 전표 그룹, writer 계약으로 전달된다.
