...
PR #648의 후속 소스 정리다. 스마트입력 직속 JavaScript는 1차 49개에서 25개로 정리했다(최초 52개). 구현을 아래 파일로 옮기고 이전 파일 25개를 제거하고 report.js 1개를 추가했으며, 별도 배포 번들로 감추거나 DB 형식을 변경하지 않았다.
...
- 견적서: 거래처명은 선택값이며 거래처 없이도 새 견적서를 작성·저장할 수 있다. 견적서 선택 목록에서 여러 견적서를 체크하고 `선택 상품 불러오기`를 누르면 현재 목록 순서대로 상품을 합치고 품목코드(없으면 품명·규격·단위) 중복을 제거해 새 견적서 입력표에 불러온다. 이 조합 결과는 원본 견적서를 바꾸지 않으며 사용자가 새 이름으로 저장한다. 기존 견적서를 직접 불러와 같은 이름으로 저장하면 수정하고, 다른 이름이면 목록 최하단에 신규 복사본을 생성한다. 거래처명이나 견적서명의 중복은 허용한다. 목록 하단 `정보 변경`에서 개별 견적서의 이름과 거래처를 다시 매칭할 수 있으며 저장한 거래처 ID·코드·이름은 레코드와 작업 헤더에 함께 반영되어 다음 목록 선택과 거래처별 업데이트에 사용된다. 지정된 거래처는 같은 견적서를 연속 수정 저장해도 유지된다. 연동견적서는 단일 거래처를 갖지 않으므로 정보 변경에서 이름만 바꾼다. 같은 선택 목록이 카톡과 Excel 출력 범위가 된다. 카톡은 현재 목록 순서대로 견적서별 페이지를 만들며, `F8 EXCEL` 버튼 또는 F8은 현재 화면의 저장 전 작업값을 출력하되 선택된 개별 원본까지 별도로 검증한 뒤 `통합업로드용_QuickF8_YYYY-MM-DD.xlsx`를 생성한다. 시트 순서는 `쇼핑몰업로드` 22열, `ERP업데이트` 11열, 도매가 확인 경고가 있을 때만 `확인요청`, 마지막 `견적서 업로드` 19열이다. `견적서 업로드` 데이터행은 거래처명 → 품목코드 순으로 정렬한다. 쇼핑몰 판매여부는 산출 출고가가 있으면 1, 없으면 0이며 모든 본품·소분품 재고수량은 999다. 선택 원본에 같은 출력 품목코드가 있으면 `중복 품목 모아보기`에서 거래처·원본행·창고·품명·입고가를 나란히 표시하고, 사용자가 대표 행을 선택하거나 해당 입고가(견적단가)를 직접 수정한다. 모든 중복 그룹이 확정되기 전에는 Excel을 생성하지 않으며, 확정 뒤 쇼핑몰·ERP 상품행은 품목코드당 하나만 만들고 `견적서 업로드`의 거래처별 원본 거래행은 모두 보존한다. 매핑 기반 견적의 출고가·시중가와 견적서 업로드 단가는 입고가·외주비·노무비 및 Settings 마진룰로 계산하며, 사용자가 명시적으로 편집한 가격은 유지한다. 일반 단가 `unitPrice`는 ERP 입고가나 도매가로 대체하지 않고 원본에 없는 F8 의미는 공란으로 둔다. MerchOps 작업대에서 사용자가 별도로 예약하는 `입고가없음` 싯가판매·판매정지와 복수 원물 선택은 SmartInput에서 자동 추정하지 않으며, 해당 선택이 필요한 소분 충돌은 확인 후 다시 출력하도록 차단한다. 품목은 필수지만 수량은 선택값이므로 수량이 비어 있어도 저장할 수 있다. 미리보기 이미지 상단의 단가 필터에서 판매가를 최대 2개 선택해 `품명·규격 / 판매가1 / 판매가2` 열로 출력한다. 마지막 선택값은 다음 미리보기에도 유지하며 필터 자체는 PNG에 포함하지 않는다. 품목 20개까지는 한 줄, 21~40개는 좌우 두 줄, 41개부터는 다음 페이지로 생성한다.
- 구매 보고서: 구매전표에서 보고서 버튼을 누르면 DataOps의 판매업로드 변환 규칙으로 `확인요청`, `판매입력`, `전송출고`, `전송구매`, `거래처별`, `단가설정`을 생성하고, 원본 구매행을 공식 20열에 맞춘 `구매 업로드`를 마지막 시트로 추가한다. `구매 업로드` 데이터행은 거래처명 → 코드 순으로 정렬해 거래처별 전표 행을 연속 배치한다. 불러온 Excel의 숨은 원본 열은 `sourceMatrix`에서 보존하고 화면에서 명시 수정한 값만 덮어쓴다. 거래처별 단가 우선순위, 수량 0·음수, 역마진·단가누락 확인 규칙을 유지하며 파일명은 `2전송_판매업로드_생성_YYYYMMDD.xlsx`다.
...
- 견적서 탭에서 ERP 23열 견적서현황 양식을 불러오면 정확한 `견적서현황내역` 시트를 다른 동점 시트보다 우선한다. 전체 시트와 그 부분집합을 함께 읽지 않으며 원본입력뷰에 선택 시트명·거래처 수·품목 수를 표시한다. A:F가 모두 있는 행만 품목행으로 계산하고 생성시각 푸터는 제외한다. 거래처별 갱신 모달은 기존 견적 `UPDATE`, 이름을 입력한 명시적 `CREATE`, `EXCLUDE` 중 작업자가 선택하며 신규 견적을 자동 생성하지 않는다.
...
- 견적서 현황 popup은 기존 개별 견적 exact 연결, 명시적 신규 등록, 이번 작업 제외를 거래처별로 받는다. 성공한 수동 대상은 기존 `customerAliasMappings` Store의 별도 견적서 매칭 유형으로 저장하고, 다음 파일에서 거래처 ID → 코드 → 정규화 이름 순으로 자동 적용한다. 삭제·연동 대상과 충돌 사전은 자동 적용하지 않는다. 완료/변경 없음/확인 필요 진행상태는 현재 draft의 파일·그룹 fingerprint에 묶어 복구하며 새 DB Store나 schema를 만들지 않는다.
...
- 거래처나 창고가 현재 작업과 다르면 확인 전 자동으로 합치지 않는다. 대상 작업본에서 행 추가·선택삭제·개별수정을 해도 원본 전표는 바뀌지 않는다.
