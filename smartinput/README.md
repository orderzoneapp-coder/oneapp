...
- 견적서: 거래처를 먼저 선택하지 않아도 전체 견적서를 헤더에서 불러올 수 있다. 기존 견적서를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정하며 `신규·저장/수정·삭제`를 같은 영역에서 처리한다. 품목은 필수지만 수량은 선택값이므로 수량이 비어 있어도 저장할 수 있다. 견적서명은 `거래처명(품목수)`로 생성한다. 별도 상단 견적 목록은 사용하지 않는다. `카톡 공지 복사`는 환경설정에서 선택한 판매가를 최대 2개까지 `품명·규격 / 판매가1 / 판매가2` 열로 출력한다. 품목 20개까지는 한 줄, 21~40개는 좌우 두 줄, 41개부터는 다음 페이지로 생성한다. `견적 Excel`은 현재 작성값만으로 MerchOps F8의 `쇼핑몰업로드·ERP업데이트·확인요청` 형식을 생성한다.
- 환경설정: 처음에는 모든 그룹을 닫고 `전표별 상단 정보 열·전표별 표시 열·카톡 공지 판매가·배송 정책` 순서로 노출한다. `주문서·구매·판매·견적서`마다 상단 정보 열, 표 열, Enter 입력 순서, 열 너비를 각각 저장한다. 전표 표시 열의 전체 필드 사전은 `품목정보·수량·단가·원가·부가정보` 순서이며 사용자지정 필드는 부가정보에 포함한다. 문자형·숫자형 사용자지정 필드는 각각 최대 10개다.
