...
- 일체형 입력: 주황색 보더의 전표 선택은 앱 헤더 중앙에 두고, 본문 상단정보는 한 줄 헤더로 통합한다. 왼쪽에는 배송일자·출하창고·거래유형, 오른쪽에는 거래처·견적서 선택·저장·카톡·Excel 작업을 배치한다. 별도 `표준 입력표` 제목 영역은 두지 않고 확보한 높이를 입력표에 사용한다. 세무 거래처는 본문에 노출하지 않고 다중 연결 거래처 검색 결과에서만 `세무거래처`로 구분한다.
...
- 브랜드: 공통 배너와 스마트입력 앱 바는 확정 로고 원본을 사용하고 공통 테마의 일반·다크 모드에 맞는 로고를 자동 선택한다.
