...
- 입력 방식: 직접입력, Excel·파일, 텍스트, `Ctrl+V` 텍스트·이미지, 사진 OCR, 음성 STT. Excel은 시트 이름이나 전표 종류에 고정하지 않고 상품 필드명이 가장 많이 일치하는 헤더 행을 찾아 공통 표준 필드로 변환한 뒤 현재 선택한 전표 입력표에 즉시 반영한다. 사진 OCR은 명암·고대비 전처리와 표 영역 재인식을 수행하고 `수량 × 단가 = 금액` 및 합계가 모두 일치한 행만 전표에 입력한다.
...
- 견적서: 거래처를 먼저 선택하지 않아도 전체 견적서를 헤더에서 불러올 수 있다. 기존 견적서를 선택하면 저장된 배송 거래처와 품목을 함께 자동 지정하며 `신규·저장/수정·삭제`를 같은 영역에서 처리한다. 견적서명은 `거래처명(품목수)`로 생성한다. 별도 상단 견적 목록은 사용하지 않는다. `카톡 공지 복사`는 직전 저장 공지단가와 비교한 `품명·규격·단가·변동액·적요` 이미지를 페이지별로 만들고, `견적 Excel`은 현재 작성값만으로 MerchOps F8의 `쇼핑몰업로드·ERP업데이트·확인요청` 형식을 생성한다.
