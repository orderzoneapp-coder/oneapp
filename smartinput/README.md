...
- 입력 집중 작업영역: 기본 화면은 왼쪽 원문 파서와 중앙 상단정보·전표만 사용한다. 저장 견적서와 관련 전표는 입력표 상단의 명시적 버튼을 눌렀을 때만 하나의 팝업으로 연다. 데스크톱에서는 두 기본 영역의 하단을 브라우저 하단에 맞추고 페이지 전체 세로 스크롤을 만들지 않는다. 행이 늘어나면 중앙 입력표 본문만 독립 스크롤하며 전표 상단정보와 하단 저장 작업은 고정한다. 좁은 화면은 기존 세로 문서 흐름을 유지한다.
...
- 견적서: 거래처명은 선택값이며 거래처 없이도 새 견적서를 작성·저장할 수 있다. `견적서 목록` 팝업은 개별 저장 견적서만 간단히 표시하며 한 건을 선택하면 팝업을 닫고 입력표에서 자유롭게 수정한다. 같은 이름으로 저장하면 수정하고, `새 견적서 저장`에서 다른 이름을 입력하면 신규 복사본을 생성한다. 목록에서는 선택한 한 건의 이름 변경과 삭제만 제공한다. 기존 연동견적서 레코드와 내부 호환 로직은 삭제하지 않지만 연동목록·다중선택·연동견적서 생성 UI는 기본 화면에 노출하지 않는다. 품목은 필수지만 수량은 선택값이므로 수량이 비어 있어도 저장할 수 있다. 카톡과 Excel 출력은 현재 작업 중인 견적서를 기준으로 실행한다.
...
- 입력표 상단의 `관련 전표 불러오기`를 누른 경우에만 공통 팝업을 열고 견적·주문·구매·판매 중 원본 전표를 선택한다. 원본 수량과 단가는 대상 전표의 전용 fieldId로 복사하고 원본 voucher/line 증거를 유지한다.
