...
- 앱 헤더 대상 탭: `주문서 / 구매 / 판매 / 견적서` (`Alt+1 / Alt+2 / Alt+3 / Alt+4`). 전표 선택은 본문 상단정보에서 제거하고 앱 헤더 중앙에 고정한다.
...
- 일체형 입력: 주황색 보더의 전표 선택은 앱 헤더 중앙에 두고, 본문 상단정보에는 배송일자·출하창고·거래유형만 연속 배치한다. 배송 거래처는 전표 입력표 헤더에 둔다. 세무 거래처는 본문에 노출하지 않고 다중 연결 거래처 검색 결과에서만 `세무거래처`로 구분한다.
