...
- 견적서 카드 열기·정렬: 카드는 후보 작업본과 입력형·원본형 행을 먼저 검증한 뒤 선택과 자동저장을 확정한다. 열기에 실패하면 기존 편집값·검색·선택·스크롤·포커스를 복원하고, 이전 작업에 미저장 변경이나 저장 예약이 있었다면 추가 입력 없이 자동저장을 다시 예약하며 실패 대상은 저장하지 않는다. `PARTIAL_MISSING`과 `ALL_MISSING`은 복구 필요 상태로 열어 기존 F8 독립 복구를 유지하고 `INVALID`만 차단한다. 카드 이동은 전용 핸들의 내부 드래그만 허용하며 외부 텍스트·파일 드롭은 카드 클릭을 막지 않는다. 순서는 단일 preimage 검증 트랜잭션이 성공한 뒤에만 화면과 메모리에 반영한다.
