...
var NEXUS_PUBLIC_COMPANY_ACTION = 'nexus_public_company_snapshot';
var NEXUS_PUBLIC_COMPANY_CACHE_KEY = 'NEXUS_PUBLIC_COMPANY_SNAPSHOT_ONEAPP_V1';
var NEXUS_PUBLIC_COMPANY_CACHE_TTL_SECONDS = 60;
var NEXUS_PUBLIC_COMPANY_SNAPSHOT_KEYS = Object.freeze([
...
function doPost(e) {
  try {
    ...
    if (action === NEXUS_PUBLIC_COMPANY_ACTION) return nexusAuthPublicCompanySnapshot_(payload);
...
function nexusAuthPublicCompanySnapshot_(payload) {
  var allowedKeys = ['action', 'knownRevision'];
  ...
  var requestId = 'PUB-' + Utilities.getUuid();
  var forwarded = {
    action: 'nexus_gateway_company_public_profile_get',
    token: credential.token,
    actorId: 'NEXUS_GATEWAY',
    roleIds: ['FOUNDATION_READ'],
    deviceId: 'NEXUS_GATEWAY',
    device: 'NEXUS_GATEWAY',
    environment: 'PRODUCTION',
    scope: { companyId: 'ONEAPP' },
    requestId: requestId,
    nexusRequest: {
      requestId: requestId,
      subjectUserId: 'PUBLIC_FOOTER',
      subjectLoginId: 'PUBLIC_FOOTER',
      appId: 'public-footer',
...
function nexusAuthPublicCompanyProject_(source) {
  if (!source || typeof source !== 'object' || Array.isArray(source)) return null;
  var sourceKeys = Object.keys(source).sort();
  var expectedKeys = NEXUS_PUBLIC_COMPANY_SNAPSHOT_KEYS.slice().sort();
  if (sourceKeys.length !== expectedKeys.length || sourceKeys.some(function (key, index) { return key !== expectedKeys[index]; })) return null;
  var snapshot = NEXUS_PUBLIC_COMPANY_SNAPSHOT_KEYS.reduce(function (result, key) {
    result[key] = key === 'revision' ? Number(source[key]) : nexusAuthText_(source[key]);
    return result;
  }, {});
  if (!Number.isInteger(snapshot.revision) || snapshot.revision < 1 || !snapshot.companyName
      || !snapshot.businessNumber || !snapshot.representativeName || !snapshot.businessAddress) return null;
  return snapshot;
}

function nexusAuthPublicCompanyCacheRead_() {
  try {
    return nexusAuthPublicCompanyProject_(JSON.parse(CacheService.getScriptCache().get(NEXUS_PUBLIC_COMPANY_CACHE_KEY) || 'null'));
  } catch (_) {
    CacheService.getScriptCache().remove(NEXUS_PUBLIC_COMPANY_CACHE_KEY);
    return null;
  }
}

function nexusAuthPublicCompanyCacheWrite_(snapshot) {
  var projected = nexusAuthPublicCompanyProject_(snapshot);
  if (!projected) return false;
  CacheService.getScriptCache().put(NEXUS_PUBLIC_COMPANY_CACHE_KEY, JSON.stringify(projected), NEXUS_PUBLIC_COMPANY_CACHE_TTL_SECONDS);
  return true;
}

function nexusAuthPublicCompanyCacheInvalidate_() {
  CacheService.getScriptCache().remove(NEXUS_PUBLIC_COMPANY_CACHE_KEY);
}

function nexusAuthPublicCompanyCacheAfterGateway_(operationId, data) {
  if (operationId === 'company.profile_write') {
    var writeSnapshot = nexusAuthPublicCompanyProject_(data && data.publicSnapshot);
    if (writeSnapshot) return nexusAuthPublicCompanyCacheWrite_(writeSnapshot);
    nexusAuthPublicCompanyCacheInvalidate_();
    return false;
  }
  if (/^company\.(accounting_period_write|migrate_oneapp)$/.test(operationId)) {
    nexusAuthPublicCompanyCacheInvalidate_();
  }
  return false;
}

function nexusAuthPublicCompanyResponse_(data) {
  return nexusAuthJson_({ status: 'success', action: NEXUS_PUBLIC_COMPANY_ACTION, data: data });
}
...
function nexusAuthGateway_(payload) {
  ...
  try {
    ...
    nexusAuthPublicCompanyCacheAfterGateway_(operationId, parsed.data === undefined ? parsed : parsed.data);
