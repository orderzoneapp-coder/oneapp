(() => {
  ...
  const sessionScope = session => text(session?.user?.userId);
  ...
  const revalidate = ({ force = false } = {}) => {
    ...
    if (!auth?.ready || typeof auth.gateway !== 'function') return Promise.resolve(currentSnapshot);
    refreshPromise = Promise.resolve(auth.ready)
      .then(session => {
        if (!session?.user) {
          applySessionScope(null, 'auth-cleared');
  ...
  window.addEventListener('nexus-auth-ready', event => applySessionScope(event.detail));
