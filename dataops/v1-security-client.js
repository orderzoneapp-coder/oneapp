(function(global){
  ...
  function evaluate(ping,expected=EXPECTED_DEPLOYMENT){const evidence=Boolean(text(ping?.deploymentVersion)&&text(ping?.gitCommit)&&(!text(expected.deploymentVersion)||text(ping?.deploymentVersion)===text(expected.deploymentVersion))&&(!text(expected.gitCommit)||text(ping?.gitCommit)===text(expected.gitCommit)));return Boolean(released(expected)&&text(ping?.deploymentId)===text(expected.deploymentId)&&evidence&&ping?.capabilityVersion===CAPABILITY&&ping?.mode===CUTOVER&&ping?.readAuthRequired===true&&ping?.writeAuthRequired===true&&JSON.stringify(ping?.roles)===JSON.stringify(['DATAOPS_SNAPSHOT_V1_READ','DATAOPS_SNAPSHOT_V1_WRITE'])&&JSON.stringify(ping?.actions)===JSON.stringify(['dataops_snapshot_get','dataops_snapshot_commit']));}
