  ...
  function createDataOpsCloseOperator(options = {}) {
    ...
    async function start(context = {}) {
      ...
      try {
        ...
        const closeSnapshotA = canonical({ businessDate, scope: connection.dataOpsCredential.scope, contextDigest: authoritativeContext.contextDigest, productData, dataops: { session: sources.dataops.session, head: sources.dataops.head, rows: sources.dataops.rows }, orderq: { session: sources.orderq.session, head: sources.orderq.head, entities: sources.orderq.entities || [] } });
    ...
    async function confirm() {
      ...
      try {
        ...
        const staged=[];for(const [kind,value] of [['A',draft.closeSnapshotA],['B',resultB],['ISSUES',draft.review.issues||[]],['DECISIONS',finalized.decisions],['REPORT',finalized.reportManifest],['BASELINES',finalized.approvedOpeningBaselines||[]]]){const prepared=await Promise.all(chunks(kind,value).map(async chunk=>({chunkIndex:chunk.chunkIndex,content:chunk.content,chunkDigest:await sha256(chunk.content)})));staged.push({kind,value,chunks:prepared});}
