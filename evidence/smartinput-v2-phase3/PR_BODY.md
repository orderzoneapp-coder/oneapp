...
- PM G3 보완: native date 삭제 시 기존 연·월 1일 복구, Snapshot 표시/원문 trim-only 보존, 구매 V2 Finalize preflight-first/V1 legacy 유지
...
- 비-Snapshot `reason`만 변경한 동일 commandId를 Gateway/Repository 각각 정확한 payload conflict로 차단
- 실제 날짜 삭제 뒤 화면/호환 작업본/IndexedDB 자동저장/공식 구매 저장 요청 모두 `YYYY-MM-01`; 전각·호환문자 Snapshot 저장 불변
- DOM/단축키 동일, 구매·판매 클릭 수 각각 6→6, 실제 외부 변경 요청 0, console error 0

## 범위 통제

- HTML/CSS/DOM/버튼/단축키 변경 없음; 기존 date input event와 비가시 작업본 필드만 사용
