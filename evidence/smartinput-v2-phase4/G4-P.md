...
- 회사와 사용자 상품코드가 정확히 일치한 활성 상품만 매칭했다. 이름-only 및 미등록 코드 행은 자동매칭하지 않았다.
...
- 정확매칭 거래처에서 최종금액 `2,661` 기준 기본 채무 1건을 만들었다. 거래처 없음·미매칭은 별도 순수 계약 검사에서 전표 확정과 원장 미생성 사유를 확인했다.
