...
- 회사+사용자 상품코드 exact-only resolver 추가; 이름 자동매칭 금지, ERP 코드 원문/선행 0 보존
...
- 회사+거래처코드 exact-only 매칭 때만 최종금액 기준 구매 채무/판매 채권 생성; 없음·미매칭은 Revision에 미생성 이유 보존
...
- 실제 Chrome/IndexedDB E2E PASS: 구매 `+10`, `0 ZERO_EFFECT`, 미매칭 pending 3/공식재고 0, 동일 미매칭 검수 링크 2, 채무 `2,661`, 판매 `-4`, 미매칭 거래처 채권 0 및 Revision 사유 보존
