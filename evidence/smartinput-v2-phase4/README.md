...
- ERP 코드 원문과 앞자리 0을 보존한다. 기존 `productId`는 정확매칭 결과가 있을 때만 비노출 기술키로 전달하며, 입력에 남아 있던 stale ID는 매칭 결과로 덮어쓰거나 제거한다.
...
- 거래처는 선택 입력이다. 정확한 회사별 코드 매칭 때만 구매 채무 또는 판매 채권을 최종금액 기준으로 만든다. 없음·미매칭이면 전표는 확정하고 원장효과는 만들지 않으며 Revision의 `partnerEffectDecision`에 이유를 남긴다.
...
- `node scripts/test-smartinput-v2-inventory-unresolved-ledger.mjs`: PASS. 구매·판매 정확/미매칭/이름-only/0·음수/선행 0/거래처 없음·미매칭/Revision 사유/원자성/멱등성/owner 쓰기 금지를 검증했다.
...
- 브라우저 구매 결과: 정확매칭 재고 `+10 APPLIED_NORMAL`, `0 ZERO_EFFECT`; 미매칭 3행의 공식재고 0건·pending 3건; unresolved 대상 2건과 링크 수 `2/1`; 정확 거래처 채무 1건 `2,661`; 같은 명령 재시도 후 문서 1·Revision 1·명령 1·queue 1·재고효과 2로 유지됐다.
- 브라우저 판매 결과: 정확매칭 재고 `-4`; 미매칭 거래처 채권 0건; Revision 사유 `CUSTOMER_CODE_UNMATCHED`.
